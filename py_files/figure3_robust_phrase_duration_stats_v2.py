...
def load_metadata(
    ...
    preferred_sheet: Optional[str] = "animal_hit_type_summary",
    preferred_hit_col: Optional[str] = None,
) -> pd.DataFrame:
    """Load animal-level lesion groups from metadata.

    The earlier v1 script could accidentally choose the first sheet containing
    "Animal ID" and then use "Treatment type" as the group column. In your
    metadata that can classify only saline birds correctly and leave lesion birds
    as unknown, which produces empty/NaN group contrasts.

    This version scores all sheets and all plausible lesion-group columns, and
    prefers the sheet/column that maps the most animals into sham, lateral-only,
    partial ML, or complete ML groups. You can also force the sheet/column with
    --metadata-sheet and --metadata-hit-col.
    """
    ...
    hit_candidates = [
        ...
        "lesion_hit_type_grouping",
        "Medial/Lateral hit type",
        "Medial hit type",
        "Lateral hit type",
        "Group",
        "lesion_group",
        "Treatment group",
        # Low-priority fallback only; this is often too coarse.
        "Treatment type",
    ]

    def score_candidate(df: pd.DataFrame, sheet: str, hit_col: str) -> tuple[int, int, int, int]:
        groups = df[hit_col].apply(canonical_group)
        n_known = int(groups.isin([SHAM, LATERAL, PARTIAL_ML, COMPLETE_ML, POOLED_ML]).sum())
        n_ml = int(groups.isin([PARTIAL_ML, COMPLETE_ML, POOLED_ML]).sum())
        n_lateral = int((groups == LATERAL).sum())
        name = hit_col.lower()
        # Strongly prefer explicit lesion/hit columns and the summary sheet.
        name_bonus = 0
        if "hit" in name:
            name_bonus += 30
        if "lesion" in name:
            name_bonus += 20
        if "treatment type" in name:
            name_bonus -= 40
        sheet_bonus = 25 if str(sheet).lower() == str(preferred_sheet).lower() else 0
        return (n_known + name_bonus + sheet_bonus, n_ml, n_lateral, -hit_candidates.index(hit_col) if hit_col in hit_candidates else 0)

    candidate_records = []

    for sheet in xls.sheet_names:
        if preferred_sheet is not None and str(sheet).lower() != str(preferred_sheet).lower():
            # Still inspect non-preferred sheets later, but do not skip them.
            pass
        try:
        ...
        if preferred_hit_col is not None:
            possible = [preferred_hit_col] if preferred_hit_col in df.columns else []
        else:
            possible = [c for c in hit_candidates if c in df.columns]
            # Add fuzzy hit/lesion columns not listed above.
            for c in df.columns:
                cl = str(c).lower()
                if c not in possible and (("hit" in cl and "type" in cl) or ("lesion" in cl and "group" in cl)):
                    possible.append(c)

        for hit_col in possible:
            sc = score_candidate(df, sheet, hit_col)
            candidate_records.append((sc, sheet, hit_col, df.copy()))

    if not candidate_records:
        raise ValueError(
            f"Could not find a usable metadata sheet/column in {metadata_excel}. "
            f"Need {animal_col!r} and a lesion hit/group column."
        )

    candidate_records.sort(key=lambda x: x[0], reverse=True)
    score, sheet, hit_col, df = candidate_records[0]

    out = pd.DataFrame()
    out["animal_id"] = df[animal_col].astype(str).str.strip()
    out["raw_hit_type"] = df[hit_col]
    out["display_group"] = df[hit_col].apply(canonical_group)
    out["_metadata_sheet"] = sheet
    out["_metadata_hit_col"] = hit_col
    ...
    if pct_col:
        out["percent_area_x_lesioned"] = df[pct_col]
    if date_col:
        out["treatment_date"] = df[date_col]

    out = out.dropna(subset=["animal_id"]).drop_duplicates("animal_id")

    print(f"[INFO] Metadata group source: sheet={sheet!r}, column={hit_col!r}")
    print("[INFO] Metadata group counts:")
    print(out["display_group"].value_counts(dropna=False).to_string())

    # Warn if one of the essential groups is missing.
    groups_present = set(out["display_group"].astype(str))
    missing_groups = [g for g in [SHAM, LATERAL, PARTIAL_ML, COMPLETE_ML] if g not in groups_present]
    if missing_groups:
        warnings.warn(
            "Metadata did not classify all expected groups. Missing: "
            + ", ".join(missing_groups)
            + ". If this looks wrong, rerun with --metadata-sheet and --metadata-hit-col."
        )
...
def plot_robustness_contrast(contrast_df: pd.DataFrame, out_path: Path) -> Path:
    ...
    if not np.any(np.isfinite(x)):
        ...
        ax.text(
            0.5,
            0.5,
            "No finite ML vs lateral contrasts.\nCheck metadata group mapping.",
            ha="center",
            va="center",
            fontsize=12,
        )
...
def plot_selected_counts(animal_table: pd.DataFrame, out_path: Path) -> Path:
    ...
    if df.empty:
        return out_path
    if df["display_group"].nunique() < 2:
        warnings.warn(
            "Selected-count plot has fewer than two recognized groups. "
            "This usually indicates metadata group mapping failed."
        )
...
def main() -> None:
    ...
    parser.add_argument("--metadata-sheet", default="animal_hit_type_summary",
                        help="Preferred metadata sheet containing animal-level lesion hit types.")
    parser.add_argument("--metadata-hit-col", default=None,
                        help="Optional exact metadata column to use for lesion hit/group labels.")
    ...
    metadata = (
        load_metadata(
            ...
            preferred_hit_col=args.metadata_hit_col,
        )
        if args.metadata_excel
        else pd.DataFrame(columns=["animal_id", "display_group"])
    )
    if not metadata.empty:
        metadata.to_csv(out_dir / "metadata_group_mapping_used.csv", index=False)
