#!/usr/bin/env python3
"""
cluster_pitch_entropy_panels_v12_segment_qc_figures.py
...
5) optional pitch-contour FFT segmentation CSVs, visual segmentation guides, and segmented-syllable QC figures
Assumptions:
- NPZ contains:
...
def plot_segment_qc_figure(
        ...
        max_freq_khz=12.0,
        max_segments=12):
    """Plot cropped spectrograms and per-segment feature traces for FFT-segmented syllable candidates."""
    ...
    fig, axes = plt.subplots(
        n_segments * 3,
        1,
        figsize=(18, max(4.8 * n_segments, 7)),
        squeeze=False,
    ...
    for i, ent in enumerate(entries):
        ...
        X = ent["X"]
        pitch_khz = ent["pitch_khz"]
        filtered_pitch_khz = ent["filtered_pitch_khz"]
        pitch_deriv_khz_per_s = ent["pitch_deriv_khz_per_s"]
        wiener = ent["wiener"]
        total_power_norm = ent["total_power_norm"]
        ...
        if i == 0:
            spec_ax.set_title(
                f"{animal_id} — label {target_label} — {pretty_period} segmented syllable QC\n"
                f"Each candidate segment shows its cropped spectrogram plus pitch, pitch derivative, Wiener entropy, and total power.",
                fontsize=13
            )
    ...
    axes[-1].set_xlabel("Time within segmented candidate (s)")
    plt.tight_layout()
...
def main():
    ...
    parser.add_argument("--segment-qc-max-segments", type=int, default=12,
                        help="Maximum number of segmented-syllable QC examples to show per period.")
