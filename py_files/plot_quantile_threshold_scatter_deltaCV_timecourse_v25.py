...
def plot_delta_cv_boxplot(bird_df: pd.DataFrame, stat_df: pd.DataFrame, q: float, out_dir: Path, dpi: int, show: bool):
    ...
    def draw_one(show_points: bool, outfile_name: str):
        ...
        ax.set_title(f"{qname} threshold", fontsize=16)
        apply_style(ax)

        xpos_lookup = {g: i for i, g in enumerate(GROUP_ORDER, start=1)}
        stat_df_local = stat_df.copy()
        stat_df_local["_order"] = stat_df_local["comparison"].map({
    ...
    draw_one(show_points=True, outfile_name=f"{qname}_deltaCV_boxplot_by_group.png")
    draw_one(show_points=False, outfile_name=f"{qname}_deltaCV_boxplot_by_group_boxesonly.png")
