#!/usr/bin/env python3
"""
phrase_position_acoustic_crosscorr_v13_local_rate_qc.py
...
def extract_global_segment_spectrogram(X: np.ndarray, start_bin, end_bin) -> Optional[np.ndarray]:
    try:
        s0 = int(start_bin)
        s1 = int(end_bin)
    except Exception:
...
def _plot_spectrogram_on_axis(ax, X_seg: Optional[np.ndarray], args, title: str) -> None:
    ...
    Y = transform_spectrogram_for_corr(Xc, spec_scale=args.spec_scale, corr_transform=args.qc_plot_transform)
    vmin, vmax = robust_minmax(Y)
    if not np.isfinite(vmin) or not np.isfinite(vmax) or vmax <= vmin:
        vmin, vmax = np.nanmin(Y), np.nanmax(Y)
    extent = [0.0, Xc.shape[0] * (args.bin_ms / 1000.0), freq_min if freq_min is not None else 0.0, freq_max if freq_max is not None else args.max_freq_khz]
    ax.imshow(np.asarray(Y, dtype=float).T, origin="lower", aspect="auto", interpolation="nearest", extent=extent, vmin=vmin, vmax=vmax)
    ax.set_title(title, fontsize=10)
    ax.set_xlabel("Time within syllable (s)")
...
def make_local_pair_qc_outputs(pair_df: pd.DataFrame, X: np.ndarray, out_dir: Path, prefix: str, args) -> None:
    ...
    with PdfPages(qc_pdf) as pdf:
        for _, row in qc_rows.iterrows():
            X_a = extract_global_segment_spectrogram(X, row.get("pair_start_segment_global_start_bin"), row.get("pair_start_segment_global_end_bin"))
            X_b = extract_global_segment_spectrogram(X, row.get("pair_end_segment_global_start_bin"), row.get("pair_end_segment_global_end_bin"))
            ...
            _plot_spectrogram_on_axis(axes[0], X_a, args, title=title_a)
            _plot_spectrogram_on_axis(axes[1], X_b, args, title=title_b)
            ...
            label = row.get("label", "")
            subtitle = (
                f"{row.get('animal_id', '')} label {label} | {period} | phrase {phrase_id} | "
                f"lag={int(lag_val) if np.isfinite(lag_val) else 'NA'} | "
                f"corr={corr_val:.3f}" if np.isfinite(corr_val) else
                f"{row.get('animal_id', '')} label {label} | {period} | phrase {phrase_id}"
            )
