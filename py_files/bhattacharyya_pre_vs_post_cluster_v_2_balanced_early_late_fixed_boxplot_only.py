...
4) Optional plots + stats (can be disabled with --no-plots):
     ...
     - Bird-level paired boxplot-only figure (no connecting lines), with
       significance shown as stars / n.s. above each panel.
...
def _add_variance_tier_column(
    ...
    *,
    ...
    # Merge tiers onto df rows (avoid clobbering df keys)
...
def _boxplot_bc_by_group(
    ...
    *,
    ...
    show_p_values_in_annotations: bool = False,
) -> None:
    ...
    if show_stats and len(groups) >= 2:
        ...
        for level, (_span, k) in enumerate(spans):
            ...
            label = f"{stars} (p={p_adj:.3g})" if show_p_values_in_annotations else stars
            y = base + level * step
...
def _paired_drift_plot_birdlevel(df: pd.DataFrame, out_png: Path, out_txt: Path, *, title: str = "Paired drift BC comparison within birds by hit type") -> None:
...
def _paired_drift_boxplot_only_birdlevel(
    df: pd.DataFrame,
    out_png: Path,
    out_txt: Path,
    *,
    ...
    pre = df[df["comparison"] == "pre_early_vs_late"].groupby(
        ["animal_id", "group"], as_index=False
    )["bhattacharyya_coeff"].mean()
    post = df[df["comparison"] == "post_early_vs_late"].groupby(
        ["animal_id", "group"], as_index=False
    )["bhattacharyya_coeff"].mean()
    merged = pd.merge(pre, post, on=["animal_id", "group"], suffixes=("_pre", "_post"))
    ...
    stats_lines.append(title)
    stats_lines.append("Metric: Bhattacharyya coefficient (higher = more similar)")
    stats_lines.append("Figure type: boxplots only (no paired connecting lines)")
    stats_lines.append("")
    ...
    for ax, g in zip(axes, _GROUP_ORDER):
        ...
        finite_vals = np.concatenate([y1[np.isfinite(y1)], y2[np.isfinite(y2)]])
        if finite_vals.size:
        ...
        if _HAVE_SCIPY and len(sub) >= 2:
            try:
                ...
                y_span = max(1e-9, ymax - y_lower)
                base = ymax + max(0.04, 0.08 * y_span)
                h = max(0.02, 0.04 * y_span)
                _add_sig_bracket(ax, 1, 2, base, h, _p_to_stars(p))

                text_pad = max(0.015, 0.6 * h)
                top_needed = base + h + text_pad + max(0.05, 0.08 * y_span)
                ax.set_ylim(y_lower, min(1.40, max(y_upper_base, top_needed)))
            except Exception as e:
                stats_lines.append(f"[{g}] n_birds={len(sub)} Wilcoxon failed: {repr(e)}")
                ax.set_ylim(y_lower, min(1.40, y_upper_base))
...
def run_root_directory(cfg: PrePostBCConfig) -> Dict[str, str]:
    ...
    if cfg_eff.make_plots and len(df) > 0:
        ...
        for tier_label, df_plot in plot_specs:
            ...
            paired_box_png = tier_dir / f"paired_drift_BC_pre_vs_post_boxplots_only_{cfg_eff.array_key}_{cfg_eff.cov_type}_{stamp}.png"
            paired_box_txt = tier_dir / f"paired_drift_BC_pre_vs_post_boxplots_only_{cfg_eff.array_key}_{cfg_eff.cov_type}_{stamp}.txt"
            _paired_drift_boxplot_only_birdlevel(
                df_plot,
                paired_box_png,
                paired_box_txt,
                title=f"Paired drift BC comparison within birds by hit type ({tier_descr})",
