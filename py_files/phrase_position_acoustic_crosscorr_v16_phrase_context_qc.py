#!/usr/bin/env python3
"""
phrase_position_acoustic_crosscorr_v16_phrase_context_qc.py
...
def _plot_spectrogram_on_axis(
    ...
    if seg_start_rel_bin is not None and seg_end_rel_bin is not None:
        ...
        ax.axvline(x0, color="red", linestyle="--", linewidth=1.1, alpha=0.85)
        ax.axvline(x1, color="red", linestyle="--", linewidth=1.1, alpha=0.85)
...
def _transform_full_phrase_spectrogram(X_phrase: Optional[np.ndarray], args) -> Optional[np.ndarray]:
    if X_phrase is None or np.asarray(X_phrase).size == 0:
        return None
    freq_min = args.qc_plot_freq_min_khz if args.qc_plot_freq_min_khz is not None else args.corr_freq_min_khz
    freq_max = args.qc_plot_freq_max_khz if args.qc_plot_freq_max_khz is not None else args.corr_freq_max_khz
    Xc = crop_frequency_band(
        ...
        for _, row in qc_rows.iterrows():
            ...
            for _, srow in sub.iterrows():
                ...
                if rep_i in highlight_repeats:
                    ax.axvspan(x0, x1, color='tab:red', alpha=0.15, lw=0)
                    ax.axvline(x0, color='tab:red', linestyle='--', linewidth=1.4, alpha=0.95)
                    ax.axvline(x1, color='tab:red', linestyle='--', linewidth=1.4, alpha=0.95)
                    txt_color = 'tab:red'
                    fw = 'bold'
                else:
                    ax.axvline(x0, color='dodgerblue', linestyle='--', linewidth=0.9, alpha=0.7)
                    ax.axvline(x1, color='dodgerblue', linestyle='--', linewidth=0.9, alpha=0.7)
...
def make_local_pair_qc_outputs(pair_df: pd.DataFrame, segment_df: pd.DataFrame, X: np.ndarray, out_dir: Path, prefix: str, args) -> None:
    ...
    with PdfPages(qc_pdf) as pdf:
        for _, row in qc_rows.iterrows():
            ...
            x_max_pair = None
            if getattr(args, "qc_match_pair_time_axis", True):
