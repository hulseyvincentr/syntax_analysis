...
Added (this update)
-------------------
A) Permutation tests on log-ratios between groups (NMA vs SHAM):

    log_ratio = log10(post / pre)
    observed_diff = mean(log_ratio_nma) - mean(log_ratio_sham)

    One-tailed (NMA > SHAM):
        p_one = P(perm_diff >= observed_diff)

    Two-tailed (NMA != SHAM):
        p_two = P(|perm_diff| >= |observed_diff|)

Printed to terminal AND appended to the right-side annotation block
(under the legend) on the discrete + continuous plots.

B) Ratio vs lesion % plot remains: y = post/pre, x = lesion_pct (% Area X lesioned),
and includes SHAM (red) + NMA (blue).
"""
...
# -----------------------------------------------------------------------------
# Permutation tests on mean log-ratio difference (NMA - SHAM)
# -----------------------------------------------------------------------------
def permutation_tests_mean_diff(
    ...
    *,
    ...
    Compute BOTH:
      ...
      - two-tailed p-value for (group2 != group1)

    Parameters
    ----------
    group1 : array-like
        Log-ratios (log10(post/pre)) for control group (sham)
    group2 : array-like
        Log-ratios (log10(post/pre)) for experimental group (NMA)
    n_permutations : int
    ...
    p_one_tailed : float
        One-tailed p-value: P(perm_diff >= observed_diff)
    p_two_tailed : float
        Two-tailed p-value: P(|perm_diff| >= |observed_diff|)
    """
    ...
    obs_abs = float(np.abs(observed_diff))
    ...
    for _ in range(int(n_permutations)):
        ...
        if perm_diff >= observed_diff:
            count_one += 1
        if abs(perm_diff) >= obs_abs:
            count_two += 1

    p_one = count_one / float(n_permutations)
    p_two = count_two / float(n_permutations)
    return observed_diff, float(p_one), float(p_two)
...
def _format_permtest_block(
    *,
    ...
    if not (np.isfinite(observed_diff) and np.isfinite(p_one) and np.isfinite(p_two)):
        return "Permutation tests: insufficient data"
    return (
        "Permutation tests on log10(post/pre)\n"
        f"SHAM mean log-ratio={mean_sham:.4f} (n={n_sham})\n"
        f"NMA  mean log-ratio={mean_nma:.4f} (n={n_nma})\n"
        f"Observed diff (NMA-SHAM)={observed_diff:.4f}\n"
        f"One-tailed (NMA > SHAM) p={p_one:.4f}\n"
        f"Two-tailed (NMA != SHAM) p={p_two:.4f}"
    )
...
def plot_pre_post_variance_scatter(
    *,
    ...
    nma_token: str = "nma",
    # permutation tests on log-ratios (adds one-tailed AND two-tailed)
    do_perm_test: bool = True,
    perm_n_permutations: int = 10000,
    perm_seed: int = 0,
    annotate_perm_test: bool = True,
    # ratio vs lesion percent (flag name kept for backward compatibility)
    ...
    if log_scale:
        merged = merged.loc[(merged["pre_variance"] > 0.0) & (merged["post_variance"] > 0.0)].copy()
    ...
    merged["lesion_pct"] = lesion_list  # % Area X lesioned
    ...
    if do_perm_test:
        ...
        permtest_results = {
            ...
            "observed_diff_nma_minus_sham": float(observed_diff),
            "p_value_one_tailed_nma_gt_sham": float(p_one),
            "p_value_two_tailed_nma_ne_sham": float(p_two),
        ...
        print("[PERMUTATION TESTS on log10(post/pre)]")
        print(f"Sham mean log-ratio: {mean_sham:.4f}")
        print(f"NMA mean log-ratio:  {mean_nma:.4f}")
        print(f"Observed difference (NMA - sham): {observed_diff:.4f}")
        print(f"One-tailed p-value (NMA > sham):  {p_one:.4f}")
        print(f"Two-tailed p-value (NMA != sham): {p_two:.4f}")

        if np.isfinite(p_one) and p_one < 0.05:
            print("One-tailed result: NMA significantly increases variance relative to sham")
        else:
            print("One-tailed result: No significant difference detected")
        print()
    ...
    if (metadata_excel is not None) and make_ratio_vs_volume_plots:
        ...
        if verbose:
            ...
            print("[INFO] Ratio-vs-% plot points by treatment_group (all outliers):",
...
def _build_argparser():
    ...
    # permutation tests
    p.add_argument("--no_perm_test", action="store_true", help="Disable permutation tests on log-ratios.")
    p.add_argument("--perm_n", default=10000, type=int, help="Number of permutations.")
    p.add_argument("--perm_seed", default=0, type=int, help="Permutation RNG seed.")
    p.add_argument("--no_perm_annot", action="store_true", help="Do not include permutation test block on plots.")

    # ratio vs lesion percent (flag name kept for backward compatibility)
    p.add_argument("--no_ratio_vs_volume", action="store_true", help="Disable ratio-vs-lesion-% plot.")
    p.add_argument("--ratio_logy", action="store_true", help="Log-scale y-axis for ratio plot.")
    p.add_argument("--volume_logx", action="store_true", help="Log-scale x-axis for % plot (usually leave off).")
...
if __name__ == "__main__":
    ...
    if res.get("permtest_results"):
        print("Permutation test results:", res["permtest_results"])
...
res = og.plot_pre_post_variance_scatter(
    ...
    # optional filter
    min_n_phrases=10,         # set 0 to disable
    ...
    # identity-line agreement stats (SHAM vs NMA)
    compute_identity_stats=True,
    annotate_identity_stats=True,
    nma_token="nma",          # token used to identify NMA rows in your metadata

    # ratio vs % lesioned plot
    make_ratio_vs_volume_plots=True,   # flag name kept for compatibility (it plots % lesioned)
    ratio_logy=False,                  # set True if you want log y-axis
    volume_logx=False,                 # usually False for %; True only if you want log-x
...
print("Identity stats dict keys:", (res.get("identity_stats") or {}).keys())
