...
# -*- coding: utf-8 -*-
"""
stutter_syllable_entropy_compare_batch_from_stats.py

Batch syllable-level comparison (stutter vs non-stutter contexts) across birds.

What it does
------------
For each bird found in --root-dir (or passed via --birds):
  1) Loads merged song annotations (decoded_database + song_detection)
  2) Uses a syllable stats lookup CSV (e.g., usage_balanced_phrase_duration_stats.csv)
     to compute a per-syllable duration threshold (PRE stats):
        thr(L) = Pre_Median_ms + k * Pre_IQR_ms
     where k depends on --threshold-method.
  3) Optionally restricts syllables to the top X% by variance (per bird) from the same stats CSV
     (e.g., top 30% by Variance_ms2) to focus on high-variance syllables.
  4) Within the requested scope (default POST), classifies each occurrence of syllable L as:
        stutter if duration_ms > thr(L)
        non-stutter otherwise
  5) For each syllable L, computes incoming/outgoing neighbor entropies separately for:
        stuttered occurrences vs non-stuttered occurrences
     and saves per-bird plots + a combined aggregate plot across birds.

Outputs
-------
For each bird:
  CSV: <out-dir>/<bird>_<baseprefix>__syllable_compare.csv
  PNG:
    <bird>_<baseprefix>__outgoing_nonstutter_x_vs_stutter_y.png
    <bird>_<baseprefix>__incoming_nonstutter_x_vs_stutter_y.png
    (optional) delta bars if --include-delta-bars

Aggregate across birds:
  CSV: <out-dir>/<baseprefix>__ALL_BIRDS__syllable_compare.csv
  PNG:
    <baseprefix>__ALL_BIRDS__outgoing_nonstutter_x_vs_stutter_y.png
    <baseprefix>__ALL_BIRDS__incoming_nonstutter_x_vs_stutter_y.png

Color-coding
------------
Points are color-coded by lesion type (looked up from --metadata-xlsx using --hit-type-sheet/--hit-type-col).
If lesion type is missing, uses "UNKNOWN".

Notes
-----
- This script expects your merge builder to exist in your codebase:
    merge_annotations_from_split_songs.build_decoded_with_split_labels
  (same as your other analysis scripts).
"""
...
def _find_birds_in_root(root: Path) -> Dict[str, Dict[str, Path]]:
    """
    Returns mapping: bird -> {"decoded": Path, "song_det": Path}
    Looks for *_decoded_database.json; pairs with *_song_detection.json.
    """
    ...
    for dec in decoded_files:
        ...
        if not song.exists():
            # try within bird directory
            song_hits = list(dec.parent.rglob(f"{bird}_song_detection.json"))
            if song_hits:
                song = song_hits[0]
            else:
                # try anywhere in root
...
# -----------------------------
# Stats-derived thresholds + variance filter
# -----------------------------
...
# -----------------------------
# Core compare: stutter vs non-stutter contexts
# -----------------------------
...
def _lesion_color_map(lesion_types: Sequence[str]) -> Dict[str, Any]:
    """
    Deterministic mapping from lesion type strings to matplotlib colors.
    """
    uniq = sorted(set(lesion_types), key=str)
    cmap = plt.get_cmap("tab10")
    colors: Dict[str, Any] = {}
    for i, lt in enumerate(uniq):
        colors[lt] = cmap(i % 10)
    return colors
...
def plot_syllable_scatter(
    ...
    *,
    ...
    filename: str,
    title: str,
    point_colors: Optional[Sequence[Any]] = None,
    label_top_k: int = 12,
    ...
    if point_colors is None:
        ...
        # align colors to filtered rows
        colors = list(point_colors)
        if len(colors) == len(df_cmp):
            colors = list(np.asarray(colors)[d.index])
        plt.scatter(x, y, s=sizes, alpha=0.75, c=colors)
    ...
    for _, r in top.iterrows():
        if direction == "outgoing":
            tx = float(r["outgoing_entropy_nonstutter_bits"])
            ty = float(r["outgoing_entropy_stutter_bits"])
        else:
            tx = float(r["incoming_entropy_nonstutter_bits"])
            ty = float(r["incoming_entropy_stutter_bits"])
        plt.text(tx, ty, str(r["syllable"]), fontsize=8)

    if legend_handles and legend_labels:
        plt.legend(legend_handles, legend_labels, frameon=False, fontsize=8, loc="best")
...
def plot_delta_bars(df_cmp: pd.DataFrame, *, out_dir: Path, prefix: str, min_stutter_tokens: int = 1) -> List[Path]:
    saved: List[Path] = []
    if df_cmp is None or df_cmp.empty:
        return saved
    d = df_cmp.copy()
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--birds", nargs="*", default=None,
                    help="Optional explicit bird IDs. If omitted, birds are discovered by scanning root-dir for *_decoded_database.json.")
    ...
    ap.add_argument("--hit-type-sheet", type=str, default="animal_hit_type_summary",
                    help="Sheet containing lesion hit type per bird (for color-coding).")
    ap.add_argument("--hit-type-col", type=str, default="Lesion hit type",
                    help="Column name containing lesion type / hit type.")
    ...
    ap.add_argument("--aggregate-only", action="store_true",
                    help="Skip per-bird plots and only produce aggregate plots/CSV.")
    return ap
...
def main() -> None:
    ...
    # Lesion type lookup (color coding)
    ...
    # Discover birds + json paths
    bird_paths = _find_birds_in_root(root)
    if args.birds:
        bird_list = [str(b) for b in args.birds]
    else:
        bird_list = sorted(bird_paths.keys(), key=str)
    ...
    for bird in bird_list:
        ...
        # Treatment date required for scope pre/post
        ...
        if treatment_date is None:
            # If scope=all, still use mid-point as harmless fallback
            treatment_date = pd.Timestamp("1970-01-01")

        # Thresholds from stats
        ...
        if not thresholds:
            skipped.append((bird, "no thresholds found in stats table for this bird (check Animal ID, Group, column names)"))
            continue
        ...
        if args.filter_top_variance_pct is not None:
            ...
            if not restrict:
                skipped.append((bird, "variance filter kept 0 syllables (adjust top-pct/min-phrases/variance-col)"))
                continue
        ...
        # If we used fallback treatment date, set to mid-point for scope=all
        ...
        if cmp_df.empty:
            skipped.append((bird, "comparison dataframe empty (no events passed filters)"))
            continue
        ...
        if not args.no_plots and (not args.aggregate_only):
            title_out = f"{bird} ({lesion_type}) outgoing entropy\nx=non-stutter, y=stutter"
            title_in = f"{bird} ({lesion_type}) incoming entropy\nx=non-stutter, y=stutter"
            ...
            plot_syllable_scatter(
                ...
                title=title_out,
                point_colors=[_lesion_color_map([lesion_type])[lesion_type]] * len(cmp_df),
            ...
            if args.include_delta_bars:
                plot_delta_bars(cmp_df, out_dir=out_dir, prefix=prefix, min_stutter_tokens=int(args.min_stutter_tokens))
    ...
    # Aggregate plots (color by lesion type)
    if not args.no_plots:
        lesion_types = all_df["lesion_type"].astype(str).tolist()
        cmap = _lesion_color_map(lesion_types)
        colors = [cmap[lt] for lt in lesion_types]

        # Legend proxies
        handles = []
        labels = []
        for lt in sorted(set(lesion_types), key=str):
            handles.append(plt.Line2D([0], [0], marker="o", linestyle="", markersize=7, color=cmap[lt]))
            labels.append(lt)
        ...
        plot_syllable_scatter(
            ...
            title="ALL BIRDS outgoing entropy\nx=non-stutter, y=stutter (color=lesion type)",
            point_colors=colors,
            label_top_k=0,  # too many points; don't label
            min_stutter_tokens=int(args.min_stutter_tokens),
        ...
        plot_syllable_scatter(
            ...
            title="ALL BIRDS incoming entropy\nx=non-stutter, y=stutter (color=lesion type)",
            point_colors=colors,
            label_top_k=0,
            min_stutter_tokens=int(args.min_stutter_tokens),
            legend_handles=handles,
    ...
    # Summary
    print("\n=== Batch syllable stutter vs non-stutter entropy ===")
    print(f"Root: {root}")
