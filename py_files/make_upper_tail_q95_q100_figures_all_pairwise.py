#!/usr/bin/env python3
"""
Make upper-tail phrase-duration prolongation metrics and paper-style figures with all pairwise lesion-group comparisons.
...
def delta_box(animal: pd.DataFrame, comp_stats: pd.DataFrame, metric: str, out_dir: Path, prefix: str):
    ...
    for h, comp in zip(heights, ["sham - lateral_only", "sham - medial_lateral", "lateral_only - medial_lateral"]):
        p = comp_stats.loc[(comp_stats["metric"].eq(metric)) & (comp_stats["comparison"].eq(comp)), "labelshuffle_p"]
        label = p_to_star(float(p.iloc[0])) if len(p) else "n.s."
