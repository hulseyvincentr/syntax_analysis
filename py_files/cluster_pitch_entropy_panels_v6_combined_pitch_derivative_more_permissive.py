#!/usr/bin/env python3
"""
cluster_pitch_entropy_panels_v6_combined_pitch_derivative_more_permissive.py
...
3) spectrograms with separate feature panels for combined pitch + pitch derivative, and Wiener entropy
...
def plot_feature_overlay_rows(
    ...
    For each row, the figure shows:
      ...
      2) a combined pitch + absolute pitch-derivative panel
      3) a separate Wiener-entropy panel
    ...
    for row_idx, seg_list in enumerate(row_segments):
        ...
        entropy_ax.legend(loc="upper right", frameon=True, fontsize=8)

        if row_idx == 0:
            spec_ax.set_title(
                f"{animal_id} — label {target_label} — {period_name}_expanded_full_runs\n"
                f"Spectrograms shown without overlays; feature panels below show combined pitch + pitch derivative, and Wiener entropy. Gaps = NaN.",
                fontsize=13
            )
