...
scripts. This v2 plotting update uses a consistent blue/red scheme: all pre-lesion
points and mean lines are blue, and all post-lesion points and mean lines are red.
It keeps the same basic processing ideas:
...
# Plot style: all pre-lesion data use the same blue, all post-lesion data use the same red.
# Points are transparent; summary lines show the mean at each repeat index.
PRE_COLOR = "#1f77b4"   # blue
POST_COLOR = "#d62728"  # red
POINT_ALPHA = 0.20
POINT_SIZE = 14
PHRASE_POINT_SIZE = 22
MEAN_LINE_WIDTH = 2.6
...
def _pre_post_for_group(group: str) -> str:
    """Map either pre/post or early/late period labels to pre/post."""
    g = str(group)
    if g in {"pre", "post"}:
        return g
    return PRE_POST_MAP.get(g, g)


def _style_for_group(group: str) -> Dict[str, object]:
    """Return plotting style so every pre group is blue and every post group is red."""
    pp = _pre_post_for_group(group)
    if pp == "post":
        return {"color": POST_COLOR, "marker": "o"}
    return {"color": PRE_COLOR, "marker": "o"}


def _mean_by_repeat(sub: pd.DataFrame, feature: str, min_n: int) -> pd.DataFrame:
    """Summarize one feature by repeat index using the mean.

    The plotting functions use the mean line requested for the figure. The raw
    transparent points are still plotted underneath, so the spread of individual
    segments remains visible without adding separate IQR colors/fills.
    """
    rows = []
...
def _plot_feature_vs_repeat(ax, df: pd.DataFrame, feature: str, group_col: str, group_order: List[str],
    ...
    for group in group_order:
        ...
        if sub.empty or feature not in sub.columns:
            continue
        style = _style_for_group(str(group))
        color = style["color"]
        marker = style["marker"]
        ...
        if np.any(valid):
            ax.scatter(
                ...
                marker=marker,
                edgecolors="none",
                label=f"{group} segments",
            )

        b = _mean_by_repeat(sub, feature, min_n=min_bin_n)
        if not b.empty:
            xx = b["repeat_index_in_phrase"].to_numpy(dtype=float)
            mean_vals = b["mean"].to_numpy(dtype=float)
            ax.plot(
                xx, mean_vals,
                marker=marker,
...
def main():
    ...
    parser.add_argument("--plot-group-col", default="pre_post", choices=["pre_post", "period"],
                        help="How to group plotted points/mean lines. Default: pre_post.")
    parser.add_argument("--min-repeat-bin-n", type=int, default=3,
                        help="Minimum number of segments required to draw a mean line point at a repeat index. Default 3.")
