...
``lesion_surgery_date``. When ``lesion_group_detailed`` is present, it is
propagated to all output tables for complete-versus-partial plotting. Each decoded database must contain a top-level
...
def period_row(
    *,
    ...
    return {
        ...
        "lesion_group_detailed": lesion_group_detailed,
...
def main() -> None:
    ...
    with long_path.open("w", newline="", encoding="utf-8") as long_handle:
        ...
        for database_path in database_paths:
            ...
            lesion_group_detailed = str(
                bird_meta.get("lesion_group_detailed", lesion_group)
            )
