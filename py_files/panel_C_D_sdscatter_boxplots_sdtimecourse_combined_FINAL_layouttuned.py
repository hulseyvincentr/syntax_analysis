...
def plot_panel_d_sd_timecourse_overlay(
    ...
    *,
    ...
    figsize: tuple[float, float] = (10.2, 4.6),
    dpi: int = 300,
    show: bool = False,
) -> Path:
    ...
    ax.legend(handles=handles, frameon=False, fontsize=legend_fontsize, loc="upper left", bbox_to_anchor=(0.015, 0.985), borderaxespad=0.0)

    fig.tight_layout(rect=[0.02, 0.02, 0.995, 0.995])
    fig.savefig(out_path, dpi=dpi, bbox_inches="tight")
    if show:
        plt.show()
    else:
...
def make_panel_c_and_d_variance_plots(
    *,
    ...
    if daily_csv is not None:
        ...
        overlay_size = (min(10.2, max(8.8, timecourse_fig_width * 0.74)), max(4.4, timecourse_fig_height * 0.52))
