...
# Final manuscript plotting defaults:
#   sham saline injection = #53B697
#   lateral lesion only = #C0A8E2
#   medial+lateral lesion = #9C7CC8
#   box fill alpha = 1.0
# The numerical analysis is unchanged from the v8 script.
...
DEFAULT_LESION_COLORS = {
    COMBINED_ML_LABEL: "#9C7CC8",
    LATERAL_LABEL: "#C0A8E2",
    SHAM_LABEL: "#53B697",
    "unknown": "#BDBDBD",
}
    ...
    *,
    ...
    for arr, pos, color in zip(arrays, positions, colors):
        ...
        ax.boxplot(
            ...
            boxprops=dict(facecolor=color, edgecolor="black", linewidth=1.1, alpha=1.0),
    ...
    *,
    ...
    for level_label, df in [("cluster", cluster_long), ("bird", bird_level)]:
        ...
        for (set_name, method), sub in df.groupby(["set_name", "bc_method"], sort=False):
            ...
            if level_label == "bird":
                title_base = f"Bird level: {set_label}\n{method_label} | N={n_birds} birds"
            else:
                title_base = f"Cluster level: {set_label}\n{method_label} | N={n_obs} clusters, {n_birds} birds"
