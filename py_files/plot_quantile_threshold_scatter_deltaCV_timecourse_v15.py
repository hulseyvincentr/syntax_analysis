...
1) scatterplot of pre_cv vs post_cv, each point = one syllable (legend bottom-right with framed swatch-style color key and no title; scatter x/y labels, ticks, and legend at fontsize 16)
2) bird-level delta-CV boxplot by lesion group, using the actual within-bird quantile
...
def plot_scatter_for_quantile(
    ...
    # Determine the scatter limits from the points actually displayed.
    #
    # If --hide-nonselected is used, only selected syllables determine the
    # limits. Otherwise, all qualifying syllables determine the limits.
    if hide_nonselected:
    ...
    # Add a proportional margin on every side so points at the extrema are
    # fully visible rather than clipped by the plot boundaries.
    x_range = x_data_max - x_data_min
    y_range = y_data_max - y_data_min

    # Handle degenerate ranges safely.
    if x_range <= 0:
...
def plot_delta_cv_boxplot(bird_df: pd.DataFrame, stat_df: pd.DataFrame, q: float, out_dir: Path, dpi: int, show: bool):
    ...
    ax.set_ylabel(f"Bird-level {qname} of ΔCV\n(Post − Late Pre)", fontsize=16)
