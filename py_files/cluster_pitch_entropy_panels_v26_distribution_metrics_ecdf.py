...
def plot_feature_overlay_rows(
    ...
    for row_idx, seg_list in enumerate(row_segments):
        ...
        for rec in segment_records:
            ...
            all_segment_rows.append({
                ...
                "median_pitch_derivative_khz_per_s": np.nanmedian(pitch_deriv_khz_per_s[sl]) if np.any(np.isfinite(pitch_deriv_khz_per_s[sl])) else np.nan,
                "q75_pitch_derivative_khz_per_s": np.nanpercentile(pitch_deriv_khz_per_s[sl], 75) if np.any(np.isfinite(pitch_deriv_khz_per_s[sl])) else np.nan,
                "q90_pitch_derivative_khz_per_s": np.nanpercentile(pitch_deriv_khz_per_s[sl], 90) if np.any(np.isfinite(pitch_deriv_khz_per_s[sl])) else np.nan,
                "q95_pitch_derivative_khz_per_s": np.nanpercentile(pitch_deriv_khz_per_s[sl], 95) if np.any(np.isfinite(pitch_deriv_khz_per_s[sl])) else np.nan,
                "mean_wiener_entropy": np.nanmean(wiener[sl]) if np.any(np.isfinite(wiener[sl])) else np.nan,
                "median_wiener_entropy": np.nanmedian(wiener[sl]) if np.any(np.isfinite(wiener[sl])) else np.nan,
                "q75_wiener_entropy": np.nanpercentile(wiener[sl], 75) if np.any(np.isfinite(wiener[sl])) else np.nan,
                "q90_wiener_entropy": np.nanpercentile(wiener[sl], 90) if np.any(np.isfinite(wiener[sl])) else np.nan,
                "q95_wiener_entropy": np.nanpercentile(wiener[sl], 95) if np.any(np.isfinite(wiener[sl])) else np.nan,
    ...
    if pitch_segmentation_enabled and len(all_segment_qc_entries) > 0:
        ...
        for seg_idx, (ent, keep) in enumerate(zip(all_segment_qc_entries, inlier_mask)):
            ent["pitch_contour_inlier"] = bool(keep)
            if seg_idx < len(all_segment_rows):
                all_segment_rows[seg_idx]["pitch_contour_inlier"] = bool(keep)
            if not keep:
                continue
        ...
        for seg_idx in range(len(all_segment_rows)):
            all_segment_rows[seg_idx]["pitch_contour_inlier"] = True

    # Rewrite segment summary after pitch-contour inlier labels are assigned.
    seg_df = pd.DataFrame(all_segment_rows)
    seg_df.to_csv(out_segment_csv, index=False)
    print(f"[SAVED] {out_segment_csv} (with pitch-contour inlier labels)")
    ...
    return {
        ...
        "segment_summary_df": pd.DataFrame(all_segment_rows),
    }
...
def compute_distribution_stats(values_by_period, value_name):
    ...
    for comparison, group1_name, group2_name, x, y in comparison_specs:
        row = {
            ...
            "q75_group1": float(np.nanpercentile(x, 75)) if len(x) else np.nan,
            "q75_group2": float(np.nanpercentile(y, 75)) if len(y) else np.nan,
            "delta_q75_group2_minus_group1": (float(np.nanpercentile(y, 75) - np.nanpercentile(x, 75)) if len(x) and len(y) else np.nan),
            "q90_group1": float(np.nanpercentile(x, 90)) if len(x) else np.nan,
            "q90_group2": float(np.nanpercentile(y, 90)) if len(y) else np.nan,
            "delta_q90_group2_minus_group1": (float(np.nanpercentile(y, 90) - np.nanpercentile(x, 90)) if len(x) and len(y) else np.nan),
            "q95_group1": float(np.nanpercentile(x, 95)) if len(x) else np.nan,
            "q95_group2": float(np.nanpercentile(y, 95)) if len(y) else np.nan,
            "delta_q95_group2_minus_group1": (float(np.nanpercentile(y, 95) - np.nanpercentile(x, 95)) if len(x) and len(y) else np.nan),
        ...
        if scipy_available and len(x) > 0 and len(y) > 0:
            ...
            row["wasserstein_distance"] = float(stats.wasserstein_distance(x, y))
...
def plot_pre_post_ecdf(
        ...
        transform=None,
        transform_label=""):
    """Plot combined pre-lesion vs combined post-lesion ECDFs."""
    pre_vals = np.concatenate([
        _clean_values(values_by_period.get("early_pre", [])),
        _clean_values(values_by_period.get("late_pre", []))
    ])
    ...
    if transform is not None:
        pre_vals = transform(pre_vals)
        post_vals = transform(post_vals)
        pre_vals = _clean_values(pre_vals)
        post_vals = _clean_values(post_vals)

    tmp = {
        "early_pre": pre_vals,
        "late_pre": np.array([], dtype=float),
        "early_post": post_vals,
        "late_post": np.array([], dtype=float),
    }
    xlo, xhi = _get_percentile_axis_range(tmp, xmin_percentile, xmax_percentile)

    pre_x, pre_y = _ecdf_xy(pre_vals)
    post_x, post_y = _ecdf_xy(post_vals)

    fig, ax = plt.subplots(figsize=(9.5, 6.0))
    if pre_x.size > 0:
        mask = (pre_x >= xlo) & (pre_x <= xhi)
        ax.step(pre_x[mask], pre_y[mask], where="post", color="#4C78A8", lw=2.2, label=f"Pre-lesion combined (N={pre_x.size:,})")
    if post_x.size > 0:
        mask = (post_x >= xlo) & (post_x <= xhi)
        ax.step(post_x[mask], post_y[mask], where="post", color="#E45756", lw=2.2, label=f"Post-lesion combined (N={post_x.size:,})")

    subtitle = _combined_pre_post_label(stats_rows)
    title = f"{animal_id} — label {target_label} — ECDF: {value_name}"
    if transform_label:
        title += f" ({transform_label})"
    if subtitle:
        title += f"\n{subtitle}"
    title += f"\nShowing {xmin_percentile:g}–{xmax_percentile:g}th percentile x-range"
    ax.set_title(title)
...
def segment_summary_values_by_period(segment_summary_by_period, column, inlier_only=True):
    values = {p: np.array([], dtype=float) for p in PERIOD_ORDER}
    for period in PERIOD_ORDER:
        df = segment_summary_by_period.get(period)
        if df is None or len(df) == 0 or column not in df.columns:
            continue
        use = df.copy()
        if inlier_only and "pitch_contour_inlier" in use.columns:
            use = use[use["pitch_contour_inlier"].astype(bool)]
        values[period] = _clean_values(use[column].to_numpy(dtype=float))
    return values


def write_combined_segment_summary(segment_summary_by_period, out_csv):
    frames = []
    for period in PERIOD_ORDER:
        df = segment_summary_by_period.get(period)
...
def main():
    ...
    segment_summary_by_period = {p: pd.DataFrame() for p in PERIOD_ORDER}
    ...
    for feature_period in feature_periods_to_plot:
        ...
        if feature_result is not None:
            ...
            segment_summary_by_period[feature_period] = feature_result.get("segment_summary_df", pd.DataFrame())

    out_segment_summary_all_csv = os.path.join(
        args.out_dir,
        f"{args.animal_id}_label{target_label}_per_segment_feature_summary_all_periods.csv"
    )
    write_combined_segment_summary(segment_summary_by_period, out_segment_summary_all_csv)
    ...
    # Syllable-level statistics: one value per segmented syllable, not one value per time bin.
    syllable_metric_specs = [
        ("mean_wiener_entropy", "Syllable mean Wiener entropy"),
        ("median_wiener_entropy", "Syllable median Wiener entropy"),
        ("q95_wiener_entropy", "Syllable 95th percentile Wiener entropy"),
        ("mean_pitch_derivative_khz_per_s", "Syllable mean pitch derivative"),
        ("median_pitch_derivative_khz_per_s", "Syllable median pitch derivative"),
        ("q95_pitch_derivative_khz_per_s", "Syllable 95th percentile pitch derivative"),
        ("segment_duration_s", "Syllable duration"),
    ]
    syllable_stats_rows = []
    syllable_values_by_metric = {}
    for col, label in syllable_metric_specs:
        vals = segment_summary_values_by_period(segment_summary_by_period, col, inlier_only=True)
        syllable_values_by_metric[col] = vals
        syllable_stats_rows.extend(compute_distribution_stats(vals, label))

    out_syllable_stats_csv = os.path.join(
        args.out_dir,
        f"{args.animal_id}_label{target_label}_syllable_level_distribution_stats.csv"
    )
    write_distribution_stats_csv(syllable_stats_rows, out_syllable_stats_csv)
    ...
    # ECDFs are useful for skewed distributions because they avoid histogram binning artifacts.
    out_wiener_ecdf_png = os.path.join(
        args.out_dir,
        f"{args.animal_id}_label{target_label}_segmented_bin_wiener_entropy_ecdf_pre_post.png"
    )
    plot_pre_post_ecdf(
        histogram_wiener_by_period,
        out_png=out_wiener_ecdf_png,
        animal_id=args.animal_id,
        target_label=target_label,
        value_name="segmented-bin Wiener entropy",
        x_label="Wiener entropy",
        xmin_percentile=args.hist_xmin_percentile,
        xmax_percentile=args.hist_xmax_percentile,
        stats_rows=wiener_stats_rows,
    ...
    eps = 1e-12
    out_wiener_log_ecdf_png = os.path.join(
        args.out_dir,
        f"{args.animal_id}_label{target_label}_segmented_bin_log_wiener_entropy_ecdf_pre_post.png"
    )
    plot_pre_post_ecdf(
        histogram_wiener_by_period,
        out_png=out_wiener_log_ecdf_png,
        animal_id=args.animal_id,
        target_label=target_label,
        value_name="segmented-bin Wiener entropy",
        x_label="log10(Wiener entropy + 1e-12)",
        xmin_percentile=args.hist_xmin_percentile,
        xmax_percentile=args.hist_xmax_percentile,
        stats_rows=wiener_stats_rows,
        transform=lambda a: np.log10(np.asarray(a, dtype=float) + eps),
        transform_label="log10 + epsilon",
    )

    out_pitch_deriv_ecdf_png = os.path.join(
        args.out_dir,
        f"{args.animal_id}_label{target_label}_segmented_bin_pitch_derivative_ecdf_pre_post.png"
    )
    plot_pre_post_ecdf(
        histogram_pitch_deriv_by_period,
        out_png=out_pitch_deriv_ecdf_png,
        animal_id=args.animal_id,
        target_label=target_label,
        value_name="segmented-bin pitch derivative",
        x_label="Pitch derivative (kHz/s)",
        xmin_percentile=args.hist_xmin_percentile,
        xmax_percentile=args.hist_xmax_percentile,
        stats_rows=pitch_deriv_stats_rows,
    ...
    # Syllable-level ECDFs for especially interpretable summary values.
    syllable_ecdf_specs = [
        ("mean_wiener_entropy", "syllable mean Wiener entropy", "Mean Wiener entropy"),
        ("q95_wiener_entropy", "syllable 95th percentile Wiener entropy", "95th percentile Wiener entropy"),
        ("mean_pitch_derivative_khz_per_s", "syllable mean pitch derivative", "Mean pitch derivative (kHz/s)"),
        ("q95_pitch_derivative_khz_per_s", "syllable 95th percentile pitch derivative", "95th percentile pitch derivative (kHz/s)"),
    ]
    for col, value_label, x_lab in syllable_ecdf_specs:
        vals = syllable_values_by_metric.get(col, {p: np.array([], dtype=float) for p in PERIOD_ORDER})
        stats_for_col = compute_distribution_stats(vals, value_label)
        out_ecdf = os.path.join(
            args.out_dir,
            f"{args.animal_id}_label{target_label}_{col}_syllable_level_ecdf_pre_post.png"
        )
        plot_pre_post_ecdf(
            vals,
            out_png=out_ecdf,
