...
# -*- coding: utf-8 -*-
"""
figure3_panelE_longevity_normalized_sd_cv_v2.py
...
Updates in v2
-------------
1. Log-scaled timecourse plots use plain numeric tick labels (for example 1
   instead of 10^0).
2. Each timecourse plot is also saved in a summary-only version that includes
   just the equal-bird rolling median, IQR, lesion day, and reference line.
...
def run_metric_workflow(
    ...
    *,
    ...
    plot_specs = [
        ("delta", f"Δ {raw_label}", 0.0, "linear"),
        ("percent_change", f"{raw_label} change (%)", 0.0, "symlog"),
        ("fold_change", f"{raw_label} / late-pre baseline", 1.0, "log"),
    ]
