#!/usr/bin/env python3
"""
run_cluster_pitch_entropy_high_phrase_variance_batch_v28.py

Batch wrapper for running the single-label pitch / entropy / segmentation pipeline
on the highest phrase-duration-variance syllables for each bird.
...
Default behavior:
    ...
    - Rank syllables within each bird by Post_vs_Pre_Delta_Variance_ms2, descending
    - Select the top 30% of syllables per bird
    - Run the single-label driver once per selected bird/syllable label

The wrapper can run one bird or all birds in the duration-stat file.
Use --animal-id USA5288 for one bird, or omit --animal-id for all birds.
...
def main():
    parser = argparse.ArgumentParser(
        description="Run pitch/entropy/segmentation analysis on the highest phrase-duration-variance syllables per bird.",
    ...
    parser.add_argument("--metadata-excel-path", required=True,
                        help="Metadata Excel path to pass to the single-label driver.")
