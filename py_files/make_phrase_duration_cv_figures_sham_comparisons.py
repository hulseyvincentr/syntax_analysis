...
Recommended example
-------------------
python make_phrase_duration_cv_figures.py \
...
def make_stats(animal: pd.DataFrame, args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Ordering is lesion - sham, so alternative="greater" tests lesion ΔCV > sham ΔCV.
    pairwise = [
        ("lateral_only", "sham"),
        ("medial_lateral", "sham"),
    ]
...
def plot_delta_cv(animal: pd.DataFrame, comp_stats: pd.DataFrame, fig_dir: Path, correction: str) -> None:
    ...
    if all_vals:
        ...
        # Planned lesion-vs-sham comparisons only.
...
def main() -> None:
    ...
    ap.add_argument("--between-alternative", choices=["greater", "less", "two-sided"], default="greater",
                    help="Between-group label-shuffle alternative for planned lesion-vs-sham ΔCV comparisons. With default group ordering, greater tests lesion ΔCV > sham ΔCV.")
    ...
    print("\n[CHECK] Pairwise ΔCV comparisons:")
