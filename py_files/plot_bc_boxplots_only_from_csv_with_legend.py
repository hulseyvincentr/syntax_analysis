...
def save_color_legend_png(
    ...
    *,
    ...
    handles = [
        Line2D([0], [0], color=GROUP_COLORS["sham saline injection"], lw=4, label="sham saline injection"),
        Line2D([0], [0], color=GROUP_COLORS["Area X visible (single hit)"], lw=4, label="Area X visible (single hit)"),
        Line2D([0], [0], color=GROUP_COLORS["Medial/Lateral visible + large lesion"], lw=4, label="Medial/Lateral visible + large lesion"),
    ]
...
def make_boxplots_only(
    ...
    *,
    ...
    y_label: str = "Bhattacharyya coefficient (early vs late, equal groups)",
    ...
    for ax, g in zip(axes, GROUP_ORDER):
        ...
        ax.set_title(g, fontsize=panel_title_fontsize)
        ...
        ax.set_ylim(0.0, min(1.25, ymax + 0.18))
        ...
        if HAVE_SCIPY and pre.size >= 2 and post.size >= 2:
            try:
                ...
                y = min(1.18, max(np.nanmax(pre), np.nanmax(post)) + 0.06)
                add_sig_bracket(ax, 1, 2, y, 0.03, stars)
            except Exception:
                pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--y-label", default="Bhattacharyya coefficient (early vs late, equal groups)", type=str)
