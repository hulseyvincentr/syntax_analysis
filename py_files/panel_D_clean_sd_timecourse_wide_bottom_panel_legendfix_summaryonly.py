...
def plot_clean_timecourse_panels(
    ...
    *,
    ...
    compact_for_figure_panel: bool = True,
    dpi: int = 300,
    show: bool = False,
) -> Path:
    ...
    for ax, group in zip(axes, group_order):
        ...
        panel_title_fs = 10.5 if compact_for_figure_panel else 14
        ax.set_title(pretty_group_label(group).replace("\n", " "), fontsize=panel_title_fs, pad=2)
        ax.set_xlim(x_min, x_max)
        pretty_axes(ax, tick_label_fontsize=10 if compact_for_figure_panel else 12)

    if title:
        fig.suptitle(title, fontsize=13 if compact_for_figure_panel else 16, y=0.985)

    fig.supylabel(y_label, fontsize=12 if compact_for_figure_panel else 16, x=0.055)
    axes[-1].set_xlabel("Days relative to lesion", fontsize=12 if compact_for_figure_panel else 15)
    ...
    if show_raw_traces:
        handles.append(
            Line2D([0], [0], color="black", alpha=raw_alpha, linewidth=raw_linewidth,
                   marker="o", markersize=2, label="animal daily values")
        )
    handles.extend([
    ...
    if compact_for_figure_panel:
        # Put the legend in figure space (below the main title, above the top axis)
        # so it does not overlap the sham saline panel.
        fig.legend(
            handles=handles,
            frameon=False,
        ...
        top_rect = 0.76 if title else 0.88
        fig.tight_layout(rect=[0.055, 0.025, 0.99, top_rect], h_pad=0.35)
    else:
        axes[0].legend(handles=handles, frameon=False, fontsize=legend_fontsize, loc="upper left", bbox_to_anchor=(1.005, 1.0))
...
def main() -> None:
    ...
    parser.add_argument("--timecourse-fig-width", type=float, default=17.2,
                        help="Width in inches for the time-course panels. Larger width helps the bottom panel fill the artboard.")
    parser.add_argument("--timecourse-fig-height", type=float, default=5.6,
                        help="Height in inches for the time-course panels. Lower height makes a wider/flatter bottom panel.")
    parser.add_argument("--legend-fontsize", type=float, default=9.5,
                        help="Legend font size for the time-course plots.")
    parser.add_argument("--not-compact-for-figure-panel", action="store_true",
                        help="Use the older larger time-course layout with the legend outside the plot.")
    ...
    plot_clean_timecourse_panels(
        ...
        figsize=(args.timecourse_fig_width, args.timecourse_fig_height),
        compact_for_figure_panel=not args.not_compact_for_figure_panel,
    ...
    plot_clean_timecourse_panels(
        unit_df,
        smooth_raw,
        out_dir / "panel_D_clean_sd_timecourse_raw_ms_summary_only.png",
        value_col="sd_ms",
        y_label="Standard deviation of phrase durations (ms)",
        title=f"Phrase-duration SD over time ({args.smooth_window_days}-day rolling {args.smooth_stat})",
        group_order=group_order,
    ...
    plot_clean_timecourse_panels(
        unit_df,
        smooth_delta,
        out_dir / "panel_D_clean_sd_timecourse_delta_from_late_pre_summary_only.png",
        value_col="sd_delta_from_late_pre_ms",
        y_label=f"Δ SD from Late Pre baseline (ms)",
        title=f"Baseline-normalized phrase-duration SD ({args.smooth_window_days}-day rolling {args.smooth_stat})",
        group_order=group_order,
