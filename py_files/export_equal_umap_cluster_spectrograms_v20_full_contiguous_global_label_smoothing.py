...
def _fill_noise_labels_decoder_style(labels: np.ndarray, noise_label: int = -1) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    info: Dict[str, Any] = {
        ...
        "global_short_interruption_max_bins": int(round(float(max_label_interruption_ms) / float(seconds_per_bin))) if float(seconds_per_bin) > 0 else 0,
...
def export_equal_umap_cluster_spectrograms_for_one_bird(
    *,
    ...
    if bool(smooth_short_label_interruptions):
        max_interruption_bins = int(round(float(max_label_interruption_ms) / float(seconds_per_bin)))
