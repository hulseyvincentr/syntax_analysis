...
Figure 3 palette (same color/opacity for Pre and Post within lesion type):
    sham         #1B9E77
    lateral-only #A88BD9
    pooled M+L   #7A4FB7
"""
...
# Keep Pre and Post visually identical within each lesion group.  Lesion type,
# rather than recording period, determines color.  Pre/Post are distinguished
# by x-position and labels only.
BOX_ALPHA = 0.60
POINT_ALPHA = 0.95
...
def plot_panel_c(ax: plt.Axes, by_bird: pd.DataFrame, tests: pd.DataFrame, show_points: bool = True) -> None:
    ...
    for group in GROUP_ORDER:
        ...
        draw_box(ax, pre, xpre, COLORS[group], alpha=BOX_ALPHA)
        draw_box(ax, post, xpost, COLORS[group], alpha=BOX_ALPHA)
        if show_points:
            for x0, vals in [(xpre, pre), (xpost, post)]:
                x = np.full(len(vals), x0) + rng.uniform(-0.055, 0.055, len(vals))
                ax.scatter(x, vals, s=44, color=COLORS[group], alpha=POINT_ALPHA,
...
def plot_panel_d(ax: plt.Axes, by_bird: pd.DataFrame, tests: pd.DataFrame, show_points: bool = True) -> None:
    ...
    draw_box(ax, arrays["q75_pre"], positions["q75_pre"], COLORS[POOLED_ML], alpha=BOX_ALPHA)
    draw_box(ax, arrays["q75_post"], positions["q75_post"], COLORS[POOLED_ML], alpha=BOX_ALPHA)
    draw_box(ax, arrays["below_pre"], positions["below_pre"], COLORS[POOLED_ML], alpha=BOX_ALPHA)
    draw_box(ax, arrays["below_post"], positions["below_post"], COLORS[POOLED_ML], alpha=BOX_ALPHA)
    if show_points:
