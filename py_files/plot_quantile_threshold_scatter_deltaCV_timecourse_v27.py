#!/usr/bin/env python3
"""
plot_quantile_threshold_scatter_deltaCV_timecourse_v27.py

For each within-bird delta-CV quantile threshold (e.g. Q0, Q25, Q50, Q75, Q90), generate:
1) pooled scatterplot of pre_cv vs post_cv, each point = one syllable, with partial+complete M+L combined
2) detailed scatterplot of pre_cv vs post_cv, each point = one syllable, with partial and complete M+L separated
   Scatterplots use a slightly expanded lower y-limit and 14-pt legend text to reduce legend overlap.
3) bird-level delta-CV boxplot by lesion group, using the actual within-bird quantile
...
Longitudinal smoothing rule:
    centered rolling medians are calculated separately before and after lesion day;
    rolling windows never cross relative day 0.
...
DETAILED_GROUP_ORDER = [SHAM, LATERAL, PARTIAL, COMPLETE]
...
def parse_args():
    ...
    p.add_argument(
        "--scatter-axis-buffer-frac",
        type=float,
        default=0.05,
        help=(
            "Fraction of the displayed scatter data range added as padding "
            "to each side of both axes. Default: 0.05 (5%)."
        ),
    )
    p.add_argument(
        "--timecourse-x-buffer",
    ...
    p.add_argument(
        ...
        type=float,
        default=0.08,
        help="Extra padding added below and above the data range of longitudinal plots. Default: 0.08",
    )
...
def apply_style(ax: plt.Axes) -> None:
    ...
    # Base reviewer-friendly size for non-scatter panels.
...
def plot_scatter_for_quantile(
    sel_df: pd.DataFrame,
    q: float,
    out_dir: Path,
    dpi: int,
    show: bool,
    hide_nonselected: bool = False,
    axis_buffer_frac: float = 0.05,
    grouping: str = "pooled",
):
    qname = qlabel(q)
    ...
    if grouping == "pooled":
        ...
        legend_handles = [
            Patch(facecolor=COLORS[SHAM], edgecolor="none", label="Sham"),
            Patch(facecolor=COLORS[LATERAL], edgecolor="none", label="Lateral-only"),
            Patch(facecolor=COLORS[POOLED_ML], edgecolor="none", label="Medial+lateral"),
        ]
        filename_suffix = ""
        point_label_suffix = " selected"
    elif grouping == "detailed":
        group_col = "detail_group"
        group_order = DETAILED_GROUP_ORDER
        legend_handles = [
            Patch(facecolor=COLORS[SHAM], edgecolor="none", label="Sham"),
            Patch(facecolor=COLORS[LATERAL], edgecolor="none", label="Lateral-only"),
            Patch(facecolor=COLORS[PARTIAL], edgecolor="none", label="Partial medial+lateral"),
            Patch(facecolor=COLORS[COMPLETE], edgecolor="none", label="Complete medial+lateral"),
        ]
        filename_suffix = "_separate_partial_complete_ML"
        point_label_suffix = " selected"
    else:
        raise ValueError(f"Unrecognized grouping mode: {grouping!r}")
    ...
    for group in group_order:
        sub = sel_df[(sel_df[group_col] == group) & (sel_df[flag])]
        ax.scatter(
            sub["pre_cv"], sub["post_cv"],
            s=28, facecolor=COLORS[group], edgecolor="white",
            linewidth=0.4, alpha=0.95,
            label=f"{GROUP_LABELS[group]}{point_label_suffix}", zorder=2
        )
    ...
    x_data_min = float(np.nanmin(plotted_df["pre_cv"].to_numpy(float)))
    x_data_max = float(np.nanmax(plotted_df["pre_cv"].to_numpy(float)))
    y_data_min = float(np.nanmin(plotted_df["post_cv"].to_numpy(float)))
    y_data_max = float(np.nanmax(plotted_df["post_cv"].to_numpy(float)))

    x_range = x_data_max - x_data_min
    y_range = y_data_max - y_data_min

    if x_range <= 0:
        x_range = max(abs(x_data_min), 1.0)
    if y_range <= 0:
        y_range = max(abs(y_data_min), 1.0)

    x_pad = max(axis_buffer_frac * x_range, 0.01)
    y_pad = max(axis_buffer_frac * y_range, 0.01)

    # Give the lower part of the scatterplot a little extra breathing room so
    # the lower-right legend sits farther below the identity line/data cloud.
    extra_bottom_y_pad = 0.18 * y_range

    x_min = x_data_min - x_pad
    x_max = x_data_max + x_pad
    y_min = y_data_min - y_pad - extra_bottom_y_pad
    ...
    ax.set_xlabel("Late pre phrase duration CV", fontsize=16)
    ax.set_ylabel("Post phrase duration CV", fontsize=16)
    ax.set_title(f"{qname} threshold: selected syllables highlighted", fontsize=16)
    apply_style(ax)
    ...
    outfile = out_dir / f"{qname}_scatter_preCV_vs_postCV_selected_syllables{filename_suffix}.png"
    save_figure(fig, outfile, dpi, show)


def summarize_bird_delta_cv(sel_df: pd.DataFrame, q: float, group_col: str = "group") -> pd.DataFrame:
    """
    Bird-level delta-CV summary used for the quantile boxplot.

    IMPORTANT:
    This uses the actual within-bird quantile of the syllable-level delta_cv
    distribution, not the median of syllables above the threshold.

    Example:
        q = 0.75  ->  bird contributes its actual Q75(delta_cv)
        q = 0.90  ->  bird contributes its actual Q90(delta_cv)

    This keeps the boxplots aligned with the quantile-profile analysis.
    """
    rows = []
    for (bird, group), sub in sel_df.groupby(["animal_id", group_col], sort=True):
        vals = sub["delta_cv"].to_numpy(float)
        if len(vals) == 0:
            continue
        rows.append({
            ...
            "bird_quantile_delta_cv": float(np.quantile(vals, q, method="linear")),
        })
...
def compute_boxplot_stats(bird_df: pd.DataFrame, seed: int, detailed: bool = False) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    rows = []
    if detailed:
        comparisons = [
            (COMPLETE, PARTIAL, "two-sided", "Complete vs partial M+L"),
    ...
    for g1, g2, alt, label in comparisons:
        x = bird_df.loc[bird_df["group"] == g1, "bird_quantile_delta_cv"].to_numpy(float)
        y = bird_df.loc[bird_df["group"] == g2, "bird_quantile_delta_cv"].to_numpy(float)
    ...
    if kept_rows:
        ...
        for row, adj in zip(kept_rows, padj):
...
def draw_bracket(ax, x1, x2, y, h, label):
    ...
    ax.text((x1+x2)/2, y+h, label, ha="center", va="bottom", fontsize=18)
...
def plot_delta_cv_boxplot(
    ...
    qname = qlabel(q)
    group_order = DETAILED_GROUP_ORDER if detailed else GROUP_ORDER
    box_labels = DETAILED_BOX_LABELS if detailed else BOX_LABELS
    positions = np.arange(1, len(group_order) + 1)

    values = {
        g: bird_df.loc[bird_df["group"] == g, "bird_quantile_delta_cv"].dropna().to_numpy(float)
        for g in group_order
    }

    def draw_one(show_points: bool, outfile_name: str):
        fig, ax = plt.subplots(figsize=(7.0 if detailed else 5.8, 5.3))
        rng = np.random.default_rng(123 + int(round(q * 1000)) + (10000 if detailed else 0))
        ...
        ax.set_ylabel("ΔCV (Post − Late Pre)", fontsize=16)
        title = f"{qname} threshold"
        if detailed:
            title += " — partial and complete M+L separated"
        ax.set_title(title, fontsize=16)
        apply_style(ax)

        xpos_lookup = {g: i for i, g in enumerate(group_order, start=1)}
        ...
        if not stat_df_local.empty:
            stat_df_local["_order"] = stat_df_local["comparison"].map(order_map).fillna(999)
            stat_df_local = stat_df_local.sort_values("_order")

            ymin, ymax = ax.get_ylim()
            span = ymax - ymin
            finite_arrays = [v for v in values.values() if len(v) > 0]
            data_max = max(np.max(v) for v in finite_arrays)
            base = max(data_max, 0) + 0.07 * span
            step = 0.10 * span
            h = 0.022 * span

            for i, (_, row) in enumerate(stat_df_local.iterrows()):
                draw_bracket(
                    ax,
                    min(xpos_lookup[row["group1"]], xpos_lookup[row["group2"]]),
                    max(xpos_lookup[row["group1"]], xpos_lookup[row["group2"]]),
                    base + i * step,
                    h,
                    p_to_label(float(row["p_holm"])),
                )

            top_needed = base + (len(stat_df_local) - 1) * step + 0.08 * span
            ax.set_ylim(ax.get_ylim()[0], max(ax.get_ylim()[1], top_needed))

        save_figure(fig, out_dir / outfile_name, dpi, show)

    suffix = "_separate_partial_complete_ML" if detailed else ""
    draw_one(show_points=True, outfile_name=f"{qname}_deltaCV_boxplot_by_group{suffix}.png")
    draw_one(show_points=False, outfile_name=f"{qname}_deltaCV_boxplot_by_group{suffix}_boxesonly.png")
...
def smooth_equal_bird(norm_df: pd.DataFrame, x_min: int, x_max: int, rolling_window_days: int) -> pd.DataFrame:
    """
    Calculate a centered rolling median separately on the pre- and post-lesion sides.

    Day 0 is treated as a hard boundary: no rolling window is allowed to combine
    observations from relative_day < 0 with observations from relative_day > 0.
    If day-0 data are present, they are not included in either side's rolling
    calculation and cv_norm_smooth is left as NaN on day 0.
    """
    ...
    for (group, bird), sub in norm_df.groupby(["group", "animal_id"], sort=True):
        ...
        tmp["group"] = group
        tmp["animal_id"] = bird
        tmp["cv_norm_smooth"] = np.nan

        # Hard lesion-day boundary: smooth each side independently so a centered
        # window near day -1 can never borrow post-lesion values, and a window
        # near day +1 can never borrow pre-lesion values.
        pre_mask = tmp.index < 0
        post_mask = tmp.index > 0

        tmp.loc[pre_mask, "cv_norm_smooth"] = (
            tmp.loc[pre_mask, "cv_norm"]
            .rolling(window=rolling_window_days, center=True, min_periods=1)
            .median()
        )
        tmp.loc[post_mask, "cv_norm_smooth"] = (
            tmp.loc[post_mask, "cv_norm"]
            .rolling(window=rolling_window_days, center=True, min_periods=1)
            .median()
        )
...
def plot_longitudinal(
    ...
    ax.set_xlabel("Days relative to lesion", fontsize=16)

    if normalize_mode == "subtract":
        ax.set_ylabel(
            "Baseline-normalized daily\nphrase duration CV",
            fontsize=18,
        )
    else:
        ax.set_ylabel(
            "Daily\nphrase duration CV / late-pre baseline",
            fontsize=18,
        )
...
def main():
    ...
    for q in args.quantiles:
        ...
        plot_scatter_for_quantile(
            ...
            axis_buffer_frac=args.scatter_axis_buffer_frac,
            grouping="pooled",
        )
        ...
        bird_df = summarize_bird_delta_cv(sel_df, q, group_col="group")
        bird_df["quantile_label"] = qname
        bird_df["grouping_mode"] = "pooled"
        ...
        bird_df_detailed = summarize_bird_delta_cv(sel_df, q, group_col="detail_group")
        bird_df_detailed["quantile_label"] = qname
        bird_df_detailed["grouping_mode"] = "detailed"
        all_bird_summaries.append(bird_df_detailed)

        stat_df_detailed = compute_boxplot_stats(
            bird_df_detailed,
            seed=args.seed + int(round(q * 1000)) + 5000,
            detailed=True,
        )
        stat_df_detailed["quantile_label"] = qname
        stat_df_detailed["grouping_mode"] = "detailed"
        all_box_stats.append(stat_df_detailed)

        plot_delta_cv_boxplot(
            bird_df_detailed, stat_df_detailed, q,
            out_dir, args.dpi, args.show,
            detailed=True,
        )
        ...
        selected_pairs_detailed = (
            sel_df[sel_df[flag]][["animal_id", "syllable", "detail_group"]]
            .drop_duplicates()
            .rename(columns={"detail_group": "group"})
        )

        bird_day_detailed = build_daily_bird_cv(
            long_df, selected_pairs_detailed, args.min_daily_phrases
        )

        norm_detailed = normalize_to_late_pre(
            bird_day_detailed,
            late_pre_start=args.late_pre_start,
            late_pre_end=args.late_pre_end,
            mode=args.normalize_mode,
        ...
        smoothed_detailed = smooth_equal_bird(
            norm_detailed,
            x_min=args.x_min,
            x_max=args.x_max,
            rolling_window_days=args.rolling_window_days,
        ...
        summary_detailed = summarize_group_timecourse(
            smoothed_detailed,
            min_birds_for_summary=args.min_birds_for_summary,
        )
        summary_detailed["grouping_mode"] = "detailed"
        summary_detailed.to_csv(
            out_dir / f"{qname}_timecourse_summary_separate_partial_complete_ML.csv",
            index=False,
        )
        ...
        plot_longitudinal(
            ...
            dpi=args.dpi,
            show=args.show,
            groups_to_plot=DETAILED_GROUP_ORDER,
            filename_suffix="_separate_partial_complete_ML",
            title_suffix="Partial and complete M+L shown separately",
            x_buffer=args.timecourse_x_buffer,
            y_buffer=args.timecourse_y_buffer,
        )

    pd.concat(all_bird_summaries, ignore_index=True).to_csv(
        out_dir / "bird_level_quantile_delta_cv_by_quantile_threshold.csv",
        index=False,
    )
