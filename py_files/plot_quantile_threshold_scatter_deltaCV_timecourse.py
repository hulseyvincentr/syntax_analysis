...
For each within-bird delta-CV quantile threshold (e.g. Q50, Q75, Q90), generate:
1) scatterplot of pre_cv vs post_cv, each point = one syllable
2) bird-level delta-CV boxplot by lesion group
3) equal-bird rolling-median longitudinal CV plot using only syllables at or above the threshold
...
def plot_scatter_for_quantile(sel_df: pd.DataFrame, q: float, out_dir: Path, dpi: int, show: bool):
    qname = qlabel(q)
    ...
    all_vals = np.concatenate([sel_df["pre_cv"].to_numpy(), sel_df["post_cv"].to_numpy()])
...
def plot_longitudinal(time_summary: pd.DataFrame, q: float, out_dir: Path, x_min: int, x_max: int, normalize_mode: str, dpi: int, show: bool):
    qname = qlabel(q)
    fig, ax = plt.subplots(figsize=(7.4, 4.8))

    for g in GROUP_ORDER:
        sub = time_summary[time_summary["group"] == g].sort_values("relative_day")
        if sub.empty:
            continue

        ax.plot(
            sub["relative_day"], sub["median"],
            color=COLORS[g], linewidth=2.0,
            label=GROUP_LABELS[g]
        )
        ax.fill_between(
            sub["relative_day"], sub["q25"], sub["q75"],
            color=COLORS[g], alpha=0.18, linewidth=0
        )

    ax.axvline(0, color="#D62728", linestyle="--", linewidth=1.0)
    ax.axhline(
    ...
    if normalize_mode == "subtract":
        ax.set_ylabel("Baseline-normalized daily phrase-duration CV\n(selected syllables)")
    else:
        ax.set_ylabel("Daily phrase-duration CV / late-pre baseline\n(selected syllables)")

    ax.set_title(f"{qname} threshold")
    apply_style(ax)
    ax.legend(frameon=False, loc="upper left")

    save_figure(fig, out_dir / f"{qname}_longitudinal_equal_bird_timecourse.png", dpi, show)
...
def main():
    ...
    for q in args.quantiles:
        ...
        plot_scatter_for_quantile(sel_df, q, out_dir, args.dpi, args.show)
