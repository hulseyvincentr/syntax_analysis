...
def plot_timecourse(
    ...
    *,
    ...
    plot_x_min, plot_x_max = determine_groupwise_xlim(summaries, x_min, x_max)
...
# -----------------------------------------------------------------------------
def determine_groupwise_xlim(summaries: dict[str, pd.DataFrame], fallback_x_min: int, fallback_x_max: int) -> tuple[int, int]:
    """Use the actual plotted day range across groups instead of the full requested range."""
    mins: list[int] = []
    maxs: list[int] = []
    for group, summary in summaries.items():
        if summary is None or summary.empty or "relative_day" not in summary.columns:
            continue
        days = pd.to_numeric(summary["relative_day"], errors="coerce").dropna().to_numpy(dtype=int)
        if days.size == 0:
            continue
        mins.append(int(days.min()))
        maxs.append(int(days.max()))
    if not mins or not maxs:
        return fallback_x_min, fallback_x_max
...
def plot_group_comparison_timecourse(
    ...
    *,
    ...
    ax.legend(handles=handles, frameon=False, fontsize=11.0, loc="upper left")
    fig.tight_layout()
    fig.subplots_adjust(left=0.25, bottom=0.16)
    save_figure(fig, out_base, dpi=dpi, show=show)
