#!/usr/bin/env python3
"""
cluster_pitch_entropy_panels_v5_separate_feature_panels.py
...
3) spectrograms with separate feature panels for pitch, pitch derivative, and Wiener entropy
...
def plot_feature_overlay_rows(
        ...
        pitch_band_max_khz=10.0,
        separator_bins=2):
    """Plot stitched selected rows with spectrograms and separate feature panels.

    For each row, the figure shows:
      1) spectrogram only, with no time-series overlays
      2) raw pitch trace in kHz
      3) raw absolute pitch derivative in kHz/s plus Wiener entropy on a separate y-axis
    ...
    for _ in range(n_rows):
        height_ratios.extend([4.0, 1.05, 1.05])

    fig, axes = plt.subplots(
        n_rows * 3,
        1,
        figsize=(18, max(6.0 * n_rows, 7)),
        squeeze=False,
    ...
    for row_idx, seg_list in enumerate(row_segments):
        ...
        pitch_ax = axes[3 * row_idx + 1]
        deriv_ax = axes[3 * row_idx + 2]
        ...
        if len(sep_idx) > 0:
            ...
            for si in transition_idx:
                ...
                pitch_ax.axvline(x_sep, color="0.65", lw=0.8, alpha=0.9, linestyle="--")
                deriv_ax.axvline(x_sep, color="0.65", lw=0.8, alpha=0.9, linestyle="--")
        ...
        pitch_ax.axhline(0, color="black", lw=1.0, alpha=0.75, linestyle="--")
        pitch_ax.set_ylabel("Pitch\n(kHz)")
        pitch_ax.tick_params(axis="both", labelsize=8)
        pitch_ax.set_xlim(t[0], t[-1] if len(t) > 0 else 0)
        pitch_ax.legend(loc="upper right", frameon=True, fontsize=8)
        pitch_ax.tick_params(axis="x", labelbottom=False)

        # Derivative + Wiener entropy panel.
        deriv_ax.plot(
        ...
        deriv_ax.axhline(0, color="black", lw=1.0, alpha=0.75, linestyle="--")
        deriv_ax.set_ylabel("Pitch deriv.\n(kHz/s)")
        deriv_ax.tick_params(axis="both", labelsize=8)
        deriv_ax.set_xlim(t[0], t[-1] if len(t) > 0 else 0)

        entropy_ax = deriv_ax.twinx()
        entropy_ax.plot(
            t, wiener,
            color="#B83280",
            lw=1.8,
        ...
        entropy_ax.tick_params(axis="y", labelsize=8)

        handles1, labels1 = deriv_ax.get_legend_handles_labels()
        handles2, labels2 = entropy_ax.get_legend_handles_labels()
        deriv_ax.legend(handles1 + handles2, labels1 + labels2, loc="upper right", frameon=True, fontsize=8)

        if row_idx == 0:
            spec_ax.set_title(
                f"{animal_id} — label {target_label} — {period_name}_expanded_full_runs\n"
                f"Spectrograms shown without overlays; feature panels below show pitch, pitch derivative, and Wiener entropy. Gaps = NaN.",
                fontsize=13
            )
        ...
        if row_idx < n_rows - 1:
            deriv_ax.tick_params(axis="x", labelbottom=False)
