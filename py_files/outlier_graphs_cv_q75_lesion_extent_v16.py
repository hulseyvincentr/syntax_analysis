# Version: v12 — CV + within-bird Q75 ΔCV selection; darker truncated partial-lesion colormap
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
outlier_graphs_cv_q75_lesion_extent_v12.py
...
Version 12 truncates the low end of the partial-lesion ``Purples`` colormap
(default retained range 0.28-0.90) so smaller measured lesion percentages remain
visible against a white background while lesion percentage is still encoded
continuously by color.
"""
...
def _truncate_colormap(
    ...
    *,
    ...
    For the partial-lesion points, truncating ``Purples`` removes the
    near-white low end while preserving a continuous mapping from measured
    lesion percentage to color.  The colorbar uses this same truncated map.
    """
    if not (0.0 <= minval < maxval <= 1.0):
        raise ValueError(
            f"Colormap bounds must satisfy 0 <= min < max <= 1; got {minval}, {maxval}."
        )
    base = plt.get_cmap(cmap_name)
    colors = base(np.linspace(float(minval), float(maxval), int(n)))
    return LinearSegmentedColormap.from_list(
        f"{cmap_name}_trunc_{minval:.2f}_{maxval:.2f}", colors
    )
...
def plot_cv_q_lesion_extent(
    ...
    *,
    ...
    partial_cmap_min: float = 0.28,
    partial_cmap_max: float = 0.90,
    ...
    # d was created directly from df and therefore already retains selected_QXX
    # when that column exists.  Compare it in-place rather than merging it back
    # onto d; merging would create selected_QXX_x/selected_QXX_y suffixes and
    # leave no unsuffixed selected_QXX column (the v11 KeyError).
    existing_flag = f"selected_{q_label}"
    if existing_flag in d.columns:
        existing_selection = (
            d[existing_flag]
            .astype(str)
            .str.strip()
        ...
        comparable = existing_selection.notna()
        if comparable.any():
            mismatches = int((
                existing_selection.loc[comparable].astype(bool).to_numpy()
                != d.loc[comparable, "selected_at_or_above_q"].astype(bool).to_numpy()
            ).sum())
    ...
    sc = None
    partial_cmap_plot = _truncate_colormap(
        partial_cmap, minval=partial_cmap_min, maxval=partial_cmap_max
    )
    if partial_mask.any():
        ...
        if vmin is not None and vmax is not None and vmax <= vmin:
            ...
            vmax += pad
        print(
            f"[INFO] Panel A partial-lesion colormap: {partial_cmap} "
            f"truncated to [{partial_cmap_min:.2f}, {partial_cmap_max:.2f}]"
        )
        sc = ax.scatter(
            ...
            cmap=partial_cmap_plot, vmin=vmin, vmax=vmax,
    ...
    legend_handles = [
        mlines.Line2D([], [], marker="o", linestyle="none", markersize=7,
                      markerfacecolor="none", markeredgecolor="#8C1E96",
                      markeredgewidth=1.5, label="partial lesion (see colorbar)"),
        mlines.Line2D([], [], marker="o", linestyle="none", markersize=7,
                      markerfacecolor=complete_ml_color, markeredgecolor="black",
                      markeredgewidth=0.3, label="complete medial+lateral lesion"),
        mlines.Line2D([], [], marker="o", linestyle="none", markersize=7,
                      markerfacecolor=sham_color, markeredgecolor="none",
                      label="sham saline injection"),
        mlines.Line2D([], [], color="0.35", linestyle="--", label="y=x"),
    ]
    legend = ax.legend(
    ...
    # Keep all birds in the plotting table so sham controls remain visible at 0%.
    # The correlation subset is handled separately below.
    ...
    corr_birds = per_bird.copy()
    if exclude_sham_from_correlation:
        corr_birds = corr_birds[
            corr_birds["lesion_group_for_plot"] != "sham (0%)"
        ].copy()

    rho = np.nan
    p_spearman = np.nan
    if scipy_stats is not None and len(corr_birds) >= 3:
        sp = scipy_stats.spearmanr(
            corr_birds["effective_lesion_pct"].to_numpy(dtype=float),
            corr_birds["q_delta_cv"].to_numpy(dtype=float),
        )
    ...
    # No linear regression line is drawn here. The inferential analysis is a
    # Spearman rank correlation, so a least-squares trend line could imply a
    # linear model that is not actually being tested.
    ...
    report_lines = [
        ...
        f"n birds plotted = {len(per_bird)}",
        f"n birds in correlation = {len(corr_birds)}",
    ...
    if np.isfinite(rho):
        print(
            f"[STATS] Spearman effective lesion % vs {q_label} ΔCV: "
            f"rho={rho:.4f}, p={p_spearman:.6g}, n={len(corr_birds)}"
        )
    ...
    return {
        ...
        "n_panel_b_birds_plotted": int(len(per_bird)),
        "n_panel_b_birds_correlation": int(len(corr_birds)),
        "spearman_rho": rho,
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--partial_cmap_min", type=float, default=0.28,
        help="Lower fraction of the partial-lesion colormap to retain. Default 0.28 removes the near-white end.",
    )
    p.add_argument(
        "--partial_cmap_max", type=float, default=0.90,
        help="Upper fraction of the partial-lesion colormap to retain. Default 0.90 avoids the very darkest endpoint.",
    )
...
def main() -> None:
    ...
    plot_cv_q_lesion_extent(
        ...
        partial_cmap_min=args.partial_cmap_min,
        partial_cmap_max=args.partial_cmap_max,
