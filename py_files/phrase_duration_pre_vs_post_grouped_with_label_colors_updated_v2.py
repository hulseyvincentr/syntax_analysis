...
def run_phrase_duration_pre_vs_post_grouped(
    *,
    ...
    def _make_compact_grouped_syllable_points_plot():
        ...
        fig_w = max(14, 0.55 * len(labels) + 4)
        fig, ax = plt.subplots(figsize=(fig_w, 6))
        sns.set(style="white")
        rng = np.random.default_rng(0)

        any_points = False
        for boundary in np.arange(0.5, len(labels) - 0.5, 1.0):
            ax.axvline(boundary, color="0.92", linewidth=0.8, zorder=0)
        ...
        for gname, ds in group_plot_defs:
            ...
            for i, lbl in enumerate(labels):
                ...
                xvals = x_center + rng.uniform(-0.055, 0.055, size=vals.size)
                ...
                ax.vlines(x_center, q1, q3, color=color, linewidth=2.0, alpha=0.95, zorder=3)
                ax.scatter(
                    [x_center],
                    [med],
                    s=72,
        ...
        if not any_points:
            ...
            ax.set_xticklabels([str(lbl) for lbl in labels])
            ax.set_xlabel("Syllable Label")
            ax.set_ylabel("Phrase Duration (s)")
            if y_max_ms:
            ...
            _pretty_axes(ax, 0)
            _apply_seconds_yaxis(ax)
            ax.set_title(f"{animal_id} — Compact grouped phrase durations by syllable", fontsize=TITLE_FS)

            legend_handles = [
                Line2D([0], [0], marker=marker_map[g], linestyle="None", color="black", label=g,
