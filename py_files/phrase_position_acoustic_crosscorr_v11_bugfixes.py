#!/usr/bin/env python3
"""
phrase_position_acoustic_crosscorr_v11_bugfixes.py

Analyze whether acoustic structure changes as a repeated syllable phrase progresses.

Version 11 keeps the post-lesion-only tail plots, dynamic x-axis behavior, optional stitching across _segment_N chunks, target-vs-other majority-vote smoothing, and intra-phrase lag cross-correlation. It also adds bug fixes for integer-like label normalization, robust file_map lookup with int or string keys, automatic lag-correlation stats/plots, and optional leave-one-out early-template correlations.
