...
Uses the existing TweetyBERT/HDBSCAN outputs stored in USA5288.npz:
    ...
    hdbscan_colors    : existing cluster colors
...
Outputs:
    USA5288_pre_post_umap_QC/
        ...
        USA5288_cluster_centroid_shifts.png
...
def build_hdbscan_color_map(labels, stored_colors):
    """
    Build label -> RGB mapping from hdbscan_colors.

    USA5288 has labels -1, 0..26 = 28 distinct labels and
    hdbscan_colors has shape (28, 3).

    We map colors in sorted label order so that the stored color array is
    preserved exactly as written in the NPZ.
    """
    unique_labels = np.sort(np.unique(labels))

    if stored_colors.shape[0] != len(unique_labels):
        raise ValueError(
            "Cannot safely map hdbscan_colors to hdbscan_labels.\n"
            f"Unique labels: {len(unique_labels)}\n"
            f"Stored colors: {stored_colors.shape[0]}"
        )

    return {
        int(label): tuple(stored_colors[i])
...
def make_centroid_shift_plot(
    ...
    format_axis(
        ...
        xlim,
        ylim,
        "USA5288 within-cluster pre → post centroid shifts",
    )
    ...
    out = OUTPUT_DIR / "USA5288_cluster_centroid_shifts.png"
...
def main():
    ...
    # Existing HDBSCAN colors
    color_map = build_hdbscan_color_map(
        labels,
        stored_colors,
    )
    ...
    print(
        f"  stored HDBSCAN colors: {len(stored_colors)}"
    )
    ...
    shifts = make_centroid_shift_plot(
        coords,
        labels,
        pre_mask,
        post_mask,
