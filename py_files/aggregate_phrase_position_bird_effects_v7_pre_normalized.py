...
def write_readme(out_dir: Path):
    (out_dir / "README_phrase_position_bird_effects_v7.txt").write_text(
...
def main():
    ...
    if not args.skip_pre_normalized_analysis:
        ...
        if normalized_features:
            ...
            if not normalized_distribution_stat_df.empty:
                normalized_stat_df = pd.concat([normalized_stat_df, normalized_distribution_stat_df], ignore_index=True, sort=False)
    ...
    print(f"[SAVED] {out_dir / 'README_phrase_position_bird_effects_v7.txt'}")
    print("[DONE]")
