#!/usr/bin/env python3
"""
cluster_pitch_entropy_panels_v4_pitch_trace_zero_baseline.py
...
3) spectrograms with overlaid Wiener entropy + pitch + pitch-derivative traces
4) a raw-value feature panel under each spectrogram row showing pitch and pitch derivative
5) CSV of feature time series for the stitched selected rows
        ...
        for i, (start, end) in enumerate(runs):
            ...
            if row_idx == 0:
                ax.set_title(
                    ...
                    f"Colored box = target label run; text = contiguous labels; ±{context_bins} context bins",
                    fontsize=13
                )
...
def plot_feature_overlay_rows(
        ...
        pitch_band_max_khz=10.0,
        separator_bins=2):
    """Plot stitched selected rows with: top spectrogram + normalized overlays, and a bottom raw-value feature panel.

    Top panel per row: spectrogram with faint normalized overlays for Wiener entropy,
    pitch, and pitch derivative.
    Bottom panel per row: raw pitch (kHz) on the left axis and raw absolute
    pitch derivative (kHz/s) on the right axis.
    Visible dashed zero baselines are drawn so zero can be distinguished from NaN gaps.
    """
    ...
    for _ in range(n_rows):
        height_ratios.extend([4.0, 1.35])

    fig, axes = plt.subplots(
        n_rows * 2,
        1,
        figsize=(18, max(4.8 * n_rows, 6)),
        squeeze=False,
    ...
    for row_idx, seg_list in enumerate(row_segments):
        ...
        # --- Top spectrogram with normalized overlays ---
        ax2 = ax.twinx()
        ax2.axhline(0, color="black", lw=0.9, alpha=0.6, linestyle="--", zorder=1)
        ax2.plot(t, wiener_norm, color="#CC79A7", lw=1.3, alpha=0.40, label="Wiener entropy (norm)")
        ax2.plot(t, pitch_norm, color="#E69F00", lw=1.3, alpha=0.45, label="Pitch (norm)")
        ax2.plot(t, pitch_deriv_norm, color="#009E73", lw=1.3, alpha=0.45, label="Pitch derivative (norm)")
        ...
        if len(sep_idx) > 0:
            ...
            for si in transition_idx:
                ...
                feat_ax.axvline(si * bin_s, color="0.7", lw=0.8, alpha=0.8, linestyle="--")
        ...
        # --- Bottom raw feature panel ---
        feat_ax.plot(t, pitch_khz, color="#E69F00", lw=1.2, alpha=0.85, label="Pitch (kHz)")
        feat_ax.axhline(0, color="black", lw=0.9, alpha=0.6, linestyle="--")
        feat_ax.set_ylabel("Pitch\n(kHz)")
        feat_ax.tick_params(axis="both", labelsize=8)
        feat_ax.set_xlim(t[0], t[-1] if len(t) > 0 else 0)

        feat_ax2 = feat_ax.twinx()
        feat_ax2.plot(t, pitch_deriv_khz_per_s, color="#009E73", lw=1.2, alpha=0.85, label="Pitch derivative (kHz/s)")
        feat_ax2.axhline(0, color="#009E73", lw=0.9, alpha=0.55, linestyle=":")
        feat_ax2.set_ylabel("Pitch deriv.\n(kHz/s)")
        feat_ax2.tick_params(axis="y", labelsize=8)

        if row_idx == 0:
            ax.set_title(
                f"{animal_id} — label {target_label} — {period_name}_expanded_full_runs\n"
                f"Top: spectrogram with normalized overlays. Bottom: raw pitch and raw pitch derivative. Gaps = NaN (no reliable estimate).",
                fontsize=13
            )
            handles1, labels1 = ax2.get_legend_handles_labels()
            handles2, labels2 = feat_ax.get_legend_handles_labels()
            handles3, labels3 = feat_ax2.get_legend_handles_labels()
            ax2.legend(handles1 + handles2 + handles3,
        ...
        # Only the bottom raw feature panel gets x tick labels for each row-pair.
        ax.tick_params(axis="x", labelbottom=False)
        if row_idx < n_rows - 1:
            feat_ax.tick_params(axis="x", labelbottom=False)
