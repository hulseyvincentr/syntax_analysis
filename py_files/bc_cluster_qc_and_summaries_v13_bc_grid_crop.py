...
DEFAULT_V8_SCRIPT = "export_equal_umap_cluster_spectrograms_v14_bc_grid_crop.py"
...
def run_pipeline(
    *,
    ...
    if not qual_df.empty:
        ...
        if "is_high_variance_cluster" in qual_df.columns and qual_df["is_high_variance_cluster"].any():
            hv_base = qual_df[qual_df["is_high_variance_cluster"] == True].copy()
            for prefix, label in bc_method_defs:
                hv_method_df = _method_subset_for_boxplot(hv_base, prefix)
                if hv_method_df.empty:
                    continue
                hv_method_boxplot = animal_out_dir / f"{animal_id_final}_BC_boxplot_high_variance_clusters_{prefix}_min{min_balanced_duration_s:.1f}s.png"
                hv_method_subtitle = (
                    f"{label} | high-variance phrase-duration clusters only | minimum balanced duration per group = "
                    f"{float(min_balanced_duration_s):.1f} s | N clusters = {len(hv_method_df)}"
                )
                plot_bc_boxplot_for_cluster_set(
                    bc_df=hv_method_df,
                    title=f"{animal_id_final}: BC across high-variance clusters",
                    subtitle=hv_method_subtitle,
                    out_png=hv_method_boxplot,
                    dpi=int(dpi),
                )
                print(f"[SAVE] {hv_method_boxplot}")
                stats_rows.append({
                    "set_name": "high_variance_clusters",
                    "bc_method": prefix,
                    "n_clusters": int(len(hv_method_df)),
                    "mean_bc_pre": float(np.nanmean(hv_method_df["bc_pre"])),
                    "mean_bc_post": float(np.nanmean(hv_method_df["bc_post"])),
                    "mean_bc_prepost": float(np.nanmean(hv_method_df["bc_prepost"])),
                    "median_bc_pre": float(np.nanmedian(hv_method_df["bc_pre"])),
                    "median_bc_post": float(np.nanmedian(hv_method_df["bc_post"])),
                    "median_bc_prepost": float(np.nanmedian(hv_method_df["bc_prepost"])),
                })
