...
def plot_change_metric_boxplot_vs_sham(
    ...
    *,
    ...
    # Keep the delta-plot y-label short and within the saved figure bounds.
    # The longer definition of Δ is printed under each group and in the caption.
    delta_ylabel_fontsize = max(axis_label_fontsize - 2.5, 11.5)
    ax.set_ylabel(y_label, fontsize=delta_ylabel_fontsize, labelpad=6)
    ...
    fig.subplots_adjust(left=0.17, right=0.99, bottom=0.48, top=0.88)
    fig.savefig(out_path, dpi=dpi, bbox_inches="tight", pad_inches=0.12)
    if show:
        plt.show()
    else:
