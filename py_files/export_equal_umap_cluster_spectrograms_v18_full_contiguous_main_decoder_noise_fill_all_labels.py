...
SCRIPT_VERSION = "equal_umap_cluster_spectrograms_v17_full_contiguous_main_decoder_noise_fill"
...
def plot_representative_full_run_contexts(
    *,
    ...
    for ax, (_, row) in zip(axes, chosen.iterrows()):
        ...
        # Optional legacy cap: if requested, do not let the displayed context exceed
        # the max-context limit, but always render on the same fixed x-axis.
        ...
        # Blue: full HDBSCAN run (outline + bottom bar so it remains visible).
        context_end = min(N, context_start + panel_n_bins_this_row)
        ...
        for s, e in zip(seg_starts, seg_ends):
            ...
            if e2 > s2:
                ...
                ax.vlines([red_x0, red_x1], ymin=ax.get_ylim()[0], ymax=ax.get_ylim()[1],
                          colors="tab:red", linewidth=0.45, alpha=0.75, zorder=8)

        # Optional track showing every contiguous label segment in the displayed context.
        if bool(show_all_labels_track) and labels is not None:
            y0, y1 = ax.get_ylim()
            track_h = max(2.0, 0.13 * (y1 - y0))
            track_y0 = y1 - track_h
            # subtle white backing so label colors/text remain legible
            ax.add_patch(
                plt.Rectangle(
                    (0.0, track_y0),
                    float(fixed_panel_duration_s),
                    track_h,
            ...
            label_segs = _contiguous_label_segments(np.asarray(labels), context_start, context_end)
            for a, b, lab in label_segs:
                x0 = (int(a) - context_start) * float(seconds_per_bin)
                x1 = (int(b) - context_start) * float(seconds_per_bin)
                if x1 <= 0 or x0 >= float(fixed_panel_duration_s):
                    continue
                x0 = max(0.0, x0)
                x1 = min(float(fixed_panel_duration_s), x1)
                ax.add_patch(
                    plt.Rectangle(
                        (x0, track_y0),
                        max(1e-6, x1 - x0),
                        track_h,
                        facecolor=_label_track_color(int(lab)),
                        edgecolor="none",
                        alpha=0.35,
                        zorder=10,
                    )
            ...
            ax.hlines(track_y0, 0.0, float(fixed_panel_duration_s), colors=(0, 0, 0, 0.35), linewidth=0.5, zorder=12)
            ax.text(
                float(fixed_panel_duration_s) - 0.01 * float(fixed_panel_duration_s),
                track_y0 + 0.5 * track_h,
                "all labels",
                ha="right",
                va="center",
    ...
    subtitle += f"; fixed x-axis 0–{float(fixed_panel_duration_s):.1f} s"
    fig.suptitle(
    ...
    *,
    ...
    if set(group_names) == {"early_pre", "late_pre", "early_post", "late_post"}:
        ...
        def _draw_overlap(ax, hist_a, hist_b, title_text):
            ...
            ax.set_ylim(plot_ylim)
            ax.set_aspect('equal', adjustable='box')
            ax.set_title(title_text, fontsize=12, pad=4)
    ...
    if len(groups_present) >= 2:
        ...
        for sp in ax.spines.values():
            sp.set_visible(False)
    fig.suptitle(f"{animal_id} — HDBSCAN label {cluster_id} — {analysis_label}", y=0.98)
    fig.tight_layout(rect=(0, 0, 1, 0.95))
    fig.savefig(out_png, dpi=int(dpi), bbox_inches='tight')
    plt.close(fig)
...
def export_equal_umap_cluster_spectrograms_for_one_bird(
    *,
    ...
    if bool(fill_noise_labels_from_nearest_nonnoise):
        labels, label_cleanup_info = _fill_noise_labels_decoder_style(labels_raw, noise_label=int(noise_label))
    else:
        labels = labels_raw
    ...
    for cluster_id in cluster_ids:
        ...
        row_base["label_cleanup_mode"] = "decoder_style_nearest_nonnoise" if bool(fill_noise_labels_from_nearest_nonnoise) else "none"
