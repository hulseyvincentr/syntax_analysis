...
def read_bc_table(path: str, metadata_excel: Optional[str]) -> pd.DataFrame:
    ...
    animal_col = first_present(table.columns, ["animal_id", "Animal ID", "bird_id", "bird"])
    # The Figure 3 selected-pairs CSV stores the original syllable label/token.
    # In the BC batch table, cluster_id is a local sequential index, while
    # cluster_token is the original syllable token. Prefer cluster_token so the
    # merge is not based on accidental numeric overlap with cluster_id.
    syllable_col = first_present(
        table.columns,
        [
            "cluster_token",
