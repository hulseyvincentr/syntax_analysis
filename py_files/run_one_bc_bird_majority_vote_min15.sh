...
OUT_ROOT="${DATA_ROOT}/bc_full_contiguous_MAIN_ALL_BIRDS_MIN15_MAJORITY_VOTE_200bin_density20_grid99_title_spacing"
...
python bc_cluster_qc_and_summaries_v19_full_contiguous_majority_vote_smoothing.py \
  ...
  --npz-path "$NPZ" \
  --metadata-excel-path "${DATA_ROOT}/Area_X_lesion_metadata_with_hit_types.xlsx" \
  ...
  --phrase-csv "${DATA_ROOT}/usage_balanced_phrase_duration_stats.csv" \
  --out-dir "$OUT_ROOT" \
  --animal-id "$ANIMAL_ID" \
...
if [ "$STATUS" -eq 0 ]; then
    ...
    echo "[FAILED] ${ANIMAL_ID} -- see ${LOG_DIR}/${ANIMAL_ID}.log"
fi
