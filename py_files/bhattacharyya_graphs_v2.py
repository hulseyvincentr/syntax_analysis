...
Required input (Bhattacharyya CSV) columns:
...
Always:
  ...
  • stats_report_cluster_coeff.txt
  • stats_report_cluster_dist.txt
  • stats_report_bird_coeff.txt
  • stats_report_bird_dist.txt
  • coeff_vs_dist_scatter_by_hit_type.png

Optional: High-variance vs low-variance tiering (TOP 30% variance per bird)
-------------------------------------------------------------------------
If --phrase-stats is provided (CSV or XLSX), this script will:
  1) take POST rows from the phrase-duration stats table,
  2) compute per-bird 70th percentile threshold of POST variance (ms^2),
  3) label clusters with variance >= threshold as "High variance" (top 30%),
     and the rest as "Low variance" (lower 70%),
  4) rerun the *same lesion-group comparisons* for:
       • high-variance clusters (cluster-level)
       • low-variance clusters (cluster-level)
       • (optional) bird-level medians within each tier

It will save:
  • highVar_coeff_by_lesion_group_cluster.png
  • highVar_dist_by_lesion_group_cluster.png
  • lowVar_coeff_by_lesion_group_cluster.png
  • lowVar_dist_by_lesion_group_cluster.png
  • highVar_coeff_by_lesion_group_bird.png
  • highVar_dist_by_lesion_group_bird.png
  • lowVar_coeff_by_lesion_group_bird.png
  • lowVar_dist_by_lesion_group_bird.png
  • (plus *_stats.txt files)

Planned comparisons (match your Levina–Bickel graphs):
  1) Area X visible (single hit) vs sham saline injection
  2) Combined (visible ML + not visible) vs sham saline injection
using Mann–Whitney U (SciPy) and Holm correction (2 comparisons).

Example
-------
python bhattacharyya_graphs.py \
  --csv "/path/to/bhattacharyya_pre_post_predictions_*.csv" \
  --min-pre 500 --min-post 500 \
  --alternative two-sided \
  --phrase-stats "/path/to/usage_balanced_phrase_duration_stats.csv" \
  --top-pct 70 \
  --min-phrases 20
"""
...
GROUP_COLORS = {
    ...
    GROUP_COMBINED: "#4B4B4B",  # dark gray
}
...
def _map_hit_type_to_group(hit_type: str) -> str:
    """Map detailed hit_type strings into the 3-group comparison used in plots."""
    if hit_type is None:
        return "unknown"
    ...
    # combine ML visible + not visible
...
def _write_group_stats_report(
    ...
    *,
    ...
    Returns list of Holm-adjusted p-values in planned order:
      [visible_single vs sham, combined vs sham]
    """
    lines: List[str] = []
...
def load_post_variance_table(
    ...
    *,
    ...
    """
    Load phrase-duration stats (CSV or XLSX), keep POST rows, and return:
      animal_id, cluster_label, post_variance_ms2, post_n_phrases
    """
    if phrase_path.suffix.lower() in [".xlsx", ".xls"]:
        ph = pd.read_excel(phrase_path, sheet_name=sheet)
    else:
        ph = pd.read_csv(phrase_path)

    _ensure_cols(ph, [phrase_group_col, animal_col, syllable_col, variance_col], name="Phrase stats")
    ...
    # If multiple rows per (animal, syllable) survive (rare), use median variance and max N_phrases
...
def add_high_variance_flag(post_var: pd.DataFrame, top_pct: float = 70.0, min_phrases: int = 20) -> pd.DataFrame:
    ...
    If min_phrases > 0, only syllables with post_n_phrases >= min_phrases contribute to percentile.
    """
    df = post_var.copy()
    df["_use"] = True
    if min_phrases > 0:
...
def main() -> None:
    ...
    ap.add_argument("--phrase-stats", default=None, type=str,
                    help="Optional phrase duration stats table (CSV or XLSX) to do high/low-variance tier comparisons.")
    ap.add_argument("--phrase-stats-sheet", default=None, type=str,
                    help="If --phrase-stats is XLSX, optionally provide sheet name.")
    ...
    df["cluster_label"] = pd.to_numeric(df["cluster_label"], errors="coerce").astype("Int64")
    ...
    df = df.dropna(subset=["cluster_label"]).copy()
    ...
    # -----------------
    # Optional variance-tier analysis
    # -----------------
    if args.phrase_stats:
        phrase_path = Path(args.phrase_stats)
        if not phrase_path.exists():
            raise FileNotFoundError(phrase_path)

        post_var = load_post_variance_table(phrase_path, sheet=args.phrase_stats_sheet)
        ...
        # Cluster-level tier plots
        for tier_name, tier_df in [("highVar", hi), ("lowVar", lo)]:
            _boxplot_by_group(
                df=tier_df,
                out_path=out_dir / f"{tier_name}_coeff_by_lesion_group_cluster.png",
                stats_path=out_dir / f"{tier_name}_coeff_by_lesion_group_cluster_stats.txt",
                group_col=args.group_col,
                metric_col="bhattacharyya_coeff",
                ylabel="Bhattacharyya coefficient (higher = more similar)",
                title=f"{tier_name}: BC by lesion group (cluster-level)",
                alternative=args.alternative,
                level_label=f"cluster-level ({tier_name})",
            )
            _boxplot_by_group(
                df=tier_df,
                out_path=out_dir / f"{tier_name}_dist_by_lesion_group_cluster.png",
                stats_path=out_dir / f"{tier_name}_dist_by_lesion_group_cluster_stats.txt",
                group_col=args.group_col,
                metric_col="bhattacharyya_dist",
                ylabel="Bhattacharyya distance (lower = more similar)",
                title=f"{tier_name}: DB by lesion group (cluster-level)",
                alternative=args.alternative,
                level_label=f"cluster-level ({tier_name})",
            )

        # Bird-level within-tier medians
        for tier_name, tier_df in [("highVar", hi), ("lowVar", lo)]:
            bird_tier = (
                tier_df.groupby(["animal_id", args.group_col, args.hit_type_col], as_index=False)
                       .agg(
                           bhattacharyya_coeff=("bhattacharyya_coeff", "median"),
                           bhattacharyya_dist=("bhattacharyya_dist", "median"),
                           n_clusters=("cluster_label", "nunique"),
            ...
            _boxplot_by_group(
                df=bird_tier.rename(columns={args.group_col: "group"}),
                out_path=out_dir / f"{tier_name}_coeff_by_lesion_group_bird.png",
                stats_path=out_dir / f"{tier_name}_coeff_by_lesion_group_bird_stats.txt",
                group_col="group",
                metric_col="bhattacharyya_coeff",
                ylabel="Median Bhattacharyya coefficient per bird",
                title=f"{tier_name}: BC by lesion group (bird-level medians)",
                alternative=args.alternative,
                level_label=f"bird-level ({tier_name})",
            )
            _boxplot_by_group(
                df=bird_tier.rename(columns={args.group_col: "group"}),
                out_path=out_dir / f"{tier_name}_dist_by_lesion_group_bird.png",
                stats_path=out_dir / f"{tier_name}_dist_by_lesion_group_bird_stats.txt",
                group_col="group",
                metric_col="bhattacharyya_dist",
                ylabel="Median Bhattacharyya distance per bird",
                title=f"{tier_name}: DB by lesion group (bird-level medians)",
                alternative=args.alternative,
                level_label=f"bird-level ({tier_name})",
            )
