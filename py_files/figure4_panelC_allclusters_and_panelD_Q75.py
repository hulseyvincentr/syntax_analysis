...
def plot_panel_c(ax: plt.Axes, by_bird: pd.DataFrame, tests: pd.DataFrame) -> None:
    positions = {
        SHAM: (0.0, 1.0),
        LATERAL: (3.0, 4.0),
        POOLED_ML: (6.0, 7.0),
    ...
    for group in GROUP_ORDER:
        ...
        p = float(row["wilcoxon_two_sided_p"])
...
def plot_panel_d(ax: plt.Axes, by_bird: pd.DataFrame, tests: pd.DataFrame) -> None:
    ...
    bracket_base = data_max + 0.09 * span
    bracket_height = 0.035 * span
    q75_p = float(tests.loc[tests["subset"] == ">= Q75 ΔCV syllables", "wilcoxon_two_sided_p"].iloc[0])
    below_p = float(tests.loc[tests["subset"] == "< Q75 ΔCV syllables", "wilcoxon_two_sided_p"].iloc[0])
...
def save_panel_c(by_bird: pd.DataFrame, tests: pd.DataFrame, out_path: Path, dpi: int, show: bool) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE_C)
    plot_panel_c(ax, by_bird, tests)
...
def save_panel_d(by_bird: pd.DataFrame, tests: pd.DataFrame, out_path: Path, dpi: int, show: bool) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE_D)
    plot_panel_d(ax, by_bird, tests)
    fig.tight_layout(rect=[0, 0.08, 1, 1])
    fig.savefig(out_path, dpi=dpi)
    if show:
        plt.show()
...
def save_combined(panel_c_by_bird: pd.DataFrame, panel_c_tests: pd.DataFrame,
                  panel_d_by_bird: pd.DataFrame, panel_d_tests: pd.DataFrame,
                  out_path: Path, dpi: int, show: bool) -> None:
    fig = plt.figure(figsize=FIGSIZE_COMBINED)
    gs = fig.add_gridspec(2, 1, height_ratios=[1.08, 0.92], hspace=0.35)
    axc = fig.add_subplot(gs[0, 0])
    axd = fig.add_subplot(gs[1, 0])
    plot_panel_c(axc, panel_c_by_bird, panel_c_tests)
    plot_panel_d(axd, panel_d_by_bird, panel_d_tests)
...
def main() -> None:
    ...
    save_panel_c(panel_c_by_bird, panel_c_tests,
                 out_dir / "figure4_panelC_all_qualifying_by_lesion_group.png",
                 args.dpi, args.show)
    save_panel_d(panel_d_by_bird, panel_d_tests,
                 out_dir / "figure4_panelD_pooled_ML_Q75_split.png",
                 args.dpi, args.show)
    save_combined(panel_c_by_bird, panel_c_tests, panel_d_by_bird, panel_d_tests,
