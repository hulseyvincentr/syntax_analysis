...
# -*- coding: utf-8 -*-
"""
umap_pre_post_early_late_per_cluster_bc_versionA.py


Version A:
For each cluster:
  1) fit ONE UMAP using all available points from that cluster
     (pre-early, pre-late, post-early, post-late)
  2) then select balanced subsets for:
       - pre early vs pre late
       - post early vs post late
  3) plot those balanced subsets in the shared UMAP space
  4) compute Bhattacharyya coefficients (BC) in that shared UMAP space

This means the embedding geometry is determined by all cluster points,
while the plotted and BC-computed groups are balanced afterward.

Outputs:
  out_dir/
    clusters/
      <prefix>_cluster<id>_pre_post_early_late_umap_versionA.png
    <prefix>_umap_pre_post_early_late_cluster_summary_versionA.csv
"""
...
def run_one_bird(cfg: UMAPEarlyLateConfig) -> Path:
    ...
    out_dir = Path(cfg.out_dir) if cfg.out_dir is not None else (npz_path.parent / f"umap_pre_post_early_late_{animal_id}_versionA")
    ...
    for lab in unique_labels:
        ...
        # Map original point index -> cluster-local position in UMAP
        ...
        if pre_status == "ok" or post_status == "ok":
            ...
            if pre_status == "ok":
                ...
                is_pre_late_plot = np.array([False] * len(xy_pre_early) + [True] * len(xy_pre_late), dtype=bool)
            ...
            if post_status == "ok":
                ...
                is_post_late_plot = np.array([False] * len(xy_post_early) + [True] * len(xy_post_late), dtype=bool)
            ...
            fig.tight_layout()
            out_png = clusters_dir / f"{prefix}_cluster{lab}_pre_post_early_late_umap_versionA.png"
        ...
        summary_rows.append({
            "cluster": lab,
            "fit_mode": "all_cluster_points_then_balance_subsets",

            "n_cluster_total_fit": int(idx_cluster_all.size),
        ...
        print(
            f"[UMAP Version A] cluster {lab}: "
            f"fit_total={idx_cluster_all.size} | "
    ...
    summary_csv = out_dir / f"{prefix}_umap_pre_post_early_late_cluster_summary_versionA.csv"
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="umap_pre_post_early_late_per_cluster_bc_versionA.py",
...
"""
%run /Users/mirandahulsey-vincent/Documents/allPythonCode/syntax_analysis/py_files/umap_pre_post_early_late_per_cluster_bc_versionA.py --npz-path "/Volumes/my_own_SSD/updated_AreaX_outputs/USA5443/USA5443.npz" --metadata-xlsx "/Volumes/my_own_SSD/updated_AreaX_outputs/Area_X_lesion_metadata_with_hit_types.xlsx" --out-dir "/Volumes/my_own_SSD/updated_AreaX_outputs/USA5443/umap_early_late_pre_post_clusters_versionA" --array-key "predictions" --cluster-key "hdbscan_labels" --file-key "file_indices" --min-points-per-period 200 --max-points-per-period 20000 --plot-max-points-per-period 5000 --n-neighbors 30 --min-dist 0.1 --metric "euclidean" --bc-bins 100 --seed -1
