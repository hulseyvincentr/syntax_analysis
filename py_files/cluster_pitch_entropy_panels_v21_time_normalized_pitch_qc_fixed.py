#!/usr/bin/env python3
"""
cluster_pitch_entropy_panels_v21_time_normalized_pitch_qc_fixed.py
...
def plot_time_normalized_pitch_contours(
    ...
    for row in M:
        ax.plot(phase, row, color="#2E8B57", alpha=0.07, lw=0.9)
    ...
    ax.plot(phase, mean_trace, color="crimson", lw=2.4, label="mean", zorder=3)

    med_dur = np.nanmedian(durations_s) if len(durations_s) else np.nan
    ax.set_title(
        f"{animal_id} — label {target_label} — {pretty_period}\n"
        f"Pitch contours, time-normalized — {title_suffix}\n"
        f"N syllables = {M.shape[0]:,}; median duration = {med_dur * 1000.0:.1f} ms"
    )
...
def main():
    ...
    # Statistical tests for segmented-bin distributions.
