...
Creates Figure-3-style versions of:
  ...
  - delta-by-group boxplots for both metrics
...
def get_group_p(group_stats: pd.DataFrame, metric: str, group: str) -> float:
...
def get_comparison_p(comparison_stats: pd.DataFrame, metric: str, a: str, b: str) -> float:
...
def paired_boxplot(bird: pd.DataFrame, group_stats: pd.DataFrame, base: str, out_dir: Path) -> None:
    pre_col = f"pre_{base}"
    post_col = f"post_{base}"
    ...
    if all_vals:
        ...
        for i, group in enumerate(groups):
            ...
            if len(vals):
                add_bracket(ax, pre_positions[i], post_positions[i], float(np.nanmax(vals)) + 0.06 * yr, p_to_label(get_group_p(group_stats, metric, group)))
    fig.tight_layout()
...
def delta_by_group(bird: pd.DataFrame, comparison_stats: pd.DataFrame, base: str, out_dir: Path) -> None:
    metric = f"delta_{base}"
    groups = [g for g in PAPER_GROUP_ORDER if g in set(bird["group"])]
    ...
    if all_vals:
        ...
        if "lateral_only" in groups and "medial_lateral" in groups:
            x1, x2 = groups.index("lateral_only") + 1, groups.index("medial_lateral") + 1
            add_bracket(ax, x1, x2, ymax + 0.07 * yr, p_to_label(get_comparison_p(comparison_stats, metric, "medial_lateral", "lateral_only")))
    fig.tight_layout()
...
def main() -> None:
    ...
    for base in FOCUS_BASES:
        ...
        paired_boxplot(bird, group_stats, base, args.out_dir)
        delta_by_group(bird, comparison_stats, base, args.out_dir)
