...
def plot_panel_c_scatter(
    ...
    *,
    ...
    x_min, x_max = _tight_positive_limits(finite[x_col].to_numpy(dtype=float), padding_frac=axis_padding_frac)
    y_min, y_max = _tight_positive_limits(finite[y_col].to_numpy(dtype=float), padding_frac=axis_padding_frac)
...
def plot_pre_post_distribution_boxplot(
    ...
    *,
    ...
    are Post. Thin paired lines connect the same unit where possible.
    ...
    ax.legend(handles=handles, frameon=False, fontsize=13, loc="best")
    ...
    if paired_stats_df is not None and not paired_stats_df.empty and base_metric is not None:
        for i, group in enumerate(display_order):
            ...
            n_units = row.iloc[0].get("n_units", np.nan)
            p_txt = _format_p(float(p_value)) if pd.notna(p_value) else "NA"
            t_txt = f"{float(t_stat):.2g}" if pd.notna(t_stat) else "NA"
            try:
                n_txt = f"n={int(n_units)}"
            except Exception:
                n_txt = "n=NA"
            stars = _significance_stars(p_value)
            g_for_median = work[work[group_col].astype(str) == str(group)]
            pre_median = np.nanmedian(g_for_median[pre_col].to_numpy(dtype=float)) if not g_for_median.empty else np.nan
            post_median = np.nanmedian(g_for_median[post_col].to_numpy(dtype=float)) if not g_for_median.empty else np.nan
            ax.text(
                i * 1.55,
                -0.18,
                f"median pre={_format_plot_value(pre_median)}\nmedian post={_format_plot_value(post_median)}\npaired t={t_txt}\np={p_txt}\n{n_txt}",
    ...
    fig.subplots_adjust(bottom=0.44, top=0.88)
    fig.tight_layout()
    fig.savefig(out_path, dpi=dpi, bbox_inches="tight")
    if show:
        plt.show()
...
def plot_change_metric_boxplot_vs_sham(
    ...
    *,
    ...
    fig.subplots_adjust(bottom=0.50, top=0.88)
    fig.tight_layout()
    fig.savefig(out_path, dpi=dpi, bbox_inches="tight")
    if show:
        plt.show()
...
def make_panel_c_boxplots(
    ...
    *,
    ...
    outputs["panel_C_variance_pre_post_boxplot"] = plot_pre_post_distribution_boxplot(
        ...
        title="Late Pre vs Post variance by lesion group",
        y_scale="log",
    ...
    outputs["panel_C_sd_pre_post_boxplot"] = plot_pre_post_distribution_boxplot(
        ...
        title="Late Pre vs Post standard deviation by lesion group",
        y_scale="log",
        paired_stats_df=stats_paired_df,
        base_metric="sd",
        pool_ml_groups=True,
    ...
    outputs["panel_C_sd_delta_vs_sham_boxplot"] = plot_change_metric_boxplot_vs_sham(
        ...
        color_config=color_config,
        metric_col="sd_delta",
        y_label="Δ Standard Deviation (Post − Late Pre, ms)",
...
def plot_panel_d_sd_timecourse(
    ...
    *,
    color_config: Dict[str, Any],
    value_col: str = "sd_ms",
    title: Optional[str] = "Phrase-duration SD over time (7-day rolling median)",
    y_label: str = "Standard deviation of phrase durations (ms)",
    group_order: Optional[Sequence[str]] = None,
    x_min: int = -30,
    x_max: int = 30,
    ...
    fig.supylabel(y_label, fontsize=axis_label_fontsize, x=0.03)
    ...
    fig.tight_layout(rect=[0.05, 0.03, 0.985, 0.93], h_pad=0.65)
    fig.savefig(out_path, dpi=dpi, bbox_inches="tight")
    if show:
        plt.show()
    else:
...
def make_panel_c_and_d_variance_plots(
    *,
    ...
    timecourse_legend_fontsize: float = 11.5,
    boxplot_tick_label_fontsize: float = 14,
    boxplot_axis_label_fontsize: float = 18,
    boxplot_title_fontsize: float = 18,
    show: bool = False,
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--legend-fontsize", type=float, default=11.5, help="Legend font size for the SD timecourse panels.")
    p.add_argument("--boxplot-tick-fontsize", type=float, default=14, help="Tick-label font size for Panel C boxplots.")
    p.add_argument("--boxplot-axis-label-fontsize", type=float, default=18, help="Axis-label font size for Panel C boxplots.")
    p.add_argument("--boxplot-title-fontsize", type=float, default=18, help="Title font size for Panel C boxplots.")
