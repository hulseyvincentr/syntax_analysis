...
# -*- coding: utf-8 -*-
"""
phrase_duration_early_late_post_difference_in_differences_v4_CV_ML_vs_lateral.py
...
- adds dedicated CV plots comparing pooled M+L directly with lateral-only;
...
def plot_direct_ml_vs_lateral_did(
    animal: pd.DataFrame,
    within_stats: pd.DataFrame,
    ...
    *,
    ...
    """
    Dedicated direct comparison of lateral-only and pooled M+L DID values.

    This figure is intended to make the anatomical comparison visually explicit.
    It does not alter the statistical test: the annotation comes from the
    precomputed one-sided label-shuffle contrast Pooled M+L > lateral-only.
    """
    if metric_name not in {"cv_raw", "cv_log2"}:
        raise ValueError(
            "plot_direct_ml_vs_lateral_did is intended for cv_raw or cv_log2."
        )

    spec = METRIC_SPECS[metric_name]
    comparison_groups = [LATERAL, POOLED_ML]

    metric_within = within_stats[
        within_stats["metric"] == metric_name
    ].set_index("display_group")

    contrast_rows = contrasts[
        (contrasts["metric"] == metric_name)
        & (contrasts["group_A"] == POOLED_ML)
        & (contrasts["group_B"] == LATERAL)
    ]

    if contrast_rows.empty:
        print(
            f"[WARN] {metric_name}: no pooled M+L versus lateral-only "
            "contrast row was found; dedicated CV plot not written."
        )
        return False

    contrast_row = contrast_rows.iloc[0]
    rng = np.random.default_rng(seed)
    ...
    for group_name in comparison_groups:
        ...
        if values.size == 0 or group_name not in metric_within.index:
            print(
                f"[WARN] {metric_name}: no finite values for "
                f"{GROUP_LABELS[group_name]}; dedicated CV plot not written."
            )
            return False

        stats_row = metric_within.loc[group_name]
        plot_data.append((group_name, values, stats_row))

    fig, ax = plt.subplots(figsize=(6.9, 6.3))

    for group_index, (group_name, values, stats_row) in enumerate(plot_data):
        jitter = rng.uniform(-0.10, 0.10, size=len(values))
        x = np.full(values.size, group_index, dtype=float) + jitter
        ...
        estimate = float(stats_row["observed_DID"])
        ci_low = float(stats_row["bootstrap_CI95_low"])
        ci_high = float(stats_row["bootstrap_CI95_high"])

        ax.errorbar(
            group_index,
            estimate,
            yerr=np.array(
                [[estimate - ci_low], [ci_high - estimate]]
            ),
            fmt="D",
            markersize=8,
            markerfacecolor="black",
    ...
    # Draw the direct-comparison bracket.
    all_values = np.concatenate([values for _, values, _ in plot_data])
    finite_values = all_values[np.isfinite(all_values)]
    y_min = float(np.min(finite_values))
    y_max = float(np.max(finite_values))
    y_span = y_max - y_min
    if y_span <= 0:
        y_span = max(abs(y_max), 1.0) * 0.25

    bracket_y = y_max + 0.16 * y_span
    bracket_height = 0.05 * y_span
    ...
    difference = float(
        contrast_row["observed_DID_difference_A_minus_B"]
    )
    ci_low = float(contrast_row["bootstrap_CI95_low"])
    ci_high = float(contrast_row["bootstrap_CI95_high"])
    p_raw = float(contrast_row["labelshuffle_p_raw"])
    p_fdr = float(contrast_row["labelshuffle_p_fdr_bh"])

    annotation = (
        f"M+L − lateral = {difference:+.3f}\n"
        f"95% CI [{ci_low:+.3f}, {ci_high:+.3f}]\n"
        f"one-sided p={format_p(p_raw)}; "
        f"BH-FDR p={format_p(p_fdr)}"
    )
    ax.text(
        0.5,
        bracket_y + bracket_height + 0.025 * y_span,
        annotation,
        ha="center",
        va="bottom",
        fontsize=9,
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(
        [GROUP_LABELS[LATERAL], GROUP_LABELS[POOLED_ML]],
        rotation=12,
        ha="right",
    )
    ax.set_xlabel("Lesion group")
    ...
    ax.set_title(
        f"{spec['title']}\n"
        "Direct pooled M+L versus lateral-only comparison"
    )
    ax.grid(axis="y", alpha=0.22)

    current_bottom, current_top = ax.get_ylim()
    required_top = bracket_y + bracket_height + 0.22 * y_span
...
def main() -> None:
    ...
    # Dedicated CV comparison: pooled M+L versus lateral-only.
    direct_cv_contrasts = contrasts[
        contrasts["metric"].isin(["cv_raw", "cv_log2"])
        & (contrasts["group_A"] == POOLED_ML)
        & (contrasts["group_B"] == LATERAL)
    ].copy()
    direct_cv_contrasts.to_csv(
        out_dir / "CV_pooled_ML_vs_lateral_direct_contrasts.csv",
        index=False,
    )

    plot_direct_ml_vs_lateral_did(
        animal,
        within_stats,
        contrasts,
        metric_name="cv_raw",
        output_path=(
            plot_dir
            / "CV_raw_pooled_ML_vs_lateral_direct_comparison.png"
        ),
    ...
    plot_direct_ml_vs_lateral_did(
        ...
        output_path=(
            plot_dir
            / "CV_log2_pooled_ML_vs_lateral_direct_comparison.png"
        ),
        seed=args.seed + 50_100,
        dpi=args.dpi,
        show=args.show,
    )
