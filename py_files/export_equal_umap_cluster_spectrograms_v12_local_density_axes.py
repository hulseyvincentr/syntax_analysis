...
SCRIPT_VERSION = "equal_umap_cluster_spectrograms_v12_local_density_axes"
...
def plot_umap_cluster_selected_timebins(
    *,
    ...
    selected_all = np.concatenate([idx for _, idx in groups_present if np.asarray(idx).size > 0])
    sx = xy[selected_all, 0]
    sy = xy[selected_all, 1]
    x_min = float(np.nanmin(sx))
    x_max = float(np.nanmax(sx))
    y_min = float(np.nanmin(sy))
    y_max = float(np.nanmax(sy))
    pad_x = max(1e-6, (x_max - x_min) * 0.06)
    pad_y = max(1e-6, (y_max - y_min) * 0.06)
    x_edges = np.linspace(x_min - pad_x, x_max + pad_x, int(density_bins) + 1)
    y_edges = np.linspace(y_min - pad_y, y_max + pad_y, int(density_bins) + 1)
    plot_xlim = (float(x_edges[0]), float(x_edges[-1]))
    plot_ylim = (float(y_edges[0]), float(y_edges[-1]))
    ...
    if set(group_names) == {"early_pre", "late_pre", "early_post", "late_post"}:
        ...
        fig.suptitle(
            ...
            f"{n_each_text} | pre BC={bc_pre:.3f} | post BC={bc_post:.3f} | pre/post BC={bc_prepost:.3f} | density bins={int(density_bins)}",
            y=0.985, fontsize=18
        )
    ...
    *,
    ...
    for cluster_id in cluster_ids:
        ...
        if mode_bc == "frame_weighted":
            ...
            raise ValueError("bc_analysis_mode must be 'frame_weighted' or 'run_balanced_phrase_normalized'")
        if dry_run:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--bc-analysis-mode",
        choices=["run_balanced_phrase_normalized", "frame_weighted"],
        ...
        help=(
            ...
            "frame_weighted reproduces the older equal-time-bin behavior."
        ),
    )
