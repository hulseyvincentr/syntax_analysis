#!/usr/bin/env python3
"""
make_avian_brain_outline_from_reference_svg_v3.py

Create a black-and-white avian brain outline/segmentation figure from the
Birdbrain-style SVG reference, while removing labels, arrows, color fills, and
the small disconnected anterior fragment.

This version is based on the SVG element IDs from the Birdbrain.svg-style file.
It does NOT require svgpathtools. It only needs CairoSVG if you want PNG export.
...
Example:
python make_avian_brain_outline_from_reference_svg_v3.py \
  --input-svg "$HOME/Downloads/Birdbrain(2) (1).svg" \
  --out-dir "$HOME/Desktop/avian_brain_outline_outputs" \
...
# Anatomical region paths in the Birdbrain.svg-style reference.
#
# IMPORTANT:
# path3213 is the small disconnected anterior fragment that appeared at the
# far right of the previous output. It is intentionally NOT included by default.
DEFAULT_ANATOMY_PATH_IDS: Set[str] = {
    ...
    # "path3213",  # small disconnected anterior fragment; removed by default
    "path4330",  # large telencephalon / pallium region
    "path4332",  # thalamus / hindbrain / ventral region
}

# Optional path IDs.
SMALL_ANTERIOR_FRAGMENT_PATH_IDS: Set[str] = {
    "path3213",
}

# Extra source outline path. This can create a double-line effect, so it is off
# by default.
...
def style_dict_to_string(style_dict: Dict[str, str]) -> str:
    """Convert a style dictionary back into an SVG style string."""
...
def set_black_white_region_style(elem: ET.Element, line_width: float) -> None:
    ...
    # Also set plain SVG attributes in case an editor prioritizes them.
    elem.attrib["fill"] = "#ffffff"
    elem.attrib["stroke"] = "#000000"
    elem.attrib["stroke-width"] = str(line_width)
    elem.attrib["stroke-linecap"] = "round"
    elem.attrib["stroke-linejoin"] = "round"


def strip_to_anatomy_only(root: ET.Element, keep_path_ids: Set[str], line_width: float) -> None:
    """Remove labels, arrows, and unwanted paths; restyle kept anatomy paths."""
    for parent in list(root.iter()):
        for child in list(parent):
            ...
            # Remove embedded raster images if present.
            elif tag == "image":
                parent.remove(child)
...
def convert_svg_to_bw_outline(
    ...
    include_small_anterior_fragment: bool = False,
    ...
    if include_small_anterior_fragment:
        keep_path_ids |= SMALL_ANTERIOR_FRAGMENT_PATH_IDS
    else:
        print("[INFO] Removing small disconnected anterior fragment: path3213")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "the Birdbrain-style SVG reference, without the small disconnected "
            "anterior fragment."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--include-small-anterior-fragment",
        action="store_true",
        help=(
            "Keep the small disconnected anterior fragment path3213. "
...
def main() -> None:
    ...
    svg_path, png_path = convert_svg_to_bw_outline(
        ...
        include_small_anterior_fragment=args.include_small_anterior_fragment,
