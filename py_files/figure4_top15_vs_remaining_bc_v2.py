...
def load_selected_pairs(path: str) -> pd.DataFrame:
    ...
    syll_col = first_present(df.columns, ['syllable', 'Syllable', 'label', 'cluster', 'cluster_id', 'Cluster', 'Cluster ID', 'cluster_label', 'syllable_cluster'])
...
def load_bc_table(path: str, metadata_excel: Optional[str] = None) -> pd.DataFrame:
    ...
    if not p.exists():
        raise FileNotFoundError(p)
    df = pd.read_csv(p)
    print(f'[INFO] BC input columns: {list(df.columns)}')
    animal_col = first_present(df.columns, ['animal_id', 'Animal ID', 'bird_id', 'bird'])
    syll_col = first_present(df.columns, ['syllable', 'Syllable', 'label', 'cluster', 'cluster_id', 'Cluster', 'Cluster ID', 'cluster_label', 'syllable_cluster'])
    if animal_col is None or syll_col is None:
    ...
    # Wide format first
    pre_col = first_present(df.columns, ['pre_bc', 'bc_pre', 'pre_BC', 'Pre BC', 'late_pre_bc', 'median_pre_bc', 'pre_bhattacharyya_coefficient', 'pre_bc_value'])
    post_col = first_present(df.columns, ['post_bc', 'bc_post', 'post_BC', 'Post BC', 'post_period_bc', 'median_post_bc', 'post_bhattacharyya_coefficient', 'post_bc_value'])
    ...
    if pre_col and post_col:
        ...
        value_col = first_present(df.columns, ['bc', 'BC', 'bhattacharyya_coefficient', 'Bhattacharyya coefficient', 'bhattacharyya_coeff', 'median_bc', 'BC_value', 'bc_value', 'value'])
