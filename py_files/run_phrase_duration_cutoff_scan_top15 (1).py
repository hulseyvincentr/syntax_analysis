...
for the search across cutoffs. It also writes a dedicated report for a target
upper fraction (15% by default), including the exact syllables selected and the
number retained for every bird and cross-fitting direction:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.15,
        help=(
            "Upper-ranked fraction for the dedicated selection/count report. "
            "The default 0.15 corresponds to the exploratory top-15% candidate."
        ),
    )
    parser.add_argument(
        "--target-method",
        choices=METHODS,
        default="crossfit_pooled_variance",
        help=(
            "Ranking design used for the dedicated target-fraction report. "
            "Crossfit pooled variance is the default because it separates "
            "selection and held-out effect estimation."
        ),
    )
    parser.add_argument(
        "--no-plots",
...
def validate_args(args: argparse.Namespace) -> list[float]:
    ...
    if not any(math.isclose(args.target_fraction, value, abs_tol=1e-12) for value in fractions):
        fractions.append(float(args.target_fraction))
...
# -----------------------------------------------------------------------------
# Dedicated target-fraction selection and count report
# -----------------------------------------------------------------------------


def parse_selected_labels(value: Any) -> list[str]:
    """Parse a comma-separated selected-label field without changing labels."""
...
def target_fraction_tag(fraction: float) -> str:
    percentage = 100.0 * fraction
    if math.isclose(percentage, round(percentage), abs_tol=1e-9):
        return f"top{int(round(percentage))}pct"
    return f"top{percentage:g}pct".replace(".", "p")


def build_target_fraction_outputs(
    ...
    """Build exact selected-syllable and count summaries for one target fraction.

    Cross-fitted methods have two legitimate selected sets per bird: one set is
    ranked in fold A and evaluated in fold B, and the reciprocal set is ranked in
    fold B and evaluated in fold A. The outputs therefore retain both sets and
    report their intersection and union rather than forcing them into one list.
    """
    selected_direction_rows = [
        dict(row)
        for row in direction_rows
        if str(row.get("method")) == target_method
        and math.isclose(
            float(row.get("fraction", math.nan)), target_fraction, abs_tol=1e-12
        )
    ]
    ...
    # One row per selected syllable, preserving the screen rank and held-out
    # delta values already calculated by the cutoff scan.
    rank_lookup: dict[tuple[str, str], list[Mapping[str, Any]]] = defaultdict(list)
    for row in rank_rows:
        if str(row.get("method")) != target_method:
            continue
        rank_lookup[(str(row.get("bird")), str(row.get("direction")))].append(row)

    selected_syllable_rows: list[dict[str, Any]] = []
    direction_count_rows: list[dict[str, Any]] = []
    for row in selected_direction_rows:
        bird = str(row["bird"])
        direction = str(row["direction"])
        labels = parse_selected_labels(row.get("selected_labels", ""))
        n_selected = int(round(float(row["n_selected_syllables"])))
        n_eligible = int(round(float(row["n_eligible_syllables"])))
        realized_fraction = n_selected / n_eligible if n_eligible else math.nan
        expected_count_unrounded = target_fraction * n_eligible

        direction_count_rows.append(
            {
                ...
                "direction": direction,
                "n_eligible_syllables": n_eligible,
                "target_count_before_rounding": expected_count_unrounded,
                "rounding_rule": "ceil(target_fraction * n_eligible), capped at n_eligible - 1",
                "n_selected_syllables": n_selected,
                "realized_selected_fraction": realized_fraction,
                "n_remaining_syllables": int(round(float(row["n_remaining_syllables"]))),
                "selected_labels": ",".join(labels),
                "selected_min_rank_score": row.get("selected_min_rank_score", math.nan),
                "only_one_selected": n_selected == 1,
                "two_or_fewer_selected": n_selected <= 2,
                "three_or_more_selected": n_selected >= 3,
            }
        )

        matching_ranks = sorted(
            rank_lookup.get((bird, direction), []),
            key=lambda rank_row: int(rank_row["rank"]),
        )
        selected_from_ranks = [
            rank_row
            for rank_row in matching_ranks
            if int(rank_row["rank"]) <= n_selected
        ]
        rank_labels = [str(rank_row["syllable"]) for rank_row in selected_from_ranks]
        if rank_labels != labels:
            raise RuntimeError(
                f"Selected-label mismatch for {bird}, {direction}: "
                f"cutoff row={labels}, ranking rows={rank_labels}"
            )
        for rank_row in selected_from_ranks:
            selected_syllable_rows.append(
                {
                    "method": target_method,
                    "target_fraction": target_fraction,
                    "bird": bird,
                    "group": row["group"],
                    "direction": direction,
                    "syllable": rank_row["syllable"],
                    "rank": rank_row["rank"],
                    "n_eligible_syllables": n_eligible,
                    "rank_percentile": rank_row["rank_percentile"],
                    "rank_score": rank_row["rank_score"],
                    "heldout_delta_sd": rank_row["delta_sd"],
                    "heldout_delta_cv": rank_row["delta_cv"],
                }
            )

    # Collapse reciprocal directions to one count/overlap row per bird.
    rows_by_bird: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in direction_count_rows:
        rows_by_bird[str(row["bird"])].append(row)

    bird_rows: list[dict[str, Any]] = []
    for bird, rows in sorted(rows_by_bird.items()):
        rows = sorted(rows, key=lambda row: str(row["direction"]))
        sets = [set(parse_selected_labels(row["selected_labels"])) for row in rows]
        union = set().union(*sets) if sets else set()
        intersection = set.intersection(*sets) if sets else set()
        counts = [int(row["n_selected_syllables"]) for row in rows]
        eligible_counts = [int(row["n_eligible_syllables"]) for row in rows]
        realized = [float(row["realized_selected_fraction"]) for row in rows]
        jaccard = (
            len(intersection) / len(union)
            if union
            else math.nan
        )
        bird_row: dict[str, Any] = {
            "method": target_method,
            "target_fraction": target_fraction,
            "bird": bird,
            "group": rows[0]["group"],
            "n_directions_available": len(rows),
            "directions_available": ",".join(str(row["direction"]) for row in rows),
            "mean_n_eligible_syllables": safe_mean(eligible_counts),
            "min_n_eligible_syllables": min(eligible_counts),
            "max_n_eligible_syllables": max(eligible_counts),
            "mean_n_selected_syllables": safe_mean(counts),
            "min_n_selected_syllables": min(counts),
            "max_n_selected_syllables": max(counts),
            "mean_realized_selected_fraction": safe_mean(realized),
            "minimum_direction_has_one_selected": min(counts) == 1,
            "minimum_direction_has_two_or_fewer_selected": min(counts) <= 2,
            "all_directions_have_three_or_more_selected": min(counts) >= 3,
            "selected_union_count": len(union),
            "selected_union_labels": ",".join(sorted(union, key=label_sort_key)),
            "selected_intersection_count": len(intersection),
            "selected_intersection_labels": ",".join(
                sorted(intersection, key=label_sort_key)
            ),
            "selected_set_jaccard_across_directions": jaccard,
        }
        for row in rows:
            suffix = str(row["direction"])
            bird_row[f"{suffix}_n_eligible"] = row["n_eligible_syllables"]
            bird_row[f"{suffix}_n_selected"] = row["n_selected_syllables"]
            bird_row[f"{suffix}_realized_fraction"] = row[
                "realized_selected_fraction"
            ]
            bird_row[f"{suffix}_selected_labels"] = row["selected_labels"]
        bird_rows.append(bird_row)

    # Group-level check focused on whether the 15% rule leaves too few
    # syllables for a stable within-bird median.
    bird_rows_by_group: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in bird_rows:
        bird_rows_by_group[str(row["group"])].append(row)

    group_rows: list[dict[str, Any]] = []
    for group, rows in sorted(bird_rows_by_group.items()):
        mean_counts = [float(row["mean_n_selected_syllables"]) for row in rows]
        min_counts = [int(row["min_n_selected_syllables"]) for row in rows]
        intersection_counts = [int(row["selected_intersection_count"]) for row in rows]
        union_counts = [int(row["selected_union_count"]) for row in rows]
        group_rows.append(
            {
                "method": target_method,
                "target_fraction": target_fraction,
                "group": group,
                "n_birds": len(rows),
                "median_mean_selected_count": safe_median(mean_counts),
                "minimum_mean_selected_count": min(mean_counts),
                "maximum_mean_selected_count": max(mean_counts),
                "n_birds_with_one_selected_in_at_least_one_direction": sum(
                    count == 1 for count in min_counts
                ),
                "n_birds_with_two_or_fewer_in_at_least_one_direction": sum(
                    count <= 2 for count in min_counts
                ),
                "n_birds_with_three_or_more_in_every_direction": sum(
                    count >= 3 for count in min_counts
                ),
                "median_intersection_count": safe_median(intersection_counts),
                "median_union_count": safe_median(union_counts),
            }
        )

    return selected_syllable_rows, direction_count_rows, bird_rows, group_rows


def write_target_fraction_summary(
    path: Path,
    target_fraction: float,
    target_method: str,
    bird_rows: Sequence[Mapping[str, Any]],
    group_rows: Sequence[Mapping[str, Any]],
) -> None:
    pct = 100.0 * target_fraction
    one_birds = [
        str(row["bird"])
        for row in bird_rows
        if bool(row["minimum_direction_has_one_selected"])
    ]
    two_or_fewer_birds = [
        str(row["bird"])
        for row in bird_rows
        if bool(row["minimum_direction_has_two_or_fewer_selected"])
    ]
    lines = [
        f"Upper {pct:g}% selected-syllable count check",
        "=" * 45,
        "",
        f"Method: {target_method}",
        f"Target fraction: {target_fraction:g}",
        "Selection count rule: ceil(target fraction × eligible syllables).",
        "",
        "Important cross-fitting note",
        "----------------------------",
        "Cross-fitted methods select one set in A for testing in B and a reciprocal set in B for testing in A.",
        "The two sets need not be identical. The bird summary reports both sets, plus their union, intersection, and Jaccard overlap.",
        "",
        f"Birds represented: {len(bird_rows)}",
        f"Birds with only one selected syllable in at least one direction: {len(one_birds)}",
        f"  {', '.join(one_birds) if one_birds else 'None'}",
        f"Birds with two or fewer selected syllables in at least one direction: {len(two_or_fewer_birds)}",
        f"  {', '.join(two_or_fewer_birds) if two_or_fewer_birds else 'None'}",
        "",
        "Group summaries",
        "---------------",
    ]
    for row in group_rows:
        lines.extend(
            [
                f"{row['group']} (n={row['n_birds']} birds)",
                f"  Median selected count per direction: {row['median_mean_selected_count']}",
                f"  Range of bird mean selected counts: {row['minimum_mean_selected_count']} to {row['maximum_mean_selected_count']}",
                f"  Birds with one selected in at least one direction: {row['n_birds_with_one_selected_in_at_least_one_direction']}",
                f"  Birds with <=2 selected in at least one direction: {row['n_birds_with_two_or_fewer_in_at_least_one_direction']}",
                f"  Birds with >=3 selected in every direction: {row['n_birds_with_three_or_more_in_every_direction']}",
                f"  Median reciprocal-set intersection count: {row['median_intersection_count']}",
                f"  Median reciprocal-set union count: {row['median_union_count']}",
                "",
            ]
        )
    path.write_text("\n".join(lines))
...
def main() -> None:
    ...
    (
        target_selected_syllable_rows,
        target_direction_count_rows,
        target_bird_rows,
        target_group_rows,
    ) = build_target_fraction_outputs(
        rank_rows,
        direction_rows,
    ...
    print(
        f"[INFO] Built target-fraction report for top "
        f"{100.0 * args.target_fraction:g}% using {args.target_method}"
    )
    ...
    target_tag = target_fraction_tag(args.target_fraction)
    write_csv(
        args.output / f"{target_tag}_selected_syllables.csv",
        target_selected_syllable_rows,
    )
    write_csv(
        args.output / f"{target_tag}_counts_by_bird_and_direction.csv",
        target_direction_count_rows,
    )
    write_csv(
        args.output / f"{target_tag}_bird_count_and_overlap_summary.csv",
        target_bird_rows,
    )
    write_csv(
        args.output / f"{target_tag}_count_summary_by_group.csv",
        target_group_rows,
    )
    write_target_fraction_summary(
        args.output / f"{target_tag}_summary.txt",
        args.target_fraction,
        args.target_method,
        target_bird_rows,
        target_group_rows,
