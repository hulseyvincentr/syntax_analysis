...
def degradation_multiplier(feature: str) -> float:
    ...
    # Any corr_to_* feature is a similarity measure, so lower correlation means more change.
    if str(feature).startswith("corr_to_"):
        return -1.0
    return 1.0
...
def degradation_value(feature: str, values) -> np.ndarray:
    ...
    # Any corr_to_* feature is converted to distance/degradation = 1 - correlation.
    if str(feature).startswith("corr_to_"):
        return 1.0 - vals
    return vals
...
def degradation_axis_label(feature: str) -> str:
    ...
    if str(feature).startswith("distance_from_"):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default=None,
                   help="Output folder. Default: <phrase-position-root>/aggregate_bird_effects_v10_lag10_previous")
...
def write_readme(out_dir: Path):
    (out_dir / "README_phrase_position_bird_effects_v10.txt").write_text(
...
def main():
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else root / "aggregate_bird_effects_v10_lag10_previous"
    ...
    print(f"[SAVED] {out_dir / 'README_phrase_position_bird_effects_v10.txt'}")
    print("[DONE]")
