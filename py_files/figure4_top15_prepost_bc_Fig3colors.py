...
Primary displayed tests:
  - Top 15%: paired Wilcoxon, one-sided Post < Pre
  - Remaining 85%: paired Wilcoxon, one-sided Post < Pre
  - Holm adjustment across those two tests
...
def read_bc_table(path: str, metadata_excel: Optional[str]) -> pd.DataFrame:
    csv_path = Path(path)
    if not csv_path.exists():
        raise FileNotFoundError(csv_path)
    table = pd.read_csv(csv_path)
    ...
    if animal_col is None or syllable_col is None or pre_col is None or post_col is None:
        raise ValueError(
    ...
    out = table.copy()
    out = out.rename(columns={animal_col: "animal_id", syllable_col: "syllable"})
    out["animal_id"] = out["animal_id"].astype(str).str.strip()
    out["syllable"] = out["syllable"].astype(str).str.strip()
...
def paired_wilcoxon(pre: np.ndarray, post: np.ndarray) -> float:
...
def run_tests(by_bird: pd.DataFrame) -> pd.DataFrame:
    ...
    for subset, pre_col, post_col in [
        ...
        diff = post - pre
        signflip_p, n_permutations = exact_signflip_p(diff, alternative="less")
        wilcoxon_p = paired_wilcoxon(pre, post)
        ci_low, ci_high = bootstrap_mean_ci(diff)
        rows.append(
            {
                "comparison": f"{subset}: Post < Pre",
                "subset": subset,
                "n_birds": len(diff),
                ...
                "signflip_one_sided_p": signflip_p,
                "n_signflip_permutations": n_permutations,
            }
        )

    tests = pd.DataFrame(rows)
    tests["wilcoxon_holm_p"] = holm_adjust(tests["wilcoxon_one_sided_p"].tolist())
    tests["signflip_holm_p"] = holm_adjust(tests["signflip_one_sided_p"].tolist())
        ...
        [
            {
                ...
                "signflip_one_sided_p": interaction_p,
                "n_signflip_permutations": n_permutations,
                "wilcoxon_holm_p": np.nan,
                "signflip_holm_p": np.nan,
            }
        ]
    )
...
def plot_panel(by_bird: pd.DataFrame, tests: pd.DataFrame, output: Path, dpi: int, show: bool) -> None:
    ...
    top_p = float(top_row["wilcoxon_holm_p"])
    remaining_p = float(remaining_row["wilcoxon_holm_p"])
    add_bracket(
        ax,
        positions["top15_pre"],
...
def write_text_summary(
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("Displayed tests: one-sided paired Wilcoxon (Post < Pre), Holm-adjusted across two subsets\n\n")
        ...
        for subset in ["Top 15%", "Remaining 85%"]:
            ...
            handle.write(f"one-sided Wilcoxon raw p: {row['wilcoxon_one_sided_p']:.8g}\n")
            handle.write(f"one-sided Wilcoxon Holm p: {row['wilcoxon_holm_p']:.8g}\n")
            handle.write(f"one-sided exact sign-flip raw p: {row['signflip_one_sided_p']:.8g}\n")
            handle.write(f"one-sided exact sign-flip Holm p: {row['signflip_holm_p']:.8g}\n\n")
        ...
        handle.write(
            ...
            f"{interaction['signflip_one_sided_p']:.8g}\n"
        )
        handle.write(
            "One-sided paired Wilcoxon p (Top 15% more negative): "
            f"{interaction['wilcoxon_one_sided_p']:.8g}\n"
        )
...
def main() -> None:
    ...
    bc = read_bc_table(args.bc_csv, args.metadata_excel)
