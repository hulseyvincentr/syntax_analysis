...
Primary design
--------------
* Early pre: the singing days immediately preceding the late-pre window.
* Late pre: the final singing days before surgery.
...
python run_segmented_extreme_phrase_localization.py \
  ...
  --early-pre-singing-days 7 \
  --late-pre-singing-days 7 \
  --post-end-day 28 \
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum available early-pre singing days required for a bird.",
    )
    parser.add_argument(
        "--min-late-pre-singing-days",
        type=int,
        default=5,
        help="Minimum available late-pre singing days required for a bird.",
    )
...
def choose_period_dates(
    ...
    pre_dates = sorted(d for d in all_dates if (d - surgery).days <= -1)
    late_pre = pre_dates[-spec.late_pre_singing_days :]
    remaining = pre_dates[: max(0, len(pre_dates) - len(late_pre))]
...
def build_fold_assignment(
    ...
    return {"late_pre": late_pre_assignment, "post": post_assignment}
...
def analyze_bird(
    ...
    bird_pre_eligible = bool(
        len(period_dates["early_pre"]) >= args.min_early_pre_singing_days
        and len(period_dates["late_pre"]) >= args.min_late_pre_singing_days
    )

    date_rows = period_date_rows(
        bird, group, surgery, period_dates, folds, spec
    ) if detailed else []
    ...
    for label in sorted(durations, key=label_sort_key):
        ...
        if spec.remove_longest_post_per_syllable:
            post_dates_for_label = maps["post"]
            trimmed = remove_longest(post_dates_for_label)
            removed_dates = set(post_dates_for_label) - set(trimmed)
            maps["post"] = trimmed
            # Rebuild every post segment from the trimmed full-post map.
            for period in (
                "early_post",
                "late_post",
                "post_days_1_7",
        ...
        threshold = (
            ...
            if bird_pre_eligible and early_values.size >= args.min_early_pre_phrases
            else math.nan
        )
        summaries = {
        ...
        full_ok = bool(
            bird_pre_eligible
            and eligible_full(
                summaries["early_pre"], summaries["late_pre"], summaries["post"], args
            )
        )

        if detailed and math.isfinite(threshold):
            for period, day_map in maps.items():
                if period == "post":
                    continue
        ...
        full_row = {
            ...
            **summaries["post"].as_dict("post_"),
            "delta_late_pre_to_early_post_mean_daily_burden_seconds_per_100": delta(
                summaries["early_post"], summaries["late_pre"], "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_pre_to_late_post_mean_daily_burden_seconds_per_100": delta(
                summaries["late_post"], summaries["late_pre"], "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_pre_to_full_post_mean_daily_burden_seconds_per_100": delta(
                summaries["post"], summaries["late_pre"], "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_post_minus_early_post_mean_daily_burden_seconds_per_100": delta(
                summaries["late_post"], summaries["early_post"], "mean_daily_burden_seconds_per_100"
            ),
            "delta_early_pre_to_late_pre_mean_daily_burden_seconds_per_100": delta(
                summaries["late_pre"], summaries["early_pre"], "mean_daily_burden_seconds_per_100"
            ),
        }
        ...
        for fold in ("A", "B"):
            ...
            early_post_map = filter_fold(maps["early_post"], folds["post"], fold)
            late_post_map = filter_fold(maps["late_post"], folds["post"], fold)
            block_maps = {
    ...
    for direction, screen_fold, test_fold in DIRECTIONS:
        ...
        for label, cache in label_cache.items():
            ...
            screen_delta = delta(
                ...
                screen["summaries"]["late_pre"],
                "mean_daily_burden_seconds_per_100",
            )
            heldout = {
                "heldout_delta_full_post": delta(
                    test["summaries"]["post"],
                    test["summaries"]["late_pre"],
                    "mean_daily_burden_seconds_per_100",
                ),
            ...
            for row in candidates:
                ...
                if selected_any and args.selected_syllable_day_permutations > 0:
                    day_p, day_p_method, day_p_n = day_permutation_p(
                        row["test"]["maps"]["late_pre"],
                        row["test"]["maps"]["post"],
                        label_cache[row["label"]]["threshold"],
                ...
                cross_rows.append(
                    {
                        **spec.as_dict(),
                        ...
                        "test_fold": test_fold,
                        "syllable": row["label"],
                        "threshold_seconds": label_cache[row["label"]]["threshold"],
                        "eligible_both": row["eligible_both"],
                        "screen_rank": rank if rank else "",
                        "selected_within_max_requested_k": selected_any,
                        "screen_delta_full_post_mean_daily_burden_seconds_per_100": row["screen_delta"],
                        **{
                            key + "_mean_daily_burden_seconds_per_100": value
                            for key, value in row.items()
                            if key.startswith("heldout_") and isinstance(value, (int, float))
                        },
        ...
        for k in sorted(set(top_k_values)):
            ...
            direction_row = {
                **spec.as_dict(),
                ...
                "selected_syllable_labels": ";".join(row["label"] for row in selected),
    ...
    for k in sorted(set(top_k_values)):
        rows = [row for row in direction_topk_rows if row["top_k_requested"] == k]
        available = [
            row
            for row in rows
        ...
        primary_available = both_available if args.require_both_directions else bool(available)
        endpoint_names = [
            "mean_heldout_delta_full_post_mean_daily_burden_seconds_per_100",
        ...
        row = {
            **spec.as_dict(),
            ...
            "n_eligible_syllables_A_screen_B_test": next(
                (r["n_eligible_syllables"] for r in rows if r["direction"] == "A_screen_B_test"),
                0,
            ),
            "n_eligible_syllables_B_screen_A_test": next(
                (r["n_eligible_syllables"] for r in rows if r["direction"] == "B_screen_A_test"),
                0,
            ),
            "selected_labels_A_screen_B_test": next(
                (r["selected_syllable_labels"] for r in rows if r["direction"] == "A_screen_B_test"),
                "",
            ),
            "selected_labels_B_screen_A_test": next(
                (r["selected_syllable_labels"] for r in rows if r["direction"] == "B_screen_A_test"),
                "",
            ),
        }
        for endpoint in endpoint_names:
            ...
            row[output_name] = (
                mean_or_nan(r[endpoint] for r in available) if primary_available else math.nan
            )
        crossfitted_rows.append(row)

    # Supplemental bird-level summaries from all full-eligible syllables.
    eligible_full_rows = [
    ...
    ] if detailed else [
        {
            ...
            "delta_late_pre_to_early_post_mean_daily_burden_seconds_per_100": delta(
                cache["summaries"]["early_post"], cache["summaries"]["late_pre"],
                "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_pre_to_late_post_mean_daily_burden_seconds_per_100": delta(
                cache["summaries"]["late_post"], cache["summaries"]["late_pre"],
                "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_pre_to_full_post_mean_daily_burden_seconds_per_100": delta(
                cache["summaries"]["post"], cache["summaries"]["late_pre"],
                "mean_daily_burden_seconds_per_100"
            ),
            "delta_late_post_minus_early_post_mean_daily_burden_seconds_per_100": delta(
                cache["summaries"]["late_post"], cache["summaries"]["early_post"],
                "mean_daily_burden_seconds_per_100"
            ),
        }
        for label, cache in label_cache.items()
    ...
    positive_deltas = sorted(
        [
            max(0.0, finite_or_nan(row["delta_late_pre_to_full_post_mean_daily_burden_seconds_per_100"]))
            for row in eligible_full_rows
            if math.isfinite(
                finite_or_nan(row["delta_late_pre_to_full_post_mean_daily_burden_seconds_per_100"])
            )
        ],
        reverse=True,
    )
    total_positive = float(np.sum(positive_deltas)) if positive_deltas else math.nan
    concentration = {}
    for k in (1, 3, 5):
    ...
    supplemental_row = {
        **spec.as_dict(),
        ...
        "median_all_syllable_delta_early_post": median_or_nan(
            row["delta_late_pre_to_early_post_mean_daily_burden_seconds_per_100"]
            for row in eligible_full_rows
        ),
        "median_all_syllable_delta_late_post": median_or_nan(
            row["delta_late_pre_to_late_post_mean_daily_burden_seconds_per_100"]
            for row in eligible_full_rows
        ),
        "median_all_syllable_delta_full_post": median_or_nan(
            row["delta_late_pre_to_full_post_mean_daily_burden_seconds_per_100"]
            for row in eligible_full_rows
        ),
        "median_all_syllable_late_minus_early_post": median_or_nan(
            row["delta_late_post_minus_early_post_mean_daily_burden_seconds_per_100"]
            for row in eligible_full_rows
        ),
        **concentration,
    }
...
def analyze_spec(
    ...
    for bird in sorted(bird_data):
        ...
        for key in outputs:
            source_key = "supplemental_row" if key == "supplemental_rows" else key
...
def main() -> None:
    ...
    # Sensitivity: change one component at a time from the primary specification.
    ...
    if not args.skip_sensitivity:
        ...
        for n_days in sorted(set(args.sensitivity_pre_singing_days)):
            if n_days == args.early_pre_singing_days == args.late_pre_singing_days:
                continue
            sensitivity_specs.append(
                AnalysisSpec(
                    name=f"pre_windows_{n_days}_singing_days_each",
                    quantile=args.primary_quantile,
        ...
        sensitivity_specs.append(
            AnalysisSpec(
                ...
                post_end_day=args.post_end_day,
                split_seed=args.seed,
                remove_longest_post_per_syllable=True,
            )
        )
    ...
    write_csv(
        ...
        primary["date_rows"],
        [
            "spec_name", "bird", "group", "period", "date", "relative_day", "fold",
            "quantile", "early_pre_singing_days_requested",
            "late_pre_singing_days_requested", "post_end_day", "split_seed",
        ],
    )
    daily_fields = [
    ...
    lines = [
        "Segmented extreme phrase localization analysis",
        "==============================================",
        f"Input directory: {args.json_dir.resolve()}",
        f"Birds read: {len(bird_data)}",
        (
            f"Pre windows: preceding {args.early_pre_singing_days} singing days "
            f"(early pre) and final {args.late_pre_singing_days} singing days "
            "(late pre)"
        ),
        (
