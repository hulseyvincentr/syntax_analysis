...
class UMAPPrePostConfig:
    ...
    # Global (all clusters) UMAP + Bhattacharyya overlap
    run_global_umap: bool = True
    global_max_points_per_period: Optional[int] = None  # optional cap for global fit (balanced)
    global_plot_max_points_per_period: Optional[int] = None  # optional cap for global plotting (balanced)
...
def run_one_bird(cfg: UMAPPrePostConfig) -> Path:
    ...
    # -----------------------------
    # Global UMAP across ALL clusters (balanced pre vs post) + overall Bhattacharyya coefficient
    # -----------------------------
    if bool(getattr(cfg, "run_global_umap", True)):
        # define "all clusters" set (optionally excluding noise)
        if cfg.include_noise:
            ok_cluster_mask = np.ones_like(clusters, dtype=bool)
        else:
            ok_cluster_mask = (clusters != -1)

        idx_pre_all = np.where(pre_mask & ok_cluster_mask)[0]
        idx_post_all = np.where(post_mask & ok_cluster_mask)[0]

        n_pre_raw_all = int(idx_pre_all.size)
        n_post_raw_all = int(idx_post_all.size)

        if n_pre_raw_all >= min_need and n_post_raw_all >= min_need:
            idx_pre_fit, idx_post_fit, n_target, _, _ = _balanced_subsample_pair(
                idx_pre_all, idx_post_all, rng=rng, cap=getattr(cfg, "global_max_points_per_period", None)
            )

            # optional plot downsample (still balanced)
            idx_pre_plot = idx_pre_fit
            idx_post_plot = idx_post_fit
            if getattr(cfg, "global_plot_max_points_per_period", None) is not None:
                idx_pre_plot, idx_post_plot, n_plot, _, _ = _balanced_subsample_pair(
                    idx_pre_fit, idx_post_fit, rng=rng, cap=getattr(cfg, "global_plot_max_points_per_period", None)
                )
            else:
                n_plot = int(n_target)
            ...
            bc_all = bhattacharyya_coefficient_2d_hist(
                xy_fit[~is_post_fit],
                xy_fit[is_post_fit],
                bins=getattr(cfg, "bc_bins", 100),
            )

            # build plotted subset from fitted embedding
            ...
            out_png_all = clusters_dir / f"{prefix}_ALLCLUSTERS_before_after_overlap.png"
            ...
            print(f"[UMAP] ALL clusters: pre_raw={n_pre_raw_all} post_raw={n_post_raw_all} used={n_target} plot_per_period={n_plot} BC={bc_all:.3f} -> {out_png_all.name}")
        else:
            print(f"[UMAP] ALL clusters skipped: pre_raw={n_pre_raw_all} post_raw={n_post_raw_all} (min_need={min_need})")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-global-umap", action="store_true", help="Disable global (all clusters) UMAP+BC plot")
    p.add_argument("--global-max-points-per-period", default=None, type=int, help="Optional cap for global UMAP fit per period (balanced)")
    p.add_argument("--global-plot-max-points-per-period", default=None, type=int, help="Optional cap for global plotting per period (balanced)")
    return p
