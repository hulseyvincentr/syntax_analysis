#!/usr/bin/env python3
"""
phrase_position_acoustic_crosscorr_v14_local_rate_qc_bw.py
...
def _transform_qc_spectrogram(X_seg: Optional[np.ndarray], args) -> Optional[np.ndarray]:
    if X_seg is None or np.asarray(X_seg).size == 0:
        return None
    ...
    return transform_spectrogram_for_corr(Xc, spec_scale=args.spec_scale, corr_transform=args.qc_plot_transform)
...
def _plot_spectrogram_on_axis(
    ...
    if seg_start_rel_bin is not None and seg_end_rel_bin is not None:
        ...
        ax.axvline(x0, color="red", linestyle="--", linewidth=1.0, alpha=0.8)
        ax.axvline(x1, color="red", linestyle="--", linewidth=1.0, alpha=0.8)
    ax.set_title(title, fontsize=10)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Frequency (kHz)")


def make_local_pair_qc_outputs(pair_df: pd.DataFrame, X: np.ndarray, out_dir: Path, prefix: str, args) -> None:
    ...
    with PdfPages(qc_pdf) as pdf:
        for _, row in qc_rows.iterrows():
            ...
            fig, axes = plt.subplots(1, 2, figsize=(10.5, 5.5), constrained_layout=True)
            ...
            _plot_spectrogram_on_axis(axes[0], Y_a, args, title=title_a, seg_start_rel_bin=rel0_a, seg_end_rel_bin=rel1_a, vmin=vmin, vmax=vmax)
            _plot_spectrogram_on_axis(axes[1], Y_b, args, title=title_b, seg_start_rel_bin=rel0_b, seg_end_rel_bin=rel1_b, vmin=vmin, vmax=vmax)
...
def main():
    ...
    parser.add_argument("--qc-plot-transform", default="log_power", choices=["raw", "linear_power", "log_power"],
                        help="Transform used for QC spectrogram displays. Default log_power.")
    parser.add_argument("--qc-plot-cmap", default="Greys_r",
                        help="Matplotlib colormap used for QC spectrogram displays. Default Greys_r for black-and-white rendering.")
    ...
    parser.add_argument("--qc-pad-ms", type=float, default=12.0,
                        help="Amount of time context to show before and after each QC syllable, in ms. Default 12 ms.")
    ...
    parser.add_argument("--qc-contrast-low-pct", type=float, default=1.0,
                        help="Lower percentile used for QC grayscale contrast scaling. Default 1.")
    parser.add_argument("--qc-contrast-high-pct", type=float, default=99.5,
                        help="Upper percentile used for QC grayscale contrast scaling. Default 99.5.")
