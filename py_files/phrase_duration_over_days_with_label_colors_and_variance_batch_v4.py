...
def _apply_aligned_variance_axis_scaling(
    ax,
    values: np.ndarray,
    *,
    y_scale: str = "linear",
    ...
    ymax = float(y_max_variance_ms2) if y_max_variance_ms2 is not None else float(np.nanmax(vals))
    ymax = max(ymax, 1.0)
    positive = vals[vals > 0]

    y_scale = str(y_scale or "linear").lower()
    if y_scale == "log":
        bottom = float(np.nanmin(positive)) if positive.size else 1.0
        bottom = max(bottom, 1e-6)
        ax.set_yscale("log")
        ax.set_ylim(bottom, ymax)
    elif y_scale == "symlog":
        if positive.size:
            linthresh = float(np.nanpercentile(positive, 10))
            if not np.isfinite(linthresh) or linthresh <= 0:
                linthresh = float(np.nanmin(positive))
        else:
            linthresh = 1.0
        linthresh = max(linthresh, 1.0)
        ax.set_yscale("symlog", linthresh=linthresh, linscale=1.0)
        ax.set_ylim(0, ymax)
    else:
        ax.set_ylim(0, ymax * 1.05 if y_max_variance_ms2 is None else ymax)
...
def _plot_batch_aligned_variance_lines(
    ...
    *,
    ...
    transparent: bool = False,
    title: Optional[str] = None,
    relative_day_min: int = -30,
    relative_day_max: int = 31,
    y_scale: str = "symlog",
    ...
    df = df.loc[(df["relative_day"] >= int(relative_day_min)) & (df["relative_day"] <= int(relative_day_max))].copy()
    if df.empty:
        return
    ...
    for (animal_id, syllable), g in df.groupby(["animal_id", "syllable"], sort=False):
        ...
        ax.plot(
            ...
            color=color,
            linewidth=1.2,
            alpha=0.38 if monochrome else 0.32,
            zorder=1,
        )
    ...
    xmin = int(relative_day_min)
    xmax = int(relative_day_max)
    ax.set_xlim(xmin, xmax)
    ax.axvline(0, color="red", linestyle="--", linewidth=1.6, zorder=0)

    _apply_aligned_variance_axis_scaling(
        ax,
        df["Variance (ms^2)"].to_numpy(dtype=float),
        y_scale=y_scale,
    ...
    if tick_positions.size > 16:
        step = max(1, int(np.ceil(tick_positions.size / 16.0)))
        tick_positions = tick_positions[::step]
        if tick_positions[-1] != xmax:
            tick_positions = np.r_[tick_positions, xmax]
...
def _plot_batch_aligned_variance_by_lesion_group(
    ...
    *,
    ...
    def _draw_one(ax, subdf: pd.DataFrame, group_name: str):
        ...
        _apply_aligned_variance_axis_scaling(
            ax,
            subdf["Variance (ms^2)"].to_numpy(dtype=float),
            y_scale=y_scale,
            y_max_variance_ms2=y_max_variance_ms2,
        )
        ax.set_xticks(tick_positions)
    ...
    if split_panels:
        fig, axes = plt.subplots(len(LESION_GROUP_ORDER), 1, figsize=(figsize[0], max(figsize[1], 14)), sharex=True, sharey=False)
        ...
        for ax, group_name in zip(axes, LESION_GROUP_ORDER):
            ...
            if subdf.empty:
                ...
                ax.axvline(0, color="red", linestyle="--", linewidth=1.5, zorder=0)
                ax.set_xlim(xmin, xmax)
                _apply_aligned_variance_axis_scaling(ax, np.array([1.0]), y_scale=y_scale, y_max_variance_ms2=y_max_variance_ms2)
                for spine in ["top", "right"]:
                    ax.spines[spine].set_visible(False)
            else:
    ...
    if title:
        ax.set_title(title)
    from matplotlib.lines import Line2D
    handles = [Line2D([0], [0], color=LESION_GROUP_COLORS[g], lw=3, label=g) for g in LESION_GROUP_ORDER if (df["lesion_group"] == g).any()]
    ...
    if not all_df.empty:
        if make_green_aggregate_variance_plots:
            _plot_batch_aligned_variance_lines(
                ...
                relative_day_min=aligned_relative_day_min,
                relative_day_max=aligned_relative_day_max,
                y_scale=aligned_variance_y_scale,
                show_mean=aligned_show_mean,
            )
        if make_colored_aggregate_variance_plots:
...
def _cache_paths_for_animal(output_dir: Path, animal_id: str) -> Dict[str, Path]:
    return {
        "metadata": output_dir / f"{animal_id}_plot_cache_metadata.csv",
        "phrase_long": output_dir / f"{animal_id}_phrase_duration_long.csv",
        "daily_variance": output_dir / f"{animal_id}_daily_variance_long.csv",
        "overall_variance": output_dir / f"{animal_id}_overall_syllable_variance.csv",
    }


def _sorted_label_list(labels: Sequence[Any]) -> List[str]:
    ...
    def _key(lbl: str):
        try:
            ...
            return (1, lbl)
    return sorted(dict.fromkeys(labs), key=_key)


def _mark_top_fraction_in_overall_df(overall_df: pd.DataFrame, top_fraction: float = 0.30) -> pd.DataFrame:
    out = overall_df.copy()
    if out.empty or "overall_variance_ms2" not in out.columns:
        out["rank"] = pd.Series(dtype="Int64")
        out["is_top30"] = False
        return out
    out["overall_variance_ms2"] = pd.to_numeric(out["overall_variance_ms2"], errors="coerce")
    out = out.dropna(subset=["overall_variance_ms2"]).sort_values("overall_variance_ms2", ascending=False).reset_index(drop=True)
    if out.empty:
        out["rank"] = pd.Series(dtype="Int64")
        out["is_top30"] = False
        return out
    n_top = max(1, int(math.ceil(len(out) * float(top_fraction))))
    out["rank"] = np.arange(1, len(out) + 1)
    out["is_top30"] = out["rank"] <= n_top
    return out


def _build_phrase_duration_long_df(
    df: pd.DataFrame,
    labels: Sequence[str],
    *,
    animal_id: str,
    treatment_date_dt: Optional[pd.Timestamp],
    lesion_group: Optional[str] = None,
) -> pd.DataFrame:
    rows: List[pd.DataFrame] = []
    treat_norm = pd.Timestamp(treatment_date_dt).normalize() if treatment_date_dt is not None and not pd.isna(treatment_date_dt) else pd.NaT
    for lbl in _sorted_label_list(labels):
        col = f"syllable_{lbl}_durations"
        if col not in df.columns:
            continue
        ex = df[["Date", col]].explode(col).rename(columns={col: "Phrase Duration (ms)"}).copy()
        ex["Phrase Duration (ms)"] = pd.to_numeric(ex["Phrase Duration (ms)"], errors="coerce")
        ex["DateDay"] = pd.to_datetime(ex["Date"], errors="coerce")
        ex["DateDay"] = _drop_tz_if_present(ex["DateDay"]).dt.normalize()
        ex = ex.dropna(subset=["Phrase Duration (ms)", "DateDay"])
        if ex.empty:
            continue
        ex["animal_id"] = animal_id
        ex["syllable"] = str(lbl)
        ex["lesion_group"] = lesion_group
        ex["treatment_date"] = treat_norm
        if pd.notna(treat_norm):
            ex["relative_day"] = (ex["DateDay"] - treat_norm).dt.days.astype(int)
        else:
            ex["relative_day"] = pd.Series([pd.NA] * len(ex), dtype="Int64")
        rows.append(ex[["animal_id", "lesion_group", "syllable", "DateDay", "relative_day", "Phrase Duration (ms)", "treatment_date"]])
    if not rows:
        return pd.DataFrame(columns=["animal_id", "lesion_group", "syllable", "DateDay", "relative_day", "Phrase Duration (ms)", "treatment_date"])
    out = pd.concat(rows, ignore_index=True)
    return out.sort_values(["syllable", "DateDay"]).reset_index(drop=True)


def _build_daily_variance_long_df(
    phrase_long_df: pd.DataFrame,
    full_date_range: pd.DatetimeIndex,
    *,
    animal_id: str,
    treatment_date_dt: Optional[pd.Timestamp],
    ...
    if phrase_long_df.empty:
        return pd.DataFrame(columns=["animal_id", "lesion_group", "syllable", "DateDay", "relative_day", "Variance (ms^2)", "N", "treatment_date"])
    treat_norm = pd.Timestamp(treatment_date_dt).normalize() if treatment_date_dt is not None and not pd.isna(treatment_date_dt) else pd.NaT
    base = pd.DataFrame({"DateDay": pd.to_datetime(full_date_range).normalize()})
    rows: List[pd.DataFrame] = []
    for lbl, g in phrase_long_df.groupby("syllable", sort=False):
        daily = (
            g.groupby("DateDay")["Phrase Duration (ms)"]
            .agg(
                N="size",
                Variance_ms2=lambda s: float(np.var(pd.to_numeric(s, errors="coerce").dropna().to_numpy(), ddof=1))
                if pd.to_numeric(s, errors="coerce").dropna().size >= 2 else np.nan,
        ...
        daily = base.merge(daily, on="DateDay", how="left")
        daily["animal_id"] = animal_id
        daily["lesion_group"] = lesion_group
        daily["syllable"] = str(lbl)
        daily["treatment_date"] = treat_norm
        if pd.notna(treat_norm):
            daily["relative_day"] = (daily["DateDay"] - treat_norm).dt.days.astype(int)
        else:
            daily["relative_day"] = pd.Series([pd.NA] * len(daily), dtype="Int64")
        rows.append(daily[["animal_id", "lesion_group", "syllable", "DateDay", "relative_day", "Variance (ms^2)", "N", "treatment_date"]])
    if not rows:
        return pd.DataFrame(columns=["animal_id", "lesion_group", "syllable", "DateDay", "relative_day", "Variance (ms^2)", "N", "treatment_date"])
    return pd.concat(rows, ignore_index=True)


def _build_overall_variance_summary_df(
    phrase_long_df: pd.DataFrame,
    *,
    animal_id: str,
    lesion_group: Optional[str] = None,
    ...
    if phrase_long_df.empty:
        return pd.DataFrame(columns=["animal_id", "lesion_group", "syllable", "overall_variance_ms2", "N", "rank", "is_top30"])
    rows = []
    for lbl, g in phrase_long_df.groupby("syllable", sort=False):
        vals = pd.to_numeric(g["Phrase Duration (ms)"], errors="coerce").dropna().to_numpy(dtype=float)
        if vals.size < 2:
            continue
        rows.append({
            "animal_id": animal_id,
            "lesion_group": lesion_group,
            "syllable": str(lbl),
            "overall_variance_ms2": float(np.var(vals, ddof=1)),
            "N": int(vals.size),
        })
    out = pd.DataFrame(rows)
    if out.empty:
        return pd.DataFrame(columns=["animal_id", "lesion_group", "syllable", "overall_variance_ms2", "N", "rank", "is_top30"])
    out = _mark_top_fraction_in_overall_df(out, top_fraction=top_fraction)
    return out


def _write_cache_tables(
    cache_paths: Dict[str, Path],
    *,
    animal_id: str,
    treatment_date_dt: Optional[pd.Timestamp],
    full_date_range: pd.DatetimeIndex,
    lesion_group: Optional[str],
    phrase_long_df: pd.DataFrame,
    daily_variance_df: pd.DataFrame,
    overall_variance_df: pd.DataFrame,
) -> None:
    meta = pd.DataFrame([{
        "animal_id": animal_id,
        "treatment_date": pd.Timestamp(treatment_date_dt).normalize().strftime("%Y-%m-%d") if treatment_date_dt is not None and not pd.isna(treatment_date_dt) else "",
        "full_date_start": pd.Timestamp(full_date_range.min()).normalize().strftime("%Y-%m-%d"),
        "full_date_end": pd.Timestamp(full_date_range.max()).normalize().strftime("%Y-%m-%d"),
        "lesion_group": lesion_group or "",
    }])
    meta.to_csv(cache_paths["metadata"], index=False)
    phrase_long_df.to_csv(cache_paths["phrase_long"], index=False)
    daily_variance_df.to_csv(cache_paths["daily_variance"], index=False)
    overall_variance_df.to_csv(cache_paths["overall_variance"], index=False)


def _read_cache_tables(cache_paths: Dict[str, Path]) -> Dict[str, pd.DataFrame]:
    meta = pd.read_csv(cache_paths["metadata"])
    phrase = pd.read_csv(cache_paths["phrase_long"])
    daily = pd.read_csv(cache_paths["daily_variance"])
    overall = pd.read_csv(cache_paths["overall_variance"])
    for df in (phrase, daily):
        if "DateDay" in df.columns:
            df["DateDay"] = pd.to_datetime(df["DateDay"], errors="coerce").dt.normalize()
        if "treatment_date" in df.columns:
            df["treatment_date"] = pd.to_datetime(df["treatment_date"], errors="coerce").dt.normalize()
    if "treatment_date" in meta.columns:
        meta["treatment_date"] = pd.to_datetime(meta["treatment_date"], errors="coerce").dt.normalize()
    return {"metadata": meta, "phrase_long": phrase, "daily_variance": daily, "overall_variance": overall}


def _run_plots_from_cached_tables(
    *,
    phrase_long_df: pd.DataFrame,
    daily_variance_df: pd.DataFrame,
    overall_variance_df: pd.DataFrame,
    ...
    labels = _sorted_label_list(overall_variance_df["syllable"].astype(str).tolist() if not overall_variance_df.empty else phrase_long_df.get("syllable", pd.Series(dtype=str)).astype(str).tolist())
    label_colors = _load_label_colors(labels, fixed_label_colors_json=fixed_label_colors_json)
    daily_var_map: Dict[str, pd.DataFrame] = {}

    for lbl in labels:
        plot_color = label_colors.get(str(lbl), "#2E4845")
        ph = phrase_long_df.loc[phrase_long_df["syllable"].astype(str) == str(lbl)].copy()
        if not ph.empty:
            exploded = ph[["DateDay", "Phrase Duration (ms)"]].rename(columns={"DateDay": "Date"})
            if make_green_plots:
                _plot_one_syllable_duration(
                    exploded=exploded,
                    col="Phrase Duration (ms)",
                    full_date_range=full_date_range,
                    out_path=save_dir / f"{animal_id}_syllable_{lbl}_phrase_duration_plot_green.png",
                    violin_color="lightgray",
                    point_color="#2E4845",
                    y_max_ms=y_max_ms,
                    jitter=jitter,
            ...
            if make_colored_plots:
                _plot_one_syllable_duration(
                    ...
                    col="Phrase Duration (ms)",
                    full_date_range=full_date_range,
                    out_path=save_dir / f"{animal_id}_syllable_{lbl}_phrase_duration_plot_colored.png",
                    violin_color=plot_color,
                    point_color=plot_color,
                    y_max_ms=y_max_ms,
                    jitter=jitter,
        ...
        dv = daily_variance_df.loc[daily_variance_df["syllable"].astype(str) == str(lbl), ["DateDay", "Variance (ms^2)", "N"]].copy()
        if dv.empty or dv["Variance (ms^2)"].notna().sum() == 0:
            continue
        daily_var_map[str(lbl)] = dv
        if make_green_variance_plots:
            _plot_one_syllable_variance_line(
                daily_var_df=dv,
                full_date_range=full_date_range,
                out_path=save_dir / f"{animal_id}_syllable_{lbl}_phrase_duration_variance_plot_green.png",
                line_color="#2E4845",
                marker_color="#2E4845",
                y_max_variance_ms2=y_max_variance_ms2,
                figsize=variance_figsize,
        ...
        if make_colored_variance_plots:
            _plot_one_syllable_variance_line(
                daily_var_df=dv,
                full_date_range=full_date_range,
                out_path=save_dir / f"{animal_id}_syllable_{lbl}_phrase_duration_variance_plot_colored.png",
                line_color=plot_color,
                marker_color=plot_color,
                y_max_variance_ms2=y_max_variance_ms2,
                figsize=variance_figsize,
    ...
    top30_df = overall_variance_df.loc[overall_variance_df.get("is_top30", False).astype(bool)].copy() if not overall_variance_df.empty else pd.DataFrame()
    top30_labels = _sorted_label_list(top30_df.get("syllable", pd.Series(dtype=str)).astype(str).tolist())
...
def run_batch_phrase_duration_over_days_from_excel(
    ...
    *,
    ...
    make_green_aggregate_variance_plots: bool = True,
    make_colored_aggregate_variance_plots: bool = True,
    save_csv_cache: bool = True,
    reuse_existing_csv_cache: bool = True,
    overwrite_csv_cache: bool = False,
    aligned_relative_day_min: int = -30,
    aligned_relative_day_max: int = 31,
    aligned_variance_y_scale: str = "symlog",
    aligned_show_mean: bool = False,
) -> Dict[str, BatchPhraseDurationOverDaysRecord]:
    metadata_excel = Path(metadata_excel)
    json_root = Path(json_root)
    output_root = Path(output_root) if output_root is not None else None
    ...
    for animal_id, group in meta_df.groupby(id_col):
        ...
        if output_root is None:
            ...
            outdir = output_root / animal_id / "phrase_duration_over_days"
        outdir.mkdir(parents=True, exist_ok=True)
        cache_paths = _cache_paths_for_animal(outdir, animal_id)

        used_cache = False
        try:
            if reuse_existing_csv_cache and all(p.exists() for p in cache_paths.values()):
                cached = _read_cache_tables(cache_paths)
                meta_row = cached["metadata"].iloc[0]
                lesion_group_cached = str(meta_row.get("lesion_group", "")).strip() or lesion_group
                treatment_dt_cached = pd.to_datetime(meta_row.get("treatment_date"), errors="coerce")
                start_dt = pd.to_datetime(meta_row.get("full_date_start"), errors="coerce")
                end_dt = pd.to_datetime(meta_row.get("full_date_end"), errors="coerce")
                if pd.isna(start_dt) or pd.isna(end_dt):
                    raise ValueError("cache metadata missing full_date_start/full_date_end")
                full_date_range = pd.date_range(start=start_dt.normalize(), end=end_dt.normalize(), freq="D")

                phrase_long_df = cached["phrase_long"].copy()
                daily_variance_df = cached["daily_variance"].copy()
                overall_variance_df = cached["overall_variance"].copy()
                if syllables_subset is not None:
                    keep = set(map(str, syllables_subset))
                    phrase_long_df = phrase_long_df.loc[phrase_long_df["syllable"].astype(str).isin(keep)].copy()
                    daily_variance_df = daily_variance_df.loc[daily_variance_df["syllable"].astype(str).isin(keep)].copy()
                    overall_variance_df = overall_variance_df.loc[overall_variance_df["syllable"].astype(str).isin(keep)].copy()
                    overall_variance_df = _mark_top_fraction_in_overall_df(overall_variance_df, top_fraction=0.30)

                print(f"[CACHE] {animal_id} | plotting from existing CSV cache in {outdir}")
                _run_plots_from_cached_tables(
                    phrase_long_df=phrase_long_df,
                    daily_variance_df=daily_variance_df,
                    overall_variance_df=overall_variance_df,
                    full_date_range=full_date_range,
                    treatment_date_dt=treatment_dt_cached,
                    animal_id=animal_id,
                    save_dir=outdir,
                    fixed_label_colors_json=fixed_label_colors_json,
                    y_max_ms=y_max_ms,
                ...
                top30_set = set(overall_variance_df.loc[overall_variance_df.get("is_top30", False).astype(bool), "syllable"].astype(str)) if not overall_variance_df.empty else set()
                tmp = daily_variance_df.copy()
                tmp["lesion_group"] = tmp.get("lesion_group", lesion_group_cached).fillna(lesion_group_cached)
                tmp["relative_day"] = pd.to_numeric(tmp["relative_day"], errors="coerce")
                tmp["Variance (ms^2)"] = pd.to_numeric(tmp["Variance (ms^2)"], errors="coerce")
                tmp = tmp.dropna(subset=["relative_day", "Variance (ms^2)"])
                for _, row in tmp.iterrows():
                    rec = {
                        "animal_id": animal_id,
                        "syllable": str(row["syllable"]),
                        "relative_day": int(row["relative_day"]),
                        "Variance (ms^2)": float(row["Variance (ms^2)"]),
                        "lesion_group": row.get("lesion_group", lesion_group_cached),
                    }
                    batch_aligned_rows_all.append(rec)
                    if str(row["syllable"]) in top30_set:
                        batch_aligned_rows_top30.append(dict(rec))
                records[animal_id] = BatchPhraseDurationOverDaysRecord(
                    animal_id=animal_id,
                    treatment_date=treatment_date,
            ...
            print(f"[WARN] {animal_id}: cache load failed, recomputing from JSONs. {type(e).__name__}: {e}")
            used_cache = False
        ...
        try:
            ...
            phrase_long_df = _build_phrase_duration_long_df(
                df,
                labels,
                animal_id=prepared_animal_id,
                treatment_date_dt=t_dt,
            ...
            daily_variance_df = _build_daily_variance_long_df(
                phrase_long_df,
                full_date_range,
                animal_id=prepared_animal_id,
                treatment_date_dt=t_dt,
            ...
            overall_variance_df = _build_overall_variance_summary_df(
                phrase_long_df,
                animal_id=prepared_animal_id,
                lesion_group=lesion_group,
                top_fraction=0.30,
            ...
            if save_csv_cache and (overwrite_csv_cache or not all(p.exists() for p in cache_paths.values())):
                _write_cache_tables(
                    cache_paths,
                    animal_id=prepared_animal_id,
                    treatment_date_dt=t_dt,
    ...
    if not all_df.empty:
        all_df.to_csv(summary_dir / "batch_aligned_phrase_duration_variance_all.csv", index=False)
    if not top30_df.empty:
        top30_df.to_csv(summary_dir / "batch_aligned_phrase_duration_variance_top30.csv", index=False)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--aligned-relative-day-min", type=int, default=-30)
    ap.add_argument("--aligned-relative-day-max", type=int, default=31)
    ap.add_argument("--aligned-variance-y-scale", type=str, default="symlog", choices=["linear", "log", "symlog"])
    ap.add_argument("--aligned-show-mean", action="store_true")
