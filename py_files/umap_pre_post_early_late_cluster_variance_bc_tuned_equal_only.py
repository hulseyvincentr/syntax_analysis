...
- Keeps the public interface expected by the batch runner:
    * UMAPEarlyLateConfig
    * run_one_bird(cfg)
- Saves ONLY the equal-sized four-group cluster figures.
- Does not compute or save raw/unbalanced UMAP figures or raw-group BC metrics.
- Keeps the large panel on the far right of each cluster figure:
    * all pre-lesion equal-group points in purple
    * all post-lesion equal-group points in green
  so you can visually assess the overall equal-group pre/post shift.

Figure layout
-------------
Each cluster figure is a 2 x 4 equal-group layout:
...
def _analyze_one_cluster(
    *,
    ...
    row: Dict[str, Any] = {
        ...
        # Raw-group metrics intentionally omitted in this equal-groups-only version.
        ...
        "bc_pre_vs_post_equal_groups": np.nan,
    ...
    # Subsample, if requested, before balancing / plotting.
    ...
    # Require all four groups to be non-empty so equal-sized balancing is possible.
    ...
    row["bc_pre_vs_post_equal_groups"] = _bhattacharyya_coefficient_2d(
        eq_xy_pre_all, eq_xy_post_all, bins=cfg.bc_bins
    )
    ...
    _plot_cluster_figure(
        ...
        bc_pre_vs_post=row["bc_pre_vs_post_equal_groups"],
        ...
        cfg=cfg,
        label_suffix="equal groups only",
    )
