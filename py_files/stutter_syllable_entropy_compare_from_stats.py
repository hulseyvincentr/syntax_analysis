...
def plot_syllable_compare(df_cmp: pd.DataFrame, *, out_dir: Path, prefix: str, label_top_k: int = 12, min_stutter_tokens: int = 1) -> List[Path]:
    saved: List[Path] = []
    ...
    plt.title("Syllable-level outgoing entropy\n(non-stutter vs stutter; size ∝ # stutter tokens)")

    top = d.sort_values("n_stutter_tokens", ascending=False).head(int(label_top_k))
    for _, r in top.iterrows():
        plt.text(float(r["outgoing_entropy_nonstutter_bits"]), float(r["outgoing_entropy_stutter_bits"]), str(r["syllable"]), fontsize=8)

    p = out_dir / f"{prefix}__outgoing_non_vs_stutter.png"
    _savefig(p)
    saved.append(p)

    # INCOMING: non vs stutter
    ...
    top = d.sort_values("n_stutter_tokens", ascending=False).head(int(label_top_k))
    for _, r in top.iterrows():
        plt.text(float(r["incoming_entropy_nonstutter_bits"]), float(r["incoming_entropy_stutter_bits"]), str(r["syllable"]), fontsize=8)

    p = out_dir / f"{prefix}__incoming_non_vs_stutter.png"
    _savefig(p)
    saved.append(p)
