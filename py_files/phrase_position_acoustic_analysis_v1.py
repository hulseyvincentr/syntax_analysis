#!/usr/bin/env python3
"""
phrase_position_acoustic_analysis_v1.py
...
def analyze_label_phrase_position(args, cluster_label: str, out_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for period in PERIOD_ORDER:
        ...
        for run_idx, run in enumerate(runs):
            ...
            if n_segments == 0:
                continue
            for seg_order, rec in enumerate(segments, start=1):
                ...
                elapsed_start = s0 * bin_s
                elapsed_end = s1 * bin_s
    ...
    return segment_df, stats_df, early_late_df
    ...
    for feature in features:
        ...
        for group_col, group_values in [
            ...
            for group in group_values:
                ...
                valid_time = np.isfinite(x_time) & np.isfinite(y)
                valid_rep = np.isfinite(x_rep) & np.isfinite(y)
                row = {
                    "feature": feature,
                    ...
                    "n_time_valid": int(np.sum(valid_time)),
                    "n_repeat_valid": int(np.sum(valid_rep)),
                    "spearman_r_elapsed_time": np.nan,
                    "spearman_p_elapsed_time": np.nan,
                    "linear_slope_per_s": np.nan,
                    "linear_intercept_elapsed_time": np.nan,
                    "linear_r_elapsed_time": np.nan,
                    "linear_p_elapsed_time": np.nan,
                    "spearman_r_previous_repeats": np.nan,
                    "spearman_p_previous_repeats": np.nan,
                    "linear_slope_per_repeat": np.nan,
                    "linear_intercept_previous_repeats": np.nan,
                    "linear_r_previous_repeats": np.nan,
                    "linear_p_previous_repeats": np.nan,
                }
                if SCIPY_AVAILABLE and np.sum(valid_time) >= min_n:
                    sp = stats.spearmanr(x_time[valid_time], y[valid_time])
                    lr = stats.linregress(x_time[valid_time], y[valid_time])
                    row.update({
                        "spearman_r_elapsed_time": float(sp.correlation),
                        "spearman_p_elapsed_time": float(sp.pvalue),
                        "linear_slope_per_s": float(lr.slope),
                        "linear_intercept_elapsed_time": float(lr.intercept),
                        "linear_r_elapsed_time": float(lr.rvalue),
                        "linear_p_elapsed_time": float(lr.pvalue),
                    })
                elif np.sum(valid_time) >= min_n:
                    slope, intercept = np.polyfit(x_time[valid_time], y[valid_time], deg=1)
                    row.update({
                        "linear_slope_per_s": float(slope),
                        "linear_intercept_elapsed_time": float(intercept),
                    })
                if SCIPY_AVAILABLE and np.sum(valid_rep) >= min_n:
                    sp = stats.spearmanr(x_rep[valid_rep], y[valid_rep])
                    lr = stats.linregress(x_rep[valid_rep], y[valid_rep])
                    row.update({
                        "spearman_r_previous_repeats": float(sp.correlation),
                        "spearman_p_previous_repeats": float(sp.pvalue),
                        "linear_slope_per_repeat": float(lr.slope),
                        "linear_intercept_previous_repeats": float(lr.intercept),
                        "linear_r_previous_repeats": float(lr.rvalue),
                        "linear_p_previous_repeats": float(lr.pvalue),
                    })
                elif np.sum(valid_rep) >= min_n:
                    slope, intercept = np.polyfit(x_rep[valid_rep], y[valid_rep], deg=1)
                    row.update({
                        "linear_slope_per_repeat": float(slope),
                        "linear_intercept_previous_repeats": float(intercept),
                    })
                rows.append(row)
    ...
    for ax, feature in zip(axes.ravel(), features):
        ...
        for group in ["pre", "post"]:
            ...
            if max_repeats is not None:
                ...
                y = y[keep]
            ax.scatter(x, y, s=10, alpha=0.16, label=f"{group} segments")
            # Median per integer repeat count
            valid = np.isfinite(x) & np.isfinite(y)
            if np.any(valid):
