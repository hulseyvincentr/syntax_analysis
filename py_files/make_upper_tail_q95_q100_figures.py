#!/usr/bin/env python3
"""
Make upper-tail phrase-duration prolongation metrics and paper-style figures.
...
It adds two complementary q95-q100 tail metrics:
    1. tail_mean_q95_to_max_s
       Mean duration of the upper 5% of renditions, i.e. all values >= q95.
    ...
    2. tail_range_max_minus_q95_s
       max - q95, i.e. the span from q95 to q100. This directly visualizes the
...
python make_upper_tail_q95_q100_figures.py \
  ...
  --out-dir "/Volumes/my_own_SSD/updated_AreaX_outputs/prolongation_pooled_prepost_top30/q95_q100_tail_figures" \
  --n-balance 200 \
  --n-perm 10000 \
  --seed 123
"""
...
Y_LABELS = {
    "tail_mean_q95_to_max_s": "Mean phrase duration from q95-q100 (s)",
    "tail_range_max_minus_q95_s": "q100-q95 phrase-duration range (s)",
    "tail_range_q999_minus_q95_s": "q99.9-q95 phrase-duration range (s)",
    "q999_duration_s": "99.9th percentile phrase duration (s)",
    "max_duration_s": "Maximum phrase duration (s)",
}

DELTA_LABELS = {
    "delta_tail_mean_q95_to_max_s": "Δ mean q95-q100 phrase duration (s)",
    "delta_tail_range_max_minus_q95_s": "Δ q100-q95 phrase-duration range (s)",
    "delta_tail_range_q999_minus_q95_s": "Δ q99.9-q95 phrase-duration range (s)",
    "delta_q999_duration_s": "Δ 99.9th percentile phrase duration (s)",
    "delta_max_duration_s": "Δ maximum phrase duration (s)",
}

PRE_POST_METRICS = [
    "tail_mean_q95_to_max_s",
    "tail_range_max_minus_q95_s",
    "tail_range_q999_minus_q95_s",
    "q999_duration_s",
    "max_duration_s",
]
...
def tail_metrics(x: np.ndarray) -> dict[str, float]:
    ...
    return {
        "q95_duration_s": q95,
        "q99_duration_s": float(np.quantile(x, 0.99)),
        "q995_duration_s": float(np.quantile(x, 0.995)),
        "q999_duration_s": q999,
        "max_duration_s": xmax,
        "tail_mean_q95_to_max_s": float(np.mean(top)) if top.size else np.nan,
        "tail_range_max_minus_q95_s": xmax - q95,
        "tail_range_q999_minus_q95_s": q999 - q95,
        "tail_n_q95_to_max": int(top.size),
        "n": int(x.size),
    }
...
def balanced_epoch_metrics(x: np.ndarray, n: int, rng: np.random.Generator, n_balance: int) -> dict[str, float]:
    ...
    x = x[np.isfinite(x)]
    if x.size < 2 or n < 2:
        return {m: np.nan for m in list(PRE_POST_METRICS) + ["q95_duration_s", "q99_duration_s", "q995_duration_s", "tail_n_q95_to_max", "n"]}
    reps = []
    replace = False
...
def make_stats(animal: pd.DataFrame, args) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for metric in DELTA_METRICS:
        ...
        for a, b in [("medial_lateral", "lateral_only"), ("medial_lateral", "sham")]:
...
def paired_box(animal: pd.DataFrame, group_stats: pd.DataFrame, metric: str, out_dir: Path, prefix: str):
    ...
    for i, group in enumerate(GROUP_ORDER):
        ...
        p = group_stats.loc[(group_stats["group"].eq(group)) & (group_stats["metric"].eq("delta_" + metric)), "signflip_p"]
...
def delta_box(animal: pd.DataFrame, comp_stats: pd.DataFrame, metric: str, out_dir: Path, prefix: str):
    ...
    pad = (ymax - ymin) * 0.25 if ymax > ymin else 1
    ax.set_ylim(ymin - pad * 0.3, ymax + pad)
    p = comp_stats.loc[(comp_stats["metric"].eq(metric)) & (comp_stats["comparison"].eq("medial_lateral - lateral_only")), "labelshuffle_p"]
    label = p_to_star(float(p.iloc[0])) if len(p) else "n.s."
    add_bracket(ax, 1, 2, ymax + pad * 0.25, label)
    set_paper_axes(ax)
    fig.tight_layout()
...
def main():
    ...
    balanced.to_csv(out_dir / "syllable_level_q95_q100_tail_metrics_balanced.csv", index=False)
    raw.to_csv(out_dir / "syllable_level_q95_q100_tail_metrics_raw.csv", index=False)
    animal_median.to_csv(out_dir / "animal_level_q95_q100_tail_metrics_median_across_syllables.csv", index=False)
    animal_mean.to_csv(out_dir / "animal_level_q95_q100_tail_metrics_mean_across_syllables.csv", index=False)
    animal_max.to_csv(out_dir / "animal_level_q95_q100_tail_metrics_max_across_syllables.csv", index=False)

    group_stats, comp_stats = make_stats(animal_median, args)
    group_stats.to_csv(out_dir / "group_q95_q100_tail_stats_median_across_syllables.csv", index=False)
    comp_stats.to_csv(out_dir / "group_comparison_q95_q100_tail_stats_median_across_syllables.csv", index=False)

    fig_dir = out_dir / "figures"
    fig_dir.mkdir(exist_ok=True)
    for metric in PRE_POST_METRICS:
        scatter_pre_post(animal_median, metric, fig_dir, prefix="animal_median")
        paired_box(animal_median, group_stats, metric, fig_dir, prefix="animal_median")
    for metric in DELTA_METRICS:
        delta_box(animal_median, comp_stats, metric, fig_dir, prefix="animal_median")
    ...
    for metric in ["tail_mean_q95_to_max_s", "tail_range_max_minus_q95_s", "q999_duration_s", "max_duration_s"]:
        scatter_pre_post(raw.rename(columns={}), metric, raw_fig_dir, prefix="raw_syllable")

    print(f"[DONE] Wrote q95-q100 tail metrics and figures to: {out_dir}")
    print("\n[CHECK] Top raw post max values:")
    cols = ["bird", "group", "syllable", "post_q95_duration_s", "post_tail_mean_q95_to_max_s", "post_q999_duration_s", "post_max_duration_s", "delta_tail_mean_q95_to_max_s", "delta_tail_range_max_minus_q95_s", "n_post_raw"]
    ...
    cols2 = ["bird", "group", "selected_syllables", "delta_tail_mean_q95_to_max_s", "delta_tail_range_max_minus_q95_s", "delta_q999_duration_s", "delta_max_duration_s"]
    print(animal_median.sort_values("delta_tail_mean_q95_to_max_s", ascending=False)[cols2].to_string(index=False))
    print("\n[CHECK] Group stats:")
