...
def _boxplot_whisker_extents(
    ...
    *,
    ...
    """Return the min lower whisker and max upper whisker for matplotlib-style boxplots.

    This ignores outlier points, matching showfliers=False boxplots. It is used
    for boxplot-only figures so the y-axis can crop to the visible whiskers
    instead of being stretched by hidden outliers.
    """
    lows: List[float] = []
    highs: List[float] = []
    for arr in arrays:
...
def _ylim_from_box_whiskers(
    ...
    *,
    ...
    """Compact y-limits based on visible boxplot whiskers, not hidden outliers."""
    ...
    if span < float(min_span):
        ...
        low = mid - 0.5 * span
        high = mid + 0.5 * span
    bottom_pad = 0.035 * span
    top_pad = (0.10 + float(extra_top_frac)) * span
    ymin = float(low) - bottom_pad
    ymax = float(high) + top_pad
    if upper_bound is not None:
        ymax = min(float(upper_bound), ymax)
        if ymax <= ymin:
            ymax = ymin + span
...
def plot_pre_post_by_lesion(
    ...
    *,
    ...
    """Plot pre/post BC by lesion group with paired-test p-values in x tick labels.

    When boxplot_only=True, the figure omits overlaid points and paired lines,
    and the y-axis is based on visible boxplot whiskers rather than hidden
    outliers.
    """
    ...
    arrays, positions, cols, tick_labels = [], [], [], []
    ...
    for i, group in enumerate(groups):
        ...
        tick_labels.extend([
            f"Pre\n{group}",
            f"Post\n{group}\npre/post p={_fmt_p_for_label(p_by_group[group]).replace('p=', '')}" if np.isfinite(p_by_group[group]) else f"Post\n{group}\npre/post p=n/a",
        ])
    ...
    # Draw paired lines within animal/cluster only for the annotated point version.
    ...
    if bool(boxplot_only):
        ymin, ymax = _ylim_from_box_whiskers(
            arrays,
            include_zero=False,
            upper_bound=1.04,
            min_span=0.04,
    ...
    for i, group in enumerate(groups):
        ...
        if vals.size == 0:
            continue
        if bool(boxplot_only):
            # Use whiskers rather than raw point extrema when points/outliers are hidden.
        ...
        stars = _stars_from_p(p_by_group.get(group, np.nan))
    ...
    ax.set_title(title, fontsize=20, pad=30, y=1.035)
...
def plot_delta_by_lesion(
    ...
    *,
    ...
    When boxplot_only=True, the figure omits overlaid points, and the y-axis is
    based on visible boxplot whiskers rather than hidden outliers.
    """
    _safe_mkdir(Path(out_png).parent)
    groups = _ordered_groups(df, order)
    rng = np.random.default_rng(1)
    fig_w = max(10.0, 2.4 * len(groups) + 2.8)
    fig, ax = plt.subplots(figsize=(fig_w, 6.9))
    ...
    # Tight data-driven y-limits, with space for pairwise comparison brackets.
    ...
    if bool(boxplot_only):
        ymin, ymax = _ylim_from_box_whiskers(
            ...
            include_zero=True,
            min_span=0.04,
            extra_top_frac=0.28 + 0.16 * max(0, n_brackets - 1),
        )
    else:
        ymin, ymax = _auto_ylim(
            all_values,
            ...
            pad_frac=0.14,
            min_span=0.08,
            extra_top_frac=0.22 + 0.12 * max(0, n_brackets - 1),
        )
    ax.set_ylim(ymin, ymax)
    yr = ymax - ymin
    ...
    if sham_present:
        ...
        # Draw the shorter-span comparison lower and the longer-span comparison higher.
        comps.sort(key=lambda t: t[0])
        y_cursor = ymax - (n_brackets + 1.5) * bracket_h
        for _, x, group, p in comps:
            stars = _stars_from_p(p)
            label = stars if stars else "n.s."
            _draw_sig_bracket(ax, x, sham_x, y_cursor, label, h=bracket_h, fontsize=13)
    ...
    for g in groups:
        if g == SHAM_LABEL:
            ...
            ptxt = _fmt_p_for_label(p_vs_sham.get(g, np.nan)).replace('p=', '') if np.isfinite(p_vs_sham.get(g, np.nan)) else 'n/a'
            tick_labels.append(f"{g}\nvs sham p={ptxt}")
