...
# -*- coding: utf-8 -*-
"""
seasonal_contemporaneous_sham_vs_ML_v3.py
...
Compared with v1, this version:
  ...
  3) adds hierarchical bird→syllable bootstrap robustness analyses
...
def syllable_delta_table(daily: pd.DataFrame, epoch_stat: str) -> pd.DataFrame:
    """One row per animal × syllable with pre/post daily-SD summaries.

    This is the table used for hierarchical bootstrap. It preserves the nested
    structure: syllables are nested inside birds, and birds remain the
    resampling unit at the top level.
    """
    val = np.nanmedian if epoch_stat == "median" else np.nanmean
    rows = []
    for (aid, syl), g in daily.groupby(["animal_id", "syllable"], dropna=False):
        pre = g.loc[g["relative_day"] < 0, "sd_s"].dropna().to_numpy(float)
        post = g.loc[g["relative_day"] > 0, "sd_s"].dropna().to_numpy(float)
        if pre.size == 0 or post.size == 0:
            continue
        rows.append({
            "animal_id": aid,
            ...
            "post_sd_s": float(val(post)),
            "delta_sd_s": float(val(post) - val(pre)),
            "n_pre_days": int(g.loc[g["relative_day"] < 0, "relative_day"].nunique()),
            "n_post_days": int(g.loc[g["relative_day"] > 0, "relative_day"].nunique()),
            "n_daily_rows": int(len(g)),
        })
    out = pd.DataFrame(rows)
    if out.empty:
        raise ValueError("Could not compute animal×syllable delta table for hierarchical bootstrap.")
    return out.sort_values(["coarse_group", "animal_id", "syllable"]).reset_index(drop=True)
...
def _stat(values: np.ndarray, stat: str) -> float:
    ...
    if values.size == 0:
        return np.nan
    return float(np.nanmedian(values)) if stat == "median" else float(np.nanmean(values))


def _bird_delta_from_syllables(syll_delta: pd.DataFrame, animal_id: str, rng: np.random.Generator, stat: str) -> float:
    vals = syll_delta.loc[syll_delta["animal_id"].astype(str) == str(animal_id), "delta_sd_s"].dropna().to_numpy(float)
    if vals.size == 0:
        return np.nan
    sampled = rng.choice(vals, size=vals.size, replace=True)
    return _stat(sampled, stat)


def hierarchical_unmatched_bootstrap(
    syll_delta: pd.DataFrame,
    *,
    label: str,
    basis: str,
    n_boot: int,
    ...
    """Hierarchical bootstrap for the unpaired ML − sham contrast.

    Bootstrap unit hierarchy:
      1. resample birds with replacement within sham and ML groups
      2. within each resampled bird, resample selected syllables with replacement
      3. compute a bird-level ΔSD
      4. compute ML − sham group contrast

    This uses syllable-level information without treating syllables as
    independent biological replicates.
    """
    rng = np.random.default_rng(seed)
    work = syll_delta[syll_delta["coarse_group"].isin(PLOT_GROUPS)].copy()

    sham_birds = sorted(work.loc[work["coarse_group"] == "Sham", "animal_id"].dropna().astype(str).unique())
    ml_birds = sorted(work.loc[work["coarse_group"] == "Medial+lateral lesion", "animal_id"].dropna().astype(str).unique())
    if len(sham_birds) == 0 or len(ml_birds) == 0:
        return pd.DataFrame()

    def group_bird_stats(birds):
        vals = []
        for b in birds:
            v = work.loc[work["animal_id"].astype(str) == str(b), "delta_sd_s"].dropna().to_numpy(float)
            vals.append(_stat(v, stat))
        return np.asarray(vals, dtype=float)

    obs_sham = group_bird_stats(sham_birds)
    obs_ml = group_bird_stats(ml_birds)
    observed = _stat(obs_ml, stat) - _stat(obs_sham, stat)

    boot = []
    for _ in range(int(n_boot)):
        bs_sham = rng.choice(sham_birds, size=len(sham_birds), replace=True)
        bs_ml = rng.choice(ml_birds, size=len(ml_birds), replace=True)
        sham_vals = [_bird_delta_from_syllables(work, b, rng, stat) for b in bs_sham]
        ml_vals = [_bird_delta_from_syllables(work, b, rng, stat) for b in bs_ml]
        boot.append(_stat(np.asarray(ml_vals, dtype=float), stat) - _stat(np.asarray(sham_vals, dtype=float), stat))
    boot = np.asarray(boot, dtype=float)
    boot = boot[np.isfinite(boot)]

    ci_low, ci_high = np.nanpercentile(boot, [2.5, 97.5]) if boot.size else (np.nan, np.nan)
    p_directional = (np.sum(boot <= 0) + 1) / (boot.size + 1) if boot.size else np.nan

    return pd.DataFrame([{
        "selection_label": label,
        "selection_basis": basis,
        "analysis": "hierarchical bootstrap unpaired ML minus sham",
        "statistic": stat,
        "n_sham_birds": int(len(sham_birds)),
        "n_ml_birds": int(len(ml_birds)),
        "n_sham_syllables": int(work.loc[work["coarse_group"] == "Sham", ["animal_id", "syllable"]].drop_duplicates().shape[0]),
        "n_ml_syllables": int(work.loc[work["coarse_group"] == "Medial+lateral lesion", ["animal_id", "syllable"]].drop_duplicates().shape[0]),
        "observed_ML_minus_sham_delta_sd_s": float(observed),
        "hierarchical_bootstrap_CI95_low_s": float(ci_low),
        "hierarchical_bootstrap_CI95_high_s": float(ci_high),
        "bootstrap_fraction_ML_minus_sham_le_0": float(p_directional),
        "n_bootstrap": int(boot.size),
    }])


def hierarchical_matched_bootstrap(
    syll_delta: pd.DataFrame,
    ...
    *,
    ...
    """Hierarchical bootstrap for nearest-date matched sham↔ML pairs.

    Bootstrap unit hierarchy:
      1. resample matched bird pairs with replacement
      2. within each resampled bird, resample selected syllables with replacement
      3. compute ML − sham ΔSD for each pair
      4. average/median across resampled pairs
    """
    rng = np.random.default_rng(seed)
    work = syll_delta.copy()

    # Observed pair differences using the original selected syllable set.
    obs_rows = []
    for _, r in matched.iterrows():
        sham_id = str(r["sham_animal_id"])
        ml_id = str(r["ml_animal_id"])
        sham_vals = work.loc[work["animal_id"].astype(str) == sham_id, "delta_sd_s"].dropna().to_numpy(float)
        ml_vals = work.loc[work["animal_id"].astype(str) == ml_id, "delta_sd_s"].dropna().to_numpy(float)
        if sham_vals.size == 0 or ml_vals.size == 0:
            continue
        sham_delta = _stat(sham_vals, stat)
        ml_delta = _stat(ml_vals, stat)
        obs_rows.append({
            "selection_label": label,
            "selection_basis": basis,
            "pair_id": r["pair_id"],
            "sham_animal_id": sham_id,
            "ml_animal_id": ml_id,
            "sham_syllable_stat_delta_sd_s": sham_delta,
            "ml_syllable_stat_delta_sd_s": ml_delta,
            "ml_minus_sham_syllable_stat_delta_sd_s": ml_delta - sham_delta,
            "n_sham_syllables": int(sham_vals.size),
            "n_ml_syllables": int(ml_vals.size),
            "treatment_dates_days_apart": r.get("treatment_dates_days_apart", np.nan),
            "cross_year_match": r.get("cross_year_match", np.nan),
        })
    observed_pair_df = pd.DataFrame(obs_rows)
    if observed_pair_df.empty:
        return pd.DataFrame(), observed_pair_df

    observed = _stat(observed_pair_df["ml_minus_sham_syllable_stat_delta_sd_s"].to_numpy(float), stat)

    boot = []
    matched_rows = matched.reset_index(drop=True)
    for _ in range(int(n_boot)):
        sampled_idx = rng.choice(np.arange(len(matched_rows)), size=len(matched_rows), replace=True)
        diffs = []
        for idx in sampled_idx:
            r = matched_rows.iloc[int(idx)]
            sham_id = str(r["sham_animal_id"])
            ml_id = str(r["ml_animal_id"])
            sham_delta = _bird_delta_from_syllables(work, sham_id, rng, stat)
            ml_delta = _bird_delta_from_syllables(work, ml_id, rng, stat)
            if np.isfinite(sham_delta) and np.isfinite(ml_delta):
                diffs.append(ml_delta - sham_delta)
        if diffs:
            boot.append(_stat(np.asarray(diffs, dtype=float), stat))
    boot = np.asarray(boot, dtype=float)
    boot = boot[np.isfinite(boot)]
    ...
    stats = pd.DataFrame([{
        "selection_label": label,
        "selection_basis": basis,
        "analysis": "hierarchical bootstrap matched ML minus sham",
        "statistic": stat,
        "n_pairs": int(len(matched_rows)),
        "n_cross_year_matches": int(matched_rows["cross_year_match"].sum()) if "cross_year_match" in matched_rows.columns else np.nan,
        "observed_ML_minus_sham_delta_sd_s": float(observed),
        "hierarchical_bootstrap_CI95_low_s": float(ci_low),
        "hierarchical_bootstrap_CI95_high_s": float(ci_high),
        "bootstrap_fraction_ML_minus_sham_le_0": float(p_directional),
...
def run_selection(args, meta, label, basis, pairs, out_dir):
    ...
    daily["selection_label"] = label
    daily["selection_basis"] = basis
    daily.to_csv(out_dir / "seasonal_control_filtered_daily_rows.csv", index=False)

    day = animal_day_summary(daily, args.daily_stat)
    day["selection_label"] = label
    day["selection_basis"] = basis
    day.to_csv(out_dir / "seasonal_control_animal_day_summary.csv", index=False)

    delta = animal_delta(day, args.epoch_stat)
    delta["selection_label"] = label
    delta["selection_basis"] = basis
    delta.to_csv(out_dir / "seasonal_control_animal_delta_sd.csv", index=False)

    # One row per animal × syllable for hierarchical resampling.
    syl_delta = syllable_delta_table(daily, args.epoch_stat)
    syl_delta["selection_label"] = label
    syl_delta["selection_basis"] = basis
    syl_delta.to_csv(out_dir / "seasonal_control_animal_syllable_delta_sd.csv", index=False)

    matched = match_sham_ml(
        delta,
        unique_ml=not args.allow_ml_reuse,
        same_year_only=args.same_year_only,
        max_days_apart=args.max_days_apart,
    )
    matched["selection_label"] = label
    matched["selection_basis"] = basis
    matched.to_csv(out_dir / "seasonal_control_matched_pairs.csv", index=False)

    # Primary animal-level matched statistics.
    ...
    # Group-level animal summaries.
    gsum = group_summary(delta, label, basis)
    gsum.to_csv(out_dir / "seasonal_control_group_delta_summary.csv", index=False)

    # Hierarchical bootstrap robustness analyses.
    hier_unmatched = hierarchical_unmatched_bootstrap(
        syl_delta,
        label=label,
        basis=basis,
        n_boot=args.n_hier_bootstrap,
        seed=args.seed,
        stat=args.hier_stat,
    )
    hier_unmatched.to_csv(out_dir / "seasonal_control_hierarchical_bootstrap_unmatched_stats.csv", index=False)

    hier_matched, hier_pair_details = hierarchical_matched_bootstrap(
        syl_delta,
        matched,
        label=label,
        basis=basis,
    ...
    hier_matched.to_csv(out_dir / "seasonal_control_hierarchical_bootstrap_matched_stats.csv", index=False)
    hier_pair_details.to_csv(out_dir / "seasonal_control_hierarchical_matched_pair_syllable_details.csv", index=False)
    ...
    print("[INFO] Matched-pair animal-level stats:")
    print(stats.to_string(index=False))
    print("[INFO] Hierarchical bootstrap, unpaired ML-sham stats:")
    print(hier_unmatched.to_string(index=False))
    print("[INFO] Hierarchical bootstrap, matched ML-sham stats:")
    print(hier_matched.to_string(index=False))

    return daily, day, delta, syl_delta, matched, stats, gsum, hier_unmatched, hier_matched, hier_pair_details
...
def main():
    ...
    p.add_argument("--n-bootstrap", type=int, default=5000,
                   help="Bootstrap iterations for simple matched-pair animal-level CI.")
    p.add_argument("--n-hier-bootstrap", type=int, default=5000,
                   help="Bootstrap iterations for hierarchical bird→syllable resampling.")
    p.add_argument("--hier-stat", choices=["mean", "median"], default="mean",
                   help="Statistic used inside hierarchical bootstrap.")
    ...
    all_daily, all_day, all_delta, all_syl_delta = [], [], [], []
    all_matched, all_stats, all_gsum = [], [], []
    all_hier_unmatched, all_hier_matched, all_hier_pair_details = [], [], []
    ...
    for label, basis, pairs in [(pooled_label, pooled_basis, pooled_pairs), (post_label, post_basis, post_pairs)]:
        ...
        daily, day, delta, syl_delta, matched, stats, gsum, hier_unmatched, hier_matched, hier_pair_details = res
        all_daily.append(daily)
        all_day.append(day)
        all_delta.append(delta)
        all_syl_delta.append(syl_delta)
        all_matched.append(matched)
        all_stats.append(stats)
        all_gsum.append(gsum)
        all_hier_unmatched.append(hier_unmatched)
        all_hier_matched.append(hier_matched)
        all_hier_pair_details.append(hier_pair_details)
    ...
    pd.concat(all_syl_delta, ignore_index=True).to_csv(args.out_dir / "seasonal_control_all_animal_syllable_delta_sd.csv", index=False)
    ...
    hier_unmatched_all = pd.concat(all_hier_unmatched, ignore_index=True)
    hier_unmatched_all.to_csv(args.out_dir / "seasonal_control_all_hierarchical_bootstrap_unmatched_stats.csv", index=False)
    hier_matched_all = pd.concat(all_hier_matched, ignore_index=True)
    hier_matched_all.to_csv(args.out_dir / "seasonal_control_all_hierarchical_bootstrap_matched_stats.csv", index=False)
    hier_pair_details_all = pd.concat(all_hier_pair_details, ignore_index=True)
    hier_pair_details_all.to_csv(args.out_dir / "seasonal_control_all_hierarchical_matched_pair_syllable_details.csv", index=False)
    ...
    # A compact copy-ready table with the main animal-level matched result and
    # the two hierarchical bootstrap robustness analyses side by side.
    hier_unmatched_short = hier_unmatched_all.rename(columns={
        "observed_ML_minus_sham_delta_sd_s": "hier_unmatched_observed_ML_minus_sham_delta_sd_s",
        "hierarchical_bootstrap_CI95_low_s": "hier_unmatched_CI95_low_s",
        "hierarchical_bootstrap_CI95_high_s": "hier_unmatched_CI95_high_s",
        "bootstrap_fraction_ML_minus_sham_le_0": "hier_unmatched_fraction_le_0",
    })[[
        "selection_label",
        "hier_unmatched_observed_ML_minus_sham_delta_sd_s",
        "hier_unmatched_CI95_low_s",
        "hier_unmatched_CI95_high_s",
        "hier_unmatched_fraction_le_0",
    ]]

    hier_matched_short = hier_matched_all.rename(columns={
        "observed_ML_minus_sham_delta_sd_s": "hier_matched_observed_ML_minus_sham_delta_sd_s",
        "hierarchical_bootstrap_CI95_low_s": "hier_matched_CI95_low_s",
        "hierarchical_bootstrap_CI95_high_s": "hier_matched_CI95_high_s",
        "bootstrap_fraction_ML_minus_sham_le_0": "hier_matched_fraction_le_0",
    })[[
        "selection_label",
        "hier_matched_observed_ML_minus_sham_delta_sd_s",
        "hier_matched_CI95_low_s",
        "hier_matched_CI95_high_s",
        "hier_matched_fraction_le_0",
    ]]

    copy_ready = summary.merge(hier_unmatched_short, on="selection_label", how="left").merge(hier_matched_short, on="selection_label", how="left")
    copy_ready.to_csv(args.out_dir / "seasonal_control_copy_ready_summary_with_hierarchical_bootstrap.csv", index=False)

    plot_summary(stats_all, args.out_dir / "seasonal_control_summary_plot.png")

    print("\n" + "=" * 80)
    print("[OK] Wrote combined outputs to:", args.out_dir)
    print("\nCombined matched-pair animal-level stats:")
    print(stats_all.to_string(index=False))
    print("\nCombined hierarchical bootstrap, unpaired ML-sham stats:")
    print(hier_unmatched_all.to_string(index=False))
    print("\nCombined hierarchical bootstrap, matched ML-sham stats:")
    print(hier_matched_all.to_string(index=False))
