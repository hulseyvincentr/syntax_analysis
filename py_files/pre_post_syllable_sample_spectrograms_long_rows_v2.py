...
stretches such as 20,000 stitched bins.
...
def plot_pre_post_spectrogram_samples_for_labels(
    ...
    *,
    ...
    for lbl in labels_to_process:
        ...
        actual_samples = min(num_sample_spectrograms, max_pre_samples, max_post_samples)
        ...
        for k in range(actual_samples):
            ...
            # Long-row layout: pre-lesion on the top row, post-lesion underneath.
            # This is easier to inspect for large stitched-bin windows such as 20,000 bins.
            fig, axes = plt.subplots(
                2,
                1,
                figsize=(figure_width, row_height * 2.0),
                sharex=True,
                sharey=True,
            )

            im0 = axes[0].imshow(S_pre, origin="lower", aspect="auto", cmap=cmap, vmin=vmin, vmax=vmax)
            axes[0].set_title(
                f"Pre-lesion  (< {treatment_date}; {spectrogram_length:,} stitched bins)",
                loc="left",
                fontsize=14,
                pad=6,
            )
            axes[0].set_ylabel("Pre", rotation=0, labelpad=28, va="center", fontsize=13)
            _hide_spectrogram_ticks(axes[0])

            im1 = axes[1].imshow(S_post, origin="lower", aspect="auto", cmap=cmap, vmin=vmin, vmax=vmax)
            axes[1].set_title(
                f"Post-lesion (> {treatment_date}; {spectrogram_length:,} stitched bins)",
                loc="left",
                fontsize=14,
                pad=6,
            )
            axes[1].set_ylabel("Post", rotation=0, labelpad=28, va="center", fontsize=13)
            _hide_spectrogram_ticks(axes[1])

            if show_colorbar:
                fig.colorbar(im1, ax=axes.ravel().tolist(), shrink=0.75, label="Spectrogram intensity")
            ...
            if save_figures and outdir is not None:
                fname = f"label{lbl}_pre_vs_post_sample{k + 1}_N{spectrogram_length}.png"
            ...
            summary_rows.append(
                {
                    ...
                    "sample_mode": sample_mode,
                    "saved_path": str(save_path) if save_path is not None else None,
                    "note": "ok",
                }
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-sample-spectrograms", type=int, default=1, help="Number of paired samples per label.")
    ...
    parser.add_argument("--row-height", type=float, default=2.8, help="Height of each pre/post row in inches.")
