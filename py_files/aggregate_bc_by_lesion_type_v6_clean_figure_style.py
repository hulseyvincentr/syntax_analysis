...
# -*- coding: utf-8 -*-
"""
aggregate_bc_by_lesion_type_v6_clean_figure_style.py
...
def plot_bc_by_lesion_type(
    ...
    *,
    ...
    dpi: int = 300,
    show_p_value_below_xaxis: bool = True,
    clean_style: bool = False,
    ...
    fig_height = 5.2 if clean_style else 6.0
    fig, axes = plt.subplots(1, n, figsize=(max(6.5, 5.2 * n), fig_height), sharey=True)
    if n == 1:
        axes = [axes]
    ...
    for ax, lesion_type in zip(axes, lesion_types):
        ...
        p_value, stars = _paired_wilcoxon(pre, post)
    ...
    if clean_style:
        axes[0].set_ylabel("Bhattacharyya coefficient", fontsize=ylabel_fontsize)
        fig.tight_layout(rect=[0, 0.01, 1, 1.0])
    else:
        axes[0].set_ylabel("Bhattacharyya coefficient\n(early vs. late, equal time-bin sample size)", fontsize=ylabel_fontsize)
...
def main() -> None:
    ...
    p.add_argument("--boxplots-only-y-min", type=float, default=0.75,
                   help="Optional y-axis minimum used only for the boxplots-only figures.")
    p.add_argument("--boxplots-only-y-max", type=float, default=1.10,
                   help="Optional y-axis maximum used only for the boxplots-only figures.")
    p.add_argument("--dpi", type=int, default=300)
    p.add_argument("--clean-xtick-fontsize", type=int, default=25,
                   help="X tick label fontsize for clean no-title figure exports.")
    p.add_argument("--clean-ytick-fontsize", type=int, default=25,
                   help="Y tick label fontsize for clean no-title figure exports.")
    p.add_argument("--clean-ylabel-fontsize", type=int, default=25,
                   help="Y label fontsize for clean no-title figure exports.")
    args = p.parse_args()
    ...
    for spec in analysis_specs:
        ...
        for mode_label, show_lines in [
            ...
            plot_bc_by_lesion_type(
                ...
                y_max=y_max_this,
                dpi=int(args.dpi),
                show_p_value_below_xaxis=True,
                clean_style=False,
            )
            print(f"[SAVE] {plot_path}")

            clean_plot_path = out_dir / f"BC_by_lesion_type_{spec['label']}_{mode_label}_clean_no_titles.png"
            plot_bc_by_lesion_type(
                sdf,
                out_png=clean_plot_path,
                title=title,
                panel_unit_name=spec["unit_name"],
                show_connecting_lines=show_lines,
                y_min=y_min_this,
                y_max=y_max_this,
                dpi=int(args.dpi),
                show_p_value_below_xaxis=False,
                clean_style=True,
                xtick_fontsize=int(args.clean_xtick_fontsize),
                ytick_fontsize=int(args.clean_ytick_fontsize),
                ylabel_fontsize=int(args.clean_ylabel_fontsize),
            )
            print(f"[SAVE] {clean_plot_path}")
