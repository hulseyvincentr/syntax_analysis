...
def make_bird_table(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            "At least one Q75 subset was absent after intersecting Figure 3 "
            "selection with BC data. "
            f"Missing columns: {missing}. Available: {list(wide.columns)}"
        )
    ...
    if len(wide) < before:
        print(
            f"[INFO] Dropped {before - len(wide)} bird(s) lacking valid BC "
            "values for both Q75 subsets."
        )
...
def run_tests(by_bird: pd.DataFrame) -> pd.DataFrame:
    ...
    # Direct within-bird test: is the BC change more negative in the
    # Figure-3-defined >=Q75 subset than in the below-Q75 subset?
    interaction = by_bird["q75_minus_below_delta_bc"].to_numpy(dtype=float)
        ...
        [
            {
                ...
                "mean_delta_bootstrap_ci_high": ci_high,
                "wilcoxon_two_sided_p": paired_wilcoxon(
                    by_bird["below_q75_delta_bc"].to_numpy(dtype=float),
                    by_bird["q75_delta_bc"].to_numpy(dtype=float),
...
def plot_panel(
    ...
    arrays = {
        "q75_pre": by_bird["q75_pre_bc"].to_numpy(dtype=float),
        "q75_post": by_bird["q75_post_bc"].to_numpy(dtype=float),
        "below_pre": by_bird["below_q75_pre_bc"].to_numpy(dtype=float),
        "below_post": by_bird["below_q75_post_bc"].to_numpy(dtype=float),
    }
...
def write_text_summary(
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        handle.write(
            f"Pooled M+L birds with BC data in both subsets: {len(by_bird)}\n"
        )
        handle.write(
