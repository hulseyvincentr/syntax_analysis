...
def parse_args():
    ...
    p.add_argument("--annotate-ns", action="store_true")
...
def pool_group(detail):
    return POOLED_ML if detail in {PARTIAL, COMPLETE} else detail
...
def build_bird_quantiles(pair_df, quantiles):
    ...
    work["group"] = work["detail_group"].map(pool_group)
...
def plot_one_quantile(bird_df, stats_df, q, out_dir, dpi, seed, annotate_ns, show, ax=None):
    col = qlabel(q)
    own_fig = ax is None
    if own_fig:
    ...
    values = {
        g: bird_df.loc[bird_df["group"] == g, col].dropna().to_numpy(float)
        for g in GROUP_ORDER
    }

    for xpos, group in zip(positions, GROUP_ORDER):
        vals = values[group]
        ...
        ax.plot([xpos-width/2, xpos+width/2], [q50, q50], color=COLORS[group], linewidth=2.1, zorder=2)
    ...
    for xpos, group in zip(positions, GROUP_ORDER):
        ...
        ax.scatter(np.full(len(vals), xpos)+jitter, vals, s=46,
                   facecolor=COLORS[group], edgecolor="white",
                   linewidth=0.7, alpha=0.98, zorder=4)

    ax.axhline(0, color="#777777", linestyle="--", linewidth=1.0)
    ax.set_xticks(positions)
    ax.set_xticklabels([LABELS[g] for g in GROUP_ORDER])
    ...
    order = {"Lateral-only vs sham": 0, "M+L vs lateral-only": 1, "M+L vs sham": 2}
    ...
    for i, row in enumerate(selected):
        x1 = xpos[row["group1"]]
        x2 = xpos[row["group2"]]
        draw_bracket(ax, min(x1,x2), max(x1,x2), base+i*step, h,
                     p_to_label(float(row["p_holm_within_quantile"])))

    if selected:
        top_needed = base + (len(selected)-1)*step + 0.08*span
        ax.set_ylim(ax.get_ylim()[0], max(ax.get_ylim()[1], top_needed))

    if own_fig:
        fig.tight_layout()
        out = out_dir / f"{col}_deltaCV_pooled_ML_vs_controls_percentile_boxplot.png"
...
def main():
    ...
    bird_df.to_csv(out_dir / "bird_quantiles_pooled_ML.csv", index=False)

    stats_df = calculate_stats(bird_df, args.quantiles, args.seed)
    stats_df.to_csv(out_dir / "pooled_ML_pairwise_stats.csv", index=False)

    for q in args.quantiles:
        plot_one_quantile(bird_df, stats_df, q, out_dir, args.dpi, args.seed, args.annotate_ns, args.show)

    if len(args.quantiles) >= 2:
        fig, axes = plt.subplots(1, len(args.quantiles), figsize=(5.6*len(args.quantiles), 5.2))
        axes = np.atleast_1d(axes)
        for ax, q in zip(axes, args.quantiles):
            plot_one_quantile(bird_df, stats_df, q, out_dir, args.dpi, args.seed, args.annotate_ns, False, ax=ax)
            ax.set_title(qlabel(q))
        for i, ax in enumerate(axes):
            ax.text(-0.12, 1.04, chr(ord("A")+i), transform=ax.transAxes,
                    fontsize=14, fontweight="bold", va="top")
        fig.tight_layout(w_pad=2.0)
        out = out_dir / "Q75_Q90_pooled_ML_vs_controls_percentile_boxplots.png"
