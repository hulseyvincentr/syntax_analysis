    ...
    *,
    ...
    for lbl in labels_to_process:
        ...
        for k in range(actual_samples):
            ...
            axes[0].set_title(f"Pre-lesion\n< {treatment_date}; {spectrogram_length} stitched bins")
            _hide_spectrogram_ticks(axes[0])

            im1 = axes[1].imshow(S_post, origin="lower", aspect="auto", cmap=cmap, vmin=vmin, vmax=vmax)
            axes[1].set_title(f"Post-lesion\n> {treatment_date}; {spectrogram_length} stitched bins")
            _hide_spectrogram_ticks(axes[1])

            if show_colorbar:
                fig.colorbar(im1, ax=axes.ravel().tolist(), shrink=0.8, label="Spectrogram intensity")

            fig.suptitle(f"{animal_id} — HDBSCAN label {lbl} — sample {k + 1}/{actual_samples}", y=1.03)
            fig.tight_layout()
...
if __name__ == "__main__":
    print("[INFO] Import this file and call plot_pre_post_spectrogram_samples_for_labels(...).")
