...
START_COL_CANDIDATES = [
    "segment_start_bin", "segment_onset_bin", "syllable_start_bin", "syllable_onset_bin", "onset_bin", "start_bin",
    "segment_start_idx", "syllable_start_idx", "start_idx", "segment_start_frame", "start_frame", "onset_frame",
    "segment_start_ms", "syllable_start_ms", "start_ms", "onset_ms",
    "segment_start_time_ms", "syllable_start_time_ms",
]
END_COL_CANDIDATES = [
    "segment_end_bin", "segment_offset_bin", "syllable_end_bin", "syllable_offset_bin", "offset_bin", "end_bin",
    "segment_end_idx", "syllable_end_idx", "end_idx", "segment_end_frame", "end_frame", "offset_frame",
    "segment_end_ms", "syllable_end_ms", "end_ms", "offset_ms",
    "segment_end_time_ms", "syllable_end_time_ms",
]
FILE_INDEX_COL_CANDIDATES = [
    "file_index", "file_idx", "file_indices", "file_indices_stitched", "source_file_index", "dataset_file_index",
]
FILE_NAME_COL_CANDIDATES = [
    "file_name", "filename", "file_stem", "File Stem", "source_file", "source_filename", "wav_file", "audio_file"
]
...
def pick_boundary_column(columns: Iterable[str], which: str) -> Tuple[Optional[str], str]:
    cols = list(columns)
    explicit = START_COL_CANDIDATES if which == "start" else END_COL_CANDIDATES
    col = find_first_existing(cols, explicit)
    if col is not None:
        unit = "ms" if "ms" in normalize_colname(col) else "bins"
        return col, unit

    want_tokens = ["start", "onset"] if which == "start" else ["end", "offset"]
    best = None
    best_score = -1e9
    best_unit = "bins"
    for c in cols:
        ...
        score = 0
        if any(tok in n for tok in want_tokens):
            score += 6
        if "segment" in n or "syllable" in n:
            score += 3
        if "bin" in n:
            score += 3
        ...
        if "frame" in n:
            score += 2
        if n.endswith("ms") or "_ms_" in n or "_ms" in n:
            score += 1
        if "phrase" in n and not ("segment" in n or "syllable" in n):
            score -= 4
        if "elapsed" in n:
            score -= 6
        if score > best_score and any(tok in n for tok in want_tokens):
            best = c
            best_score = score
            best_unit = "ms" if (n.endswith("ms") or "_ms_" in n or "_ms" in n) else "bins"
    return best, best_unit


def build_file_index_lookup(file_map_obj) -> dict:
    mapping = {}
    def add_key(k, i):
        if k is None:
            return
    ...
    if isinstance(file_map_obj, dict):
        ...
        for k, v in items:
            try:
                ...
                pass
        return mapping
    arr = np.asarray(file_map_obj, dtype=object).ravel()
...
def load_npz_bundle(npz_path: Path) -> dict:
    out = {"spec": None, "file_index_lookup": {}, "file_map_key": ""}
    try:
    ...
    for key in ["s", "spectrogram", "spectrograms", "S"]:
        if key in z.files:
            ...
            out["spec"] = arr
            break
    for key in ["file_map", "file_names", "files", "wav_files", "audio_files"]:
        if key in z.files:
            try:
                obj = z[key]
                # object arrays sometimes wrap a single dict
                if isinstance(obj, np.ndarray) and obj.dtype == object and obj.size == 1:
                    item = obj.ravel()[0]
                    if isinstance(item, dict):
                        obj = item
                out["file_index_lookup"] = build_file_index_lookup(obj)
                out["file_map_key"] = key
                break
            except Exception:
                pass
    return out
...
def spectrogram_file_count(spec) -> int:
    if spec is None:
        return 0
...
def get_file_index_from_row(row: pd.Series, spec, file_col: Optional[str], file_name_col: Optional[str], file_index_lookup: dict) -> Optional[int]:
    fi = parse_first_int(row[file_col]) if file_col and file_col in row.index else None
    nfiles = spectrogram_file_count(spec)
    if fi is not None and fi >= 0 and (nfiles == 0 or fi < nfiles):
        return fi
    if file_name_col and file_name_col in row.index and file_index_lookup:
        raw = row[file_name_col]
        if not pd.isna(raw):
            for key in [str(raw), Path(str(raw)).name, Path(str(raw)).stem]:
                if key in file_index_lookup:
                    return int(file_index_lookup[key])
    if nfiles == 1:
        return 0
    return None


def convert_boundary_to_bin(value, unit: str, bin_ms: float) -> Optional[int]:
    if pd.isna(value):
        return None
    try:
        x = float(value)
    ...
    if not np.isfinite(x):
        return None
    if unit == "ms":
        return int(round(x / bin_ms))
    return int(round(x))


def extract_spectrogram(row: pd.Series, spec, start_col: Optional[str], end_col: Optional[str], file_col: Optional[str], start_unit: str = "bins", end_unit: str = "bins", file_name_col: Optional[str] = None, file_index_lookup: Optional[dict] = None, bin_ms: float = 2.7) -> Optional[np.ndarray]:
    if spec is None or start_col is None or end_col is None:
        return None
    start = convert_boundary_to_bin(row[start_col], start_unit, bin_ms)
    end = convert_boundary_to_bin(row[end_col], end_unit, bin_ms)
    ...
    if isinstance(spec, np.ndarray):
        if spec.ndim == 3:
            fi = get_file_index_from_row(row, spec, file_col, file_name_col, file_index_lookup or {})
            if fi is None or fi < 0 or fi >= spec.shape[0]:
                return None
            arr = spec[fi]
        elif spec.ndim == 2:
            arr = spec
        elif spec.ndim == 1 and spec.dtype == object:
            fi = get_file_index_from_row(row, spec, file_col, file_name_col, file_index_lookup or {})
            if fi is None or fi < 0 or fi >= len(spec):
                return None
            arr = np.asarray(spec[fi])
        else:
            return None
    else:
        try:
...
def plot_pair_examples_for_label(
    ...
    npz_bundle = load_npz_bundle(npz_path) if npz_path is not None else {"spec": None, "file_index_lookup": {}, "file_map_key": ""}
    spec = npz_bundle.get("spec")
    file_index_lookup = npz_bundle.get("file_index_lookup", {})
    start_col, start_unit = pick_boundary_column(sub.columns, "start")
    end_col, end_unit = pick_boundary_column(sub.columns, "end")
    file_col = find_first_existing(sub.columns, FILE_INDEX_COL_CANDIDATES)
    file_name_col = find_first_existing(sub.columns, FILE_NAME_COL_CANDIDATES)
    ...
    for r, (idx, row) in enumerate(sel.iterrows()):
        ...
        current_seg = extract_spectrogram(row, spec, start_col, end_col, file_col, start_unit=start_unit, end_unit=end_unit, file_name_col=file_name_col, file_index_lookup=file_index_lookup, bin_ms=bin_ms) if can_draw_specs else None
        ref_seg = extract_spectrogram(ref_row, spec, start_col, end_col, file_col, start_unit=start_unit, end_unit=end_unit, file_name_col=file_name_col, file_index_lookup=file_index_lookup, bin_ms=bin_ms) if (can_draw_specs and ref_row is not None) else None
        ...
        out_rows.append({
            ...
            "start_col": start_col or "", "end_col": end_col or "", "start_unit": start_unit, "end_unit": end_unit,
            "file_col": file_col or "", "file_name_col": file_name_col or "", "file_map_key": npz_bundle.get("file_map_key", ""),
        })
...
def main():
    ...
    p.add_argument("--bin-ms", type=float, default=2.7, help="Bin size in ms for converting *_ms boundary columns to spectrogram bins")
    args = p.parse_args()
