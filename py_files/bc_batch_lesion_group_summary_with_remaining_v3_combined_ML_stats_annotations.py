...
# -*- coding: utf-8 -*-
"""
bc_batch_lesion_group_summary_with_remaining_v3_combined_ML_stats_annotations.py
...
def plot_pre_post_by_lesion(
    ...
    *,
    ...
    Within each lesion group, the pre vs post comparison uses a paired Wilcoxon
    signed-rank test. The p-value is printed under the x tick label, and a
    bracket with stars is drawn above the corresponding pre/post boxes.
    """
    _safe_mkdir(Path(out_png).parent)
    groups = _ordered_groups(df, order)
    rng = np.random.default_rng(0)
    fig_w = max(9.8, 2.25 * len(groups) + 2.8)
    ...
    # Tight data-driven y-limits, with extra room for significance brackets.
    ...
    for i, group in enumerate(groups):
        ...
        y = min(ymax - 2.8 * bracket_h, local_top + 0.05 * yr)
    ...
    ax.set_xticklabels([_with_p_label(g, p_by_group.get(g, np.nan), comparison_prefix="pre/post p") for g in groups], rotation=24, ha="right")
...
def plot_delta_by_lesion(
    ...
    *,
    ...
    The delta is Post BC - Pre BC. For the non-sham groups, the x tick label
    includes a Mann-Whitney U p-value versus sham, and brackets/stars mark the
    pairwise comparisons to sham on the plot.
    """
    _safe_mkdir(Path(out_png).parent)
    groups = _ordered_groups(df, order)
    rng = np.random.default_rng(1)
    fig_w = max(9.0, 1.9 * len(groups) + 2.4)
    ...
    ymin, ymax = _auto_ylim(
        ...
        pad_frac=0.14,
        min_span=0.08,
        extra_top_frac=0.16 + 0.08 * max(0, n_brackets - 1),
    )
    ax.set_ylim(ymin, ymax)
    yr = ymax - ymin
    bracket_h = max(0.008, 0.025 * yr)

    if sham_present:
        sham_x = groups.index(SHAM_LABEL)
        y_cursor = ymax - (n_brackets + 1.0) * bracket_h
        for group in groups:
            if group == SHAM_LABEL:
                continue
            ...
            _draw_sig_bracket(ax, x, sham_x, y_cursor, label, h=bracket_h, fontsize=12)
            y_cursor += 1.45 * bracket_h
    ...
    for g in groups:
        if g == SHAM_LABEL:
            ...
            tick_labels.append(_with_p_label(g, p_vs_sham.get(g, np.nan), comparison_prefix="vs sham p"))
    ax.set_xticks(range(len(groups)))
