#!/usr/bin/env python3
"""
run_cluster_pitch_entropy_high_phrase_variance_medial_lateral_batch_v30.py
...
def load_medial_lateral_animals(
    ...
    # Primary filter from the summarized lesion-hit-type label.
    hit_type_ml = work[hit_type_col].map(_is_medial_lateral_hit)

    # More robust filter for this metadata workbook: some birds with complete/large
    # lesions are labeled "large lesion Area X not visible" in Lesion hit type, so
    # the words Medial/Lateral are not present in that column. In those cases,
    # use the parsed medial and lateral hit-type columns when they are available.
    medial_col = _find_first_existing(work.columns, ["Medial Area X hit type", "Medial hit type", "medial_hit_type"])
    lateral_col = _find_first_existing(work.columns, ["Lateral Area X hit type", "Lateral hit type", "lateral_hit_type"])
    if medial_col is not None and lateral_col is not None:
        medial_hit = work[medial_col].map(lambda x: _normalize_text_for_filter(x) not in {"", "miss", "nan", "none", "no"})
        lateral_hit = work[lateral_col].map(lambda x: _normalize_text_for_filter(x) not in {"", "miss", "nan", "none", "no"})
        parsed_ml = medial_hit & lateral_hit
    else:
        parsed_ml = pd.Series(False, index=work.index)

    work["_is_medial_lateral"] = hit_type_ml | parsed_ml
    work["_medial_lateral_filter_source"] = np.where(
        hit_type_ml, "lesion_hit_type_label",
    ...
    if verbose:
        ...
        extra_cols = []
        if "medial_col" in locals() and medial_col is not None:
            extra_cols.append(f"medial_col={medial_col!r}")
        if "lateral_col" in locals() and lateral_col is not None:
            extra_cols.append(f"lateral_col={lateral_col!r}")
        extra = (", " + ", ".join(extra_cols)) if extra_cols else ""
        print(f"[INFO] Lesion metadata columns: animal_col={animal_col!r}, hit_type_col={hit_type_col!r}{extra}")
...
def main():
    ...
    # Determine which animals are eligible before selecting high-variance labels.
    lesion_meta_used = None
    requested_animals = [str(a).strip() for a in args.animal_id] if args.animal_id else None

    if args.lesion_filter == "medial_lateral":
        ml_animals, lesion_meta_used = load_medial_lateral_animals(
        ...
        if requested_animals:
            requested_set = set(requested_animals)
            excluded = sorted(requested_set - ml_animals)
            if excluded:
                print(f"[WARN] Requested animal(s) excluded by Medial+Lateral lesion filter: {excluded}")
            animal_ids_for_selection = sorted(requested_set & ml_animals)
        else:
            animal_ids_for_selection = sorted(ml_animals)
    else:
        animal_ids_for_selection = requested_animals

    if animal_ids_for_selection is not None and len(animal_ids_for_selection) == 0:
        raise ValueError(
            "No animals remain after applying the requested animal IDs and lesion filter. "
            "Use --lesion-filter all to disable the Medial+Lateral filter, or check the metadata sheet."
        )
