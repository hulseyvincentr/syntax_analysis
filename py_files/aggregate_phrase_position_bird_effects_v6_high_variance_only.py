...
def plot_pre_post_distribution_effect_by_bird(distribution_df, distribution_bird_df, stat_df, feature, out_png, title_prefix):
    ...
    ax.set_xticks(xs)
    ax.set_xticklabels(birds, rotation=30, ha="right")
    ax.set_ylabel("Post - pre median degradation value")
...
def plot_pre_vs_post_distribution_paired_birds(distribution_bird_df, stat_df, feature, out_png, title_prefix):
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["Pre", "Post"])
    ax.set_ylabel(f"Bird median {degradation_axis_label(feature)}")
...
def plot_aggregate_binned_position(df, feature, predictor, out_png, title_prefix, n_bins=40, max_x=None, min_bin_n=10, stat_df=None):
    ...
    ptxt = distribution_stat_subtitle(stat_df, feature) if stat_df is not None else ""
    add_pvalue_note(ax, ptxt)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default=None,
                   help="Output folder. Default: <phrase-position-root>/aggregate_bird_effects_v6_high_variance_only")
...
def write_readme(out_dir: Path):
    (out_dir / "README_phrase_position_bird_effects_v6.txt").write_text(
...
def main():
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else root / "aggregate_bird_effects_v6_high_variance_only"
    ...
    if not distribution_stat_df.empty:
        stat_df = pd.concat([stat_df, distribution_stat_df], ignore_index=True, sort=False)
    ...
    print(f"[SAVED] {out_dir / 'README_phrase_position_bird_effects_v6.txt'}")
    print("[DONE]")
