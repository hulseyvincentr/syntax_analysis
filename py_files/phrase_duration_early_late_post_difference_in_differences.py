...
# -*- coding: utf-8 -*-
"""
phrase_duration_early_late_post_difference_in_differences.py
    ...
    *,
    ...
    for metric_index, (metric_name, spec) in enumerate(METRIC_SPECS.items()):
        for group_index, group_name in enumerate(PLOT_GROUPS):
            ...
            values = subset[spec["did_col"]].to_numpy(float)
            ...
            rows.append(
                {
                    ...
                    "median_baseline_change": float(
                        np.nanmedian(subset[spec["baseline_col"]].to_numpy(float))
                    ),
                    "median_lesion_change": float(
                        np.nanmedian(subset[spec["lesion_col"]].to_numpy(float))
                    ),
    ...
    *,
    ...
    for group_index, group_name in enumerate(PLOT_GROUPS):
        ...
        for j, (_, row) in enumerate(subset.iterrows()):
            ...
            y1 = row[spec["baseline_col"]]
            y2 = row[spec["lesion_col"]]
        ...
        med_baseline = np.nanmedian(subset[spec["baseline_col"]].to_numpy(float))
        med_lesion = np.nanmedian(subset[spec["lesion_col"]].to_numpy(float))
        ax.scatter(
            [x_baseline, x_lesion],
        ...
        if not group_stats.empty:
            row = group_stats.iloc[0]
            annotation = (
                f"{GROUP_LABELS[group_name]}\n"
                f"n={int(row['n_birds'])}, "
                f"DID p={format_p(row['signflip_p_raw'])}"
            )
            ...
            offset = 0.08 * yspan if np.isfinite(yspan) and yspan > 0 else 0.05
            ax.text(
                midpoint,
                ymax + offset,
                annotation,
...
def plot_did_with_ci(
    ...
    *,
    ...
    stats_indexed = (
        within_stats[within_stats["metric"] == metric_name]
    ...
    for group_index, group_name in enumerate(PLOT_GROUPS):
        ...
        row = stats_indexed.loc[group_name]
        estimate = float(row["observed_DID"])
        ...
        ax.annotate(
            f"n={int(row['n_birds'])}\n"
            f"p={format_p(row['signflip_p_raw'])}",
            xy=(group_index, high),
            xytext=(0, 8),
    ...
    ax.set_xticklabels(
        [GROUP_LABELS[group] for group in PLOT_GROUPS],
        rotation=18,
        ha="right",
    )
