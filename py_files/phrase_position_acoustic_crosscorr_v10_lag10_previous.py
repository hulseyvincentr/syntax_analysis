...
Version 6 keeps the post-lesion-only tail plots, dynamic x-axis behavior, and optional stitching across _segment_N chunks. It also adds target-vs-other majority-vote smoothing for phrase-run detection so phrase durations/repeat counts are not broken by brief decoder label interruptions.
...
def make_target_phrase_mask(
    ...
    raw_mask = local_labels == str(target_label)
